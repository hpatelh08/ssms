...
2. Parent login for Class 8-10 via `parent_id` + password.
3. Parent login for Class 1-7 via `student_id` + `parent_access_key`.
...
6. Student ID.
7. Student password.
8. Parent access key generation for Classes 1-7 from phone digits.
9. Conditional fee status logic:
10. Classes 1-8 default fee status forced to `Paid`.
11. Classes 9-10 use pending/partial/paid flow.
12. Student update and delete operations.
13. Student detail view with linked parent enrichment (for Class 8-10 links).
...
4. Fee operations restricted to Class 9-10.
...
1. Parent account creation for Class 8-10 student guardians.
...
1. Admin creates parent profile and selects children (Class 8-10).
...
| POST | `/api/students/access-key-login` | Parent login for Class 1-7 students |
| POST | `/api/parents/login` | Parent login for Class 8-10 |
...
3. Fee workflow is intentionally constrained to Class 9-10; Class 1-8 are treated as no-fee in current business logic.
4. Parent authentication is split by class range:
5. Class 1-7 uses student access key.
6. Class 8-10 uses parent account credentials.
