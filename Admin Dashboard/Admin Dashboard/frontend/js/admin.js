document.addEventListener('DOMContentLoaded', () => {
  ...
  const sections = {
    ...
    parents: document.getElementById('parents-section'),
  ...
  async function refreshDashboardFeeStats() {
    ...
    const totals = (Array.isArray(fees) ? fees : []).reduce((acc, fee) => {
      acc.pending += Math.max(Number(fee?.due) || 0, 0);
      acc.revenue += Math.max(Number(fee?.paid) || 0, 0);
      return acc;
    }, { pending: 0, revenue: 0 });

    const feeStat = scope.querySelector('#stat-fees');
    if (feeStat) feeStat.dataset.shortTarget = totals.pending;

    const revenueStat = scope.querySelector('#stat-revenue');
    if (revenueStat) revenueStat.dataset.shortTarget = totals.revenue;
  ...
  async function initDashboardSection() {
    ...
    (async function buildTodayAttendance() {
      ...
      async function fetchTodayAtt() {
        try {
          const res = await fetch('/api/dashboard/today-attendance', {
      ...
      function renderToday(d) {
        if (!d) return;
        const s = d.students, t = d.teachers;

        scope.querySelector('#todayS_present').textContent  = s.present;
        scope.querySelector('#todayS_absent').textContent   = s.absent;
        scope.querySelector('#todayS_leave').textContent    = s.leave;
        scope.querySelector('#todayS_unmarked').textContent = Math.max(0, s.total - s.marked);
        scope.querySelector('#todayS_marked').textContent   = s.marked;
        scope.querySelector('#todayS_total').textContent    = s.total;
        scope.querySelector('#todayS_pct').textContent      = s.percent + '%';
        scope.querySelector('#todayS_bar').style.width      = s.percent + '%';
      ...
      setInterval(async () => {
        const fresh = await fetchTodayAtt();
        renderToday(fresh);
      }, 60000);
    })();
    ...
    (async function buildDashHeatmap() {
      ...
      async function fetchData(type) {
        ...
        try {
          const res = await fetch(`/api/dashboard/attendance-heatmap?type=${type}`, {
            headers: { Authorization: 'Bearer ' + localStorage.getItem('ssms_token') }
          });
          if (res.ok) { const _j = await res.json(); cache[type] = _j.data || []; const _yl = scope.querySelector('#dashHmYearLabel'); if (_yl && _j.academicYear) _yl.textContent = _j.academicYear; return cache[type]; }
        } catch (_) {}
        return [];
      }
      ...
      const initData = await fetchData(currentType);
      ...
      render(initData, 'Student', initHolidays, initVacations);

      // Toggle buttons
      if (card) {
        card.querySelectorAll('.dash-hm-btn').forEach(btn => {
          btn.addEventListener('click', async () => {
            card.querySelectorAll('.dash-hm-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            currentType = btn.dataset.type;
            const label = currentType === 'student' ? 'Student' : 'Teacher';
            container.innerHTML = '<p style="color:#94a3b8;text-align:center;padding:24px 0;">Loading...</p>';
    ...
    (async function buildFeeStatus() {
      ...
      const classData = await loadClassFeeSummary();
      const feeApplicableClasses = (classData || [])
        .filter(c => {
          const cls = parseInt(c.class, 10);
          return cls === 9 || cls === 10;
        })
        .sort((a, b) => parseInt(b.class, 10) - parseInt(a.class, 10));
      const colors = ['purple','blue','green','orange','red','teal'];
      scope.querySelector('#classWiseFee').innerHTML = feeApplicableClasses.map((c,i) => {
        const paidCount = c.paid_count || 0;
        const totalStudents = c.total_students || 0;
        const collectedAmt = c.collected_amount || 0;
        const totalAmt = c.total_amount || 0;
        const amountPct = totalAmt > 0 ? Math.round((collectedAmt / totalAmt) * 100) : 0;
        return `
  ...
  async function initStudentsSection() {
    ...
    function renderTable() {
      ...
      if (!students.length) {
        els.table.innerHTML = `<tr><td colspan="11" class="text-center" style="padding:40px;color:#94a3b8;">No students found.</td></tr>`;
        return;
      }
    ...
    function renderCardView() {
      if (!students.length) {
        els.cardView.innerHTML = `<div style="padding:40px;text-align:center;color:#94a3b8;">No students found.</div>`;
        return;
      }
    ...
    function renderPagination() {
      ...
      for (let p = 1; p <= totalPages; p++) {
        nums += `<button class="page-btn ${p === currentPage ? 'active' : ''}" onclick="goStudentPage(${p})">${p}</button>`;
      }
    ...
    section.querySelector('#f-class').addEventListener('change', syncStudentFeeControls);
    ...
    els.saveBtn.addEventListener('click', async () => {
      ...
      const entry = {
        ...
        class: section.querySelector('#f-class').value,
        section: section.querySelector('#f-section').value,
        parent: section.querySelector('#f-parent').value.trim(),
        phone: section.querySelector('#f-phone').value.trim(),
      ...
      if (editId) {
        await apiUpdateStudent(editId, entry);
      } else {
        await apiCreateStudent(entry);
      }
      closeModalFn();
      await fetchStudents();
    ...
    window.viewStudent = async (id) => {
      ...
      const isJunior = cls >= 1 && cls <= 7;
      const isSenior = cls >= 8 && cls <= 10;

      let accessKeyRow = '';
      if (isJunior) {
        accessKeyRow = infoRow('🔐 Parent Access Key',
          `<span style="font-family:monospace;font-weight:700;font-size:1.1rem;letter-spacing:.12em;color:#4f46e5;">${s.parent_access_key || 'N/A'}</span>`);
      } else if (isSenior) {
        if (s.parent_id) {
          accessKeyRow =
          ...
          accessKeyRow = infoRow('👪 Parent Module',
            '<span style="color:#059669;font-size:0.82rem;">No parent account linked yet — create one in the Parents section</span>');
        }
      }
  ...
  async function initTeachersSection() {
    ...
    function renderTable() {
      ...
      els.table.innerHTML = teachers.map((t, i) => {
        ...
        <tr>
          ...
          <td style="font-family:monospace;font-size:0.8rem;">${t.emp}</td>
          <td><span class="badge badge-purple">${t.subject}</span></td>
          <td style="font-size:0.82rem;">${t.class}</td>
          ...
          <div class="flip-card-inner">
            <div class="flip-card-front student-card">
              ...
              <div class="student-card-body">
                ...
                <div class="student-card-row"><span>Classes</span><strong>${t.class}</strong></div>
    ...
    function closeModalFn() { els.modal.classList.add('hidden'); editId = null; clearForm(); }
    function clearForm() {
      ['t-f-name','t-f-emp','t-f-teacherid','t-f-teacherpass','t-f-subject','t-f-class','t-f-email','t-f-phone','t-f-qual','t-f-salary','t-f-join'].forEach(id =>
    ...
    els.saveBtn.addEventListener('click', async () => {
      ...
      if (!name) { alert('Name is required.'); return; }
      const entry = {
        name, emp: section.querySelector('#t-f-emp').value.trim(),
    ...
    window.viewTeacher = (id) => {
      ...
      els.viewBody.innerHTML = `
        <div style="text-align:center;padding:10px 0 18px;">
          <div style="width:64px;height:64px;border-radius:50%;background:${avatarColor(t.name)};
            display:flex;align-items:center;justify-content:center;font-size:1.6rem;font-weight:700;
            color:#fff;margin:0 auto 14px;">${t.name.charAt(0)}</div>
          <div style="font-size:1.1rem;font-weight:700;">${t.name}</div>
          <div style="font-size:0.82rem;color:#64748b;">${t.subject} &nbsp;|&nbsp; ${t.emp}</div>
        </div>
        <table class="view-details-table">
          ${tInfoRow('📚 Subject', t.subject)}
          ${tInfoRow('🏫 Classes', t.class)}
          ${tInfoRow('🎓 Qualification', t.qualification)}
          ${isAdmin ? tInfoRow('🆔 Teacher ID', t.teacher_id || '—') : ''}
          ${isAdmin ? `<tr><td class="vdt-label">🔑 Password</td><td class="vdt-value"><div style="display:flex;align-items:center;gap:8px;"><span id="tv-pwd-${t.id}" style="font-family:monospace;letter-spacing:1px;" data-pwd="${(t.teacher_password||'').replace(/"/g,'&quot;')}" data-visible="0">••••••</span><button class="btn btn-ghost btn-sm btn-icon" style="padding:2px 4px;font-size:0.75rem;" onclick="(function(btn){var sp=btn.previousElementSibling;if(sp.dataset.visible==='1'){sp.textContent='••••••';sp.dataset.visible='0';btn.textContent='👁';}else{sp.textContent=sp.dataset.pwd;sp.dataset.visible='1';btn.textContent='🙈';}})(this)">👁</button></div></td></tr>` : ''}
          ${tInfoRow('📧 Email', t.email)}
          ${tInfoRow('📞 Phone', t.phone)}
          ${tInfoRow('💰 Salary', '\u20B9' + (t.salary || 0).toLocaleString('en-IN'))}
          ${tInfoRow('📅 Joined', t.join || '—')}
          ${tInfoRow('✅ Status', `<span class="badge ${statusBadge(t.status)}">${t.status}</span>`)}
        </table>`;
      els.viewModal.classList.remove('hidden');
    };
  ...
  async function initAttendanceSection() {
    ...
    async function loadPeople() {
      ...
      const selectedDate = attDate.value || new Date().toISOString().split('T')[0];
      const detail = await loadAttendanceDayDetail(selectedDate, personType, cls);
      ...
      people = await loadAttendancePeople(personType, cls);
    ...
    section.querySelector('#saveAttBtn').addEventListener('click', async () => {
      ...
      const records = attRecords.map(r => ({
        ...
        status: r.status,
        class: personType === 'student' ? (attClass.value || '') : null,
        subject: null
      }));

      const result = await saveAttendanceBulk({
        records, date, person_type: personType, class: attClass.value || null
      });
    ...
    async function loadAndRenderReport() {
      ...
      weeks.forEach(week => {
        ...
        week.forEach((day, rowIdx) => {
          ...
          if (cls !== 'lv-vacation' && cls !== 'lv-holiday' && !isSunday && d && d.date) clickDate = d.date;
    ...
    async function refreshVacationList() {
      ...
      list.innerHTML = vacs.map(v => `
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:8px;background:#f8fafc;">
          <div>
          ...
          <button class="btn btn-danger btn-sm" data-vacid="${v.id}" style="white-space:nowrap;">🗑️ Delete</button>
        </div>`).join('');

      list.querySelectorAll('[data-vacid]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const id = btn.dataset.vacid;
          const name = btn.closest('div[style]').querySelector('[style*="font-weight:700"]').textContent;
    ...
    section.querySelector('#exportAttBtn').addEventListener('click', () => {
      ...
      const month = parseInt(reportMonthSel.value);
      const token = localStorage.getItem('ssms_token');
      const url = `http://localhost:5001/api/attendance/export?year=${year}&month=${month}&person_type=${personType}&token=${token}`;
  ...
  async function initFeesSection() {
    ...
    function populateClassOptions() {
      ...
      const classes = [...new Set(
        fees
          .map((fee) => parseClassNumber(fee.cls))
    ...
    function populateDivisionOptions(selectedClass = '') {
      ...
      const classValue = String(selectedClass || '').trim();
      const divisions = [...new Set(
        fees
          .filter((fee) => !classValue || String(parseClassNumber(fee.cls)) === classValue)
          .map((fee) => parseDivision(fee.cls))
          .filter(Boolean)
      )].sort();
    ...
    function isZeroFeeClass(clsValue) {
      const classNo = parseClassNumber(clsValue);
      return classNo !== null && classNo <= 8;
    }

    function displayFeeAmount(clsValue, amount) {
      return isZeroFeeClass(clsValue) ? '-' : formatMoney(amount);
    }
    ...
    function updateStats() {
      ...
      const totalDue     = fees.filter(f => f.status !== 'Paid').reduce((s,f) => s + f.due, 0);
    ...
    function renderTable() {
      ...
      els.table.innerHTML = slice.map((f, i) => `
        <tr>
          ...
          <td style="color:${f.due > 0 ? '#dc2626' : '#94a3b8'};font-weight:700;">${isZeroFeeClass(f.cls) ? '-' : (f.due > 0 ? formatMoney(f.due) : '—')}</td>
    ...
    function applyFilters() {
      ...
      filtered = fees.filter((f) => {
        const feeClass = String(parseClassNumber(f.cls) || '');
        const feeDivision = parseDivision(f.cls);
        const searchMatch = !s || f.student.toLowerCase().includes(s);
        const classMatch = !cls || feeClass === cls;
        const divisionMatch = !division || feeDivision === division;
        const statusMatch = !st || f.status === st;
        return searchMatch && classMatch && divisionMatch && statusMatch;
      });
    ...
    window.markFeePaid = async (id) => {
      ...
      const idx = fees.findIndex(x => x.id === id);
    ...
    window.showReceipt = async (id) => {
      ...
      receiptBody.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;">
          ...
          ${receiptRow('Balance Due', isZeroFeeClass(f.cls) ? '-' : (f.due > 0 ? formatMoney(f.due) : '—'))}
          ${issuedLine}
        </div>
    ...
    fpClassSel.addEventListener('change', async () => {
      ...
      try {
        const res = await loadStudents({ class: classNum + (secVal || ''), limit: 300, status: 'Active' });
...
<div class="status-box">${f.status}</div>
    ...
    section.querySelector('#saveFee').addEventListener('click', async () => {
      ...
      closeFeeModal();
      filtered = [...fees]; currentPage = 1; renderTable(); renderPagination(); updateStats();
    ...
    (function buildFeeStructureCards() {
      ...
      grid.innerHTML = FEE_STRUCTURE.filter(f => parseInt(f.class, 10) >= 9).map(f => {
        const noFee = parseInt(f.class, 10) <= 8;
        const totalText = noFee ? '-' : formatMoney(f.total);
        const tuitionText = noFee ? '-' : formatMoney(f.tuition);
        const sportsText = noFee ? '-' : formatMoney(f.sports);
        const miscText = noFee ? '-' : formatMoney(f.misc);
        const labText = noFee ? '-' : formatMoney(f.lab);
        const labRow = (noFee || f.lab > 0) ? `<div class="fee-row"><span>Lab Fee</span><span>${labText}</span></div>` : '';
        return `
    ...
    updateStats(); renderTable(); renderPagination();
  }
  ...
  /* ---------- PARENTS ---------- */
  async function initParentsSection() {
    const section = sections.parents;
    let parents = [];
    let editParentId = null; // db id when editing
    let pendingCredentials = null; // { parent_id, password, dbId } after create

    const elsP = {
      createBtn:  section.querySelector('#createParentBtn'),
      modal:      section.querySelector('#parentModal'),
      modalTitle: section.querySelector('#parentModalTitle'),
      closeModal: section.querySelector('#closeParentModal'),
      cancelModal:section.querySelector('#cancelParentModal'),
      saveBtn:    section.querySelector('#saveParent'),
      table:      section.querySelector('#parentsTable'),
      search:     section.querySelector('#par-search'),
      active:     section.querySelector('#par-active'),
      inactive:   section.querySelector('#par-inactive'),
      credModal:  section.querySelector('#credentialsModal'),
      credId:     section.querySelector('#cred-par-id'),
      credPwd:    section.querySelector('#cred-par-pwd'),
      closeCred:  section.querySelector('#closeCredModal'),
      closeCredBtn: section.querySelector('#closeCredModalBtn'),
      changePwdBtn: section.querySelector('#changeParentPwdBtn'),
      childrenSlots: section.querySelector('#childrenSlots'),
      addChildBtn: section.querySelector('#addAnotherChild'),
      pageInfo:  section.querySelector('#par-pageInfo'),
      prev:      section.querySelector('#par-prevBtn'),
      next:      section.querySelector('#par-nextBtn'),
      pageNums:  section.querySelector('#par-pageNums'),
    };

    let parCurrentPage = 1;
    const PAR_PAGE_SIZE = 10;

    // ---- Load class 8-10 students for child dropdowns ----
    let class8to10 = [];
    try {
      const r8  = await loadStudents({ class: '8',  limit: 200 });
      const r9  = await loadStudents({ class: '9',  limit: 200 });
      const r10 = await loadStudents({ class: '10', limit: 200 });
      class8to10 = [...(r8.data||[]), ...(r9.data||[]), ...(r10.data||[])];
    } catch (_) {}

    function studentsByClassSection(cls, sec) {
      return class8to10.filter(s =>
        String(s.class) === String(cls) && (!sec || s.section === sec)
      );
    }

    function buildChildSlot(idx) {
      const el = document.createElement('div');
      el.className = 'child-slot';
      el.dataset.idx = idx;
      el.style.cssText = 'background:#f8fafc;border-radius:8px;padding:12px;margin-bottom:10px;position:relative;';
      el.innerHTML = `
        <button type="button" class="btn btn-ghost btn-sm btn-icon" style="position:absolute;top:8px;right:8px;font-size:0.75rem;" data-remove="${idx}">✕</button>
        <div class="form-grid" style="grid-template-columns:1fr 1fr 1fr;gap:10px;">
          <div class="form-group" style="margin:0;">
            <label class="form-label" style="font-size:0.72rem;">Class</label>
            <select class="form-control" style="height:36px;" data-child-class="${idx}">
          ...
          <div class="form-group" style="margin:0;">
            <label class="form-label" style="font-size:0.72rem;">Section</label>
            <select class="form-control" style="height:36px;" data-child-section="${idx}">
              <option value="">Select</option>
              <option>A</option><option>B</option><option>C</option>
            </select>
          </div>
          <div class="form-group" style="margin:0;">
            <label class="form-label" style="font-size:0.72rem;">Student</label>
            <select class="form-control" style="height:36px;" data-child-student="${idx}">
              <option value="">Select student</option>
            </select>
          </div>
        </div>`;

      // Populate students when class or section changes
      function refreshStudents() {
        const cls = el.querySelector(`[data-child-class="${idx}"]`).value;
        const sec = el.querySelector(`[data-child-section="${idx}"]`).value;
        const stuSel = el.querySelector(`[data-child-student="${idx}"]`);
        const filtered = studentsByClassSection(cls, sec);
        stuSel.innerHTML = '<option value="">Select student</option>' +
          filtered.map(s => `<option value="${s.id}">${s.name} (${s.class}${s.section})</option>`).join('');
      }
      el.querySelector(`[data-child-class="${idx}"]`).addEventListener('change', refreshStudents);
      el.querySelector(`[data-child-section="${idx}"]`).addEventListener('change', refreshStudents);

      el.querySelector(`[data-remove="${idx}"]`).addEventListener('click', () => el.remove());
    ...
    function addChildSlot() {
      const idx = Date.now();
      elsP.childrenSlots.appendChild(buildChildSlot(idx));
    }

    elsP.addChildBtn.addEventListener('click', addChildSlot);

    // ---- Stats ----
    async function updateParentStats() {
      try {
        const c = await api.get('/parents/counts');
        if (elsP.active)   { elsP.active.dataset.rollTarget   = c.active;   }
        if (elsP.inactive) { elsP.inactive.dataset.rollTarget = c.inactive; }
        initRollingCounters(section);
      } catch (_) {}
    }
    ...
    async function loadParents() {
      try {
        parents = await api.get('/parents');
      } catch (_) { parents = []; }
    }

    function getSearched() {
      const q = (elsP.search?.value || '').toLowerCase();
      if (!q) return parents;
      return parents.filter(p =>
        p.father_name.toLowerCase().includes(q) ||
        p.parent_id.toLowerCase().includes(q) ||
        String(p.father_phone || '').includes(q)
      );
    }

    function renderParPagination() {
      const list = getSearched();
      const total = list.length;
      const totalPages = Math.ceil(total / PAR_PAGE_SIZE) || 1;
      const start = (parCurrentPage - 1) * PAR_PAGE_SIZE + 1;
      const end   = Math.min(parCurrentPage * PAR_PAGE_SIZE, total);
      if (elsP.pageInfo) elsP.pageInfo.textContent = total ? `Showing ${start}-${end} of ${total} parents` : 'No results';
      if (elsP.prev) elsP.prev.disabled = parCurrentPage <= 1;
      if (elsP.next) elsP.next.disabled = parCurrentPage >= totalPages;
      let nums = '';
      for (let p = 1; p <= totalPages; p++) {
        nums += `<button class="page-btn ${p === parCurrentPage ? 'active' : ''}" onclick="goParentPage(${p})">${p}</button>`;
      }
      if (elsP.pageNums) elsP.pageNums.innerHTML = nums;
    }
    window.goParentPage = (p) => { parCurrentPage = p; renderParentsTable(); renderParPagination(); };

    function renderParentsTable() {
      const list = getSearched();
      const totalPages = Math.ceil(list.length / PAR_PAGE_SIZE) || 1;
      if (parCurrentPage > totalPages) parCurrentPage = totalPages;
      const pageList = list.slice((parCurrentPage - 1) * PAR_PAGE_SIZE, parCurrentPage * PAR_PAGE_SIZE);
      const offset   = (parCurrentPage - 1) * PAR_PAGE_SIZE;
      if (!list.length) {
        elsP.table.innerHTML = `<tr><td colspan="8" class="text-center" style="padding:40px;color:#94a3b8;">No parents found.</td></tr>`;
        renderParPagination();
        return;
      }
      elsP.table.innerHTML = pageList.map((p, i) => {
        const childNames = (p.children || []).map(c => `${c.name} (${c.class}${c.section})`).join(', ') || '—';
        return `
          <tr>
            <td style="color:#94a3b8;">${offset + i + 1}</td>
            <td style="font-family:monospace;font-weight:700;color:#4f46e5;">${p.parent_id}</td>
            <td>
              <div style="display:flex;align-items:center;gap:10px;">
                <div class="avatar-sm" style="background:${avatarColor(p.father_name)};">${p.father_name.charAt(0)}</div>
                <div>
                  <div class="td-name">${p.father_name}</div>
                  ${p.mother_name ? `<div class="td-meta">${p.mother_name}</div>` : ''}
                </div>
              </div>
            </td>
            <td style="font-size:0.82rem;">${p.father_phone || '—'}</td>
            <td style="font-size:0.8rem;max-width:200px;">${childNames}</td>
            <td style="font-family:monospace;font-size:0.78rem;white-space:nowrap;">
              <div style="display:flex;align-items:center;gap:6px;">
                <span class="par-pwd-mask" data-pwd="${p.father_name.replace(/\s+/g,'').slice(0,4)}@${String(p.father_phone||'').replace(/\D/g,'').slice(-4)}" data-visible="0" style="font-family:monospace;letter-spacing:1px;">••••••</span>
                <button class="btn btn-ghost btn-sm btn-icon" style="padding:2px 4px;font-size:0.75rem;" title="Show/Hide Password" onclick="toggleParentPwd(this)">👁</button>
              </div>
            </td>
            <td><span class="badge ${statusBadge(p.status)}">${p.status}</span></td>
            <td>
              <div class="flex gap-2">
                <button class="btn btn-info btn-sm btn-icon" title="View Details" onclick="viewParent(${p.id})">👁</button>
                <button class="btn btn-primary btn-sm btn-icon" title="Edit" onclick="editParent(${p.id})">✏️</button>
                <button class="btn btn-danger btn-sm btn-icon" title="Delete" onclick="deleteParent(${p.id})">🗑️</button>
              </div>
            </td>
          </tr>`;
      }).join('');
    ...
    elsP.search?.addEventListener('input', () => { parCurrentPage = 1; renderParentsTable(); });
    elsP.prev?.addEventListener('click', () => { if (parCurrentPage > 1) window.goParentPage(parCurrentPage - 1); });
    elsP.next?.addEventListener('click', () => { const t = Math.ceil(getSearched().length / PAR_PAGE_SIZE) || 1; if (parCurrentPage < t) window.goParentPage(parCurrentPage + 1); });

    // ---- Toggle password show/hide in table (matches Students page style) ----
    window.toggleParentPwd = function(btn) {
    ...
    const vpModal   = section.querySelector('#viewParentModal');
    const vpBody    = section.querySelector('#vp-body');
    const vpTitle   = section.querySelector('#vp-title');
    const vpClose   = section.querySelector('#closeViewParentModal');
    const vpCloseBtn = section.querySelector('#closeViewParentModalBtn');
    vpClose?.addEventListener('click',    () => vpModal.classList.add('hidden'));
    vpCloseBtn?.addEventListener('click', () => vpModal.classList.add('hidden'));

    window.viewParent = function(id) {
      const p = parents.find(x => x.id === id);
      if (!p) return;
      const pwd = p.father_name.replace(/\s+/g,'').slice(0,4) + '@' + String(p.father_phone||'').replace(/\D/g,'').slice(-4);
      const childrenHtml = (p.children||[]).length
        ? (p.children).map(c => `<li style="padding:3px 0;">${c.name} &mdash; Class ${c.class}${c.section||''}</li>`).join('')
        : '<li style="color:#94a3b8;">No children linked</li>';
      vpTitle.textContent = `${p.parent_id} — ${p.father_name}`;
      vpBody.innerHTML = `
        <div style="text-align:center;padding:10px 0 16px;">
          <div style="width:56px;height:56px;border-radius:50%;background:${avatarColor(p.father_name)};
            display:flex;align-items:center;justify-content:center;font-size:1.5rem;font-weight:700;
            color:#fff;margin:0 auto 10px;">${p.father_name.charAt(0)}</div>
          <div style="font-size:1.05rem;font-weight:700;">${p.father_name}</div>
          <div style="font-size:0.82rem;color:#64748b;">${p.parent_id}</div>
        </div>
        <table class="view-details-table">
          <tr><td class="vdt-label">🪪 Parent ID</td><td class="vdt-value"><span style="font-family:monospace;font-weight:700;color:#4f46e5;">${p.parent_id}</span></td></tr>
          <tr><td class="vdt-label">👨 Father Name</td><td class="vdt-value">${p.father_name}</td></tr>
          <tr><td class="vdt-label">👩 Mother Name</td><td class="vdt-value">${p.mother_name || '—'}</td></tr>
          <tr><td class="vdt-label">📞 Father Phone</td><td class="vdt-value">${p.father_phone || '—'}</td></tr>
          <tr><td class="vdt-label">📞 Mother Phone</td><td class="vdt-value">${p.mother_phone || '—'}</td></tr>
          <tr><td class="vdt-label">💼 Occupation</td><td class="vdt-value">${p.occupation || '—'}</td></tr>
          <tr>
            <td class="vdt-label">🔑 Password</td>
            <td class="vdt-value">
              <div style="display:flex;align-items:center;gap:8px;">
                <span id="vp-pwd-text" style="font-family:monospace;letter-spacing:1px;" data-pwd="${pwd}" data-visible="0">••••••</span>
                <button class="btn btn-ghost btn-sm btn-icon" style="padding:2px 4px;font-size:0.75rem;" onclick="(function(btn){var s=btn.previousElementSibling;if(s.dataset.visible==='1'){s.textContent='••••••';s.dataset.visible='0';btn.textContent='👁';}else{s.textContent=s.dataset.pwd;s.dataset.visible='1';btn.textContent='👁';}})(this)">👁</button>
              </div>
            </td>
          </tr>
          <tr><td class="vdt-label">✅ Status</td><td class="vdt-value"><span class="badge ${statusBadge(p.status)}">${p.status}</span></td></tr>
          <tr>
            <td class="vdt-label">👶 Children (${(p.children||[]).length})</td>
            <td class="vdt-value">${(p.children||[]).length ? p.children.map(c => c.name + ' — Class ' + c.class + (c.section||'')).join('<br>') : '<span style="color:#94a3b8;">No children linked</span>'}</td>
          </tr>
        </table>`;
    ...
    function openParentModal() { elsP.modal.classList.remove('hidden'); }
    function closeParentModal() {
      elsP.modal.classList.add('hidden');
      editParentId = null;
      clearParentForm();
    }

    function clearParentForm() {
      ['par-father-name','par-father-phone','par-mother-name','par-mother-phone','par-occupation'].forEach(id => {
        const el = section.querySelector('#' + id);
        if (el) el.value = '';
      });
      section.querySelector('#par-status').value = 'Active';
      elsP.childrenSlots.innerHTML = '';
      addChildSlot(); // start with one slot
    }

    elsP.createBtn.addEventListener('click', () => {
      editParentId = null;
      elsP.modalTitle.textContent = 'Create Parent';
      clearParentForm();
      openParentModal();
    });
    elsP.closeModal.addEventListener('click', closeParentModal);
    elsP.cancelModal.addEventListener('click', closeParentModal);

    // ---- Save ----
    elsP.saveBtn.addEventListener('click', async () => {
      const fatherName  = section.querySelector('#par-father-name').value.trim();
      const fatherPhone = section.querySelector('#par-father-phone').value.trim();
      if (!fatherName) { alert('Father name is required.'); return; }

      // Collect selected children (non-empty student selects)
      const childIds = [];
      section.querySelectorAll('[data-child-student]').forEach(sel => {
        if (sel.value) childIds.push(parseInt(sel.value, 10));
      });

      const payload = {
        father_name:  fatherName,
        father_phone: fatherPhone,
        mother_name:  section.querySelector('#par-mother-name').value.trim(),
        mother_phone: section.querySelector('#par-mother-phone').value.trim(),
        occupation:   section.querySelector('#par-occupation').value.trim(),
        status:       section.querySelector('#par-status').value,
        children:     childIds,
      };

      try {
        if (editParentId) {
          await api.put('/parents/' + editParentId, payload);
          closeParentModal();
          await loadParents(); renderParentsTable(); updateParentStats();
        } else {
          ...
          if (res && res.parent_id) {
            pendingCredentials = { dbId: res.id, parent_id: res.parent_id, password: res.password };
            elsP.credId.textContent  = res.parent_id;
            elsP.credPwd.textContent = res.password;
            elsP.credModal.classList.remove('hidden');
          }
        }
      } catch (e) {
        alert('Error saving parent: ' + (e.error || e.message || 'Unknown error'));
      }
    });
    ...
    window.editParent = (id) => {
      ...
      if (!p) return;
      editParentId = id;
      elsP.modalTitle.textContent = 'Edit Parent';
      section.querySelector('#par-father-name').value  = p.father_name || '';
      section.querySelector('#par-father-phone').value = p.father_phone || '';
      section.querySelector('#par-mother-name').value  = p.mother_name || '';
      section.querySelector('#par-mother-phone').value = p.mother_phone || '';
      section.querySelector('#par-occupation').value   = p.occupation || '';
      section.querySelector('#par-status').value       = p.status || 'Active';
      // Rebuild child slots from existing children
      elsP.childrenSlots.innerHTML = '';
      if (p.children && p.children.length) {
        p.children.forEach(c => {
          const slot = buildChildSlot(Date.now() + Math.random());
          elsP.childrenSlots.appendChild(slot);
          const idx = slot.dataset.idx;
          // Pre-select class
          const clsSel = slot.querySelector(`[data-child-class="${idx}"]`);
          clsSel.value = String(c.class);
          clsSel.dispatchEvent(new Event('change'));
          // Pre-select section
          const secSel = slot.querySelector(`[data-child-section="${idx}"]`);
          setTimeout(() => {
            secSel.value = c.section;
            secSel.dispatchEvent(new Event('change'));
            setTimeout(() => {
              const stuSel = slot.querySelector(`[data-child-student="${idx}"]`);
              if (stuSel) stuSel.value = String(c.id);
            }, 50);
          }, 50);
        });
      } else {
    ...
    window.deleteParent = async (id) => {
      ...
      try {
        ...
        await loadParents(); renderParentsTable(); updateParentStats();
      } catch (e) { alert('Delete failed: ' + (e.error || 'Unknown error')); }
    };

    // ---- Credentials modal ----
    [elsP.closeCred, elsP.closeCredBtn].forEach(btn => {
      btn?.addEventListener('click', () => elsP.credModal.classList.add('hidden'));
    });

    elsP.changePwdBtn?.addEventListener('click', async () => {
      if (!pendingCredentials) return;
      const newPwd = prompt('Enter new password:');
      if (!newPwd || !newPwd.trim()) return;
      try {
        await api.put('/parents/' + pendingCredentials.dbId + '/password', { password: newPwd.trim() });
        pendingCredentials.password = newPwd.trim();
        elsP.credPwd.textContent = newPwd.trim();
        // Update local list
        const p = parents.find(p => p.id === pendingCredentials.dbId);
        if (p) p.password = newPwd.trim();
        alert('Password updated successfully.');
      } catch (e) { alert('Failed to update password.'); }
    });

    // ---- copyToClipboard helper (global) ----
    window.copyToClipboard = window.copyToClipboard || ((elId) => {
  ...
  function initTimetableSection() {
    ...
    async function loadTeacherForSubject(subjectId, standard, currentTeacher) {
      const teachers = await fetchTeachersForSubject(subjectId, standard);
      if (!teachers.length) {
        showAutoTeacher('');
        popupTeacherName.textContent = 'No teacher assigned';
        popupTeacherAuto.style.background = '#fef2f2';
        popupTeacherAuto.style.borderColor = '#fca5a5';
        popupTeacherName.style.color = '#991b1b';
        popupTeacher.innerHTML = '<option value="">No teachers available</option>';
        return;
      }
      // Reset styles
      popupTeacherAuto.style.background = '#f0fdf4';
      popupTeacherAuto.style.borderColor = '#86efac';
      popupTeacherName.style.color = '#166534';
      // Build the full dropdown for override mode
      popupTeacher.innerHTML = teachers.map(t =>
        `<option value="${t.name}" ${t.name === currentTeacher ? 'selected' : ''}>${t.name}</option>`
      ).join('');
      // Auto-fill: prefer currentTeacher if in list, else first teacher
      const matched = currentTeacher && teachers.find(t => t.name === currentTeacher);
      const autoName = matched ? matched.name : teachers[0].name;
      showAutoTeacher(autoName);

      // Check conflict for auto-filled teacher
      hideConflictWarning();
      if (autoName && editingCell) {
        const result = await checkTeacherConflict(autoName, editingCell.day, editingCell.num);
    ...
    popupSubject.addEventListener('change', async () => {
      ...
      const subjectId = selectedOpt ? selectedOpt.getAttribute('data-id') : null;
      if (subjectId) {
        await loadTeacherForSubject(parseInt(subjectId, 10), std, '');
      } else {
        showAutoTeacher('');
        popupTeacherName.textContent = 'Select subject first';
      }
    });
    ...
    async function openCellPopup(day, num, cellEl) {
      ...
      const subjects = await fetchSubjectsForStd(std);
      popupSubject.innerHTML = subjects.map(s =>
        `<option value="${s.name}" data-id="${s.id}" ${s.name === current.subject ? 'selected' : ''}>${s.name}</option>`
      ).join('');

      // Auto-fill teacher for the currently selected subject
      const selSubject = subjects.find(s => s.name === current.subject) || subjects[0];
      if (selSubject) {
        await loadTeacherForSubject(selSubject.id, std, current.teacher || '');
      } else {
        showAutoTeacher('');
    ...
    function buildUnifiedTable(schedule, tbody) {
      ...
      // Attach click events in edit mode
      if (editMode) {
        tbody.querySelectorAll('.tt-cell-editable').forEach(el => {
