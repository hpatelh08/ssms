...
const MOCK_ATTENDANCE_OVERRIDES_KEY = 'ssms_legacy_mock_attendance_overrides';
const MOCK_ATTENDANCE_VERSION_KEY = 'ssms_legacy_mock_attendance_version';
...
function getMockAttendanceOverrides() {
  try {
    ...
    console.warn('Unable to parse saved legacy mock attendance overrides', err);
  }
  localStorage.setItem(MOCK_ATTENDANCE_OVERRIDES_KEY, JSON.stringify({}));
  localStorage.setItem(MOCK_ATTENDANCE_VERSION_KEY, MOCK_ATTENDANCE_VERSION);
  return {};
...
function getApprovedTeacherLeaveLookup() {
  return (typeof LEAVE_REQUESTS !== 'undefined' ? LEAVE_REQUESTS : [])
    ...
    .reduce((lookup, leave) => {
      ...
      lookup[key].push({ start_date: leave.from, end_date: leave.to });
      return lookup;
    }, {});
}
...
function getMockAttendanceStatus(person, personType, dateStr) {
  ...
  if (overrides[overrideKey]) return attendanceStatusCode(overrides[overrideKey].status);

  if (personType === 'teacher') {
    const teacherLeaves = getApprovedTeacherLeaveLookup()[String(person.name || '').trim().toLowerCase()] || [];
    if (teacherLeaves.some((leave) => isDateBetween(dateStr, leave.start_date, leave.end_date))) return 'L';
  }
...
function buildMockAttendanceDayDetail(dateStr, personType = 'student', cls = '') {
  ...
  return { date: dateStr, person_type: personType, total, present, absent, leave, percent, records, non_working: nonWorking };
}
...
function buildMockMonthlyAttendanceReport(year, month, personType = 'student') {
  ...
  const persons = people.map((person) => {
    ...
    let leaveDays = 0;
    for (let day = 1; day <= daysInMonth; day += 1) {
      const status = getMockAttendanceStatus(person, personType, toIsoDate(year, month, day));
      if (!status) continue;
  ...
  return {
    ...
    sundayCount,
    holidayCount,
    overallPercent: persons.length ? Math.round(persons.reduce((sum, person) => sum + person.percent, 0) / persons.length) : 0,
...
async function apiFetch(endpoint, options = {}) {
  ...
  try {
    ...
    throw { status: 0, error: 'Network error — is the backend running?' };
  }
}
...
async function loadStudents(params = {}) {
  try {
    ...
    const arr = typeof STUDENTS !== 'undefined' ? JSON.parse(JSON.stringify(STUDENTS)) : [];
...
function getAttendanceExportUrl(year, month, personType) {
  ...
  return `${api.defaults ? api.defaults.baseURL : 'http://localhost:5001/api'}/attendance/export?year=${year}&month=${month}&person_type=${personType}&token=${token}`;
}
...
async function loadFees() {
  try {
    ...
    return shouldUseMockFees(data)
      ? normalizeFeeRecords(typeof FEES !== 'undefined' ? JSON.parse(JSON.stringify(FEES)) : [])
      : normalizeFeeRecords(data);
  } catch (e) {
...
async function apiCreateStudent(data) {
  try { return await api.post('/students', data); } catch (e) { console.warn('Create student API failed', e); return null; }
}
async function apiUpdateStudent(id, data) {
  try { return await api.put(`/students/${id}`, data); } catch (e) { console.warn('Update student API failed', e); return null; }
}
async function apiDeleteStudent(id) {
  try { return await api.delete(`/students/${id}`); } catch (e) { console.warn('Delete student API failed', e); return null; }
}
...
async function apiCreateFee(data) {
  ...
  try { return await api.post('/fees', normalizedData); } catch (e) { console.warn('Create fee API failed', e); return { error: e?.error || 'Unable to create fee record.' }; }
}
async function apiUpdateFee(id, data) {
  const normalizedData = normalizeFeeRecord(data);
  try { return await api.put(`/fees/${id}`, normalizedData); } catch (e) { console.warn('Update fee API failed', e); return { error: e?.error || 'Unable to update fee record.' }; }
}
async function apiFetchFeeReceipt(id) {
  try { return normalizeFeeRecord(await api.get(`/fees/${id}/receipt`)); } catch (e) { console.warn('Fee receipt API failed', e); return null; }
}
