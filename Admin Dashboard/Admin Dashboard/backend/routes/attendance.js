...
router.get('/', (req, res) => {
  const { person_type, class: cls, date, page = 1, limit = 50 } = req.query;
  let where = [], params = [];
...
router.get('/people', (req, res) => {
  const { person_type = 'student', class: cls } = req.query;

  if (person_type === 'student') {
    let q = `SELECT id, name, gr_number as roll, class, section, student_id FROM students WHERE status = 'Active'`;
    const p = [];
...
router.post('/bulk', authorize('super_admin', 'admin', 'teacher'), (req, res) => {
  const { records, date, person_type, class: cls } = req.body;
  ...
  const upsert = db.prepare(`
    INSERT INTO attendance (person_id, person_type, date, status, class, subject)
    VALUES (?, ?, ?, ?, ?, ?)
  ...
  db.transaction((items) => {
    for (const r of items)
      upsert.run(r.person_id, person_type, date, r.status, r.class, r.subject);
  })(normalized);

  res.json({ success: true, total: normalized.length, new: normalized.length - updatedCount, updated: updatedCount, date, person_type });
});
...
router.get('/day', (req, res) => {
  const { date, person_type = 'student' } = req.query;
  if (!date) return res.status(400).json({ error: 'date required (YYYY-MM-DD)' });

  let rows;
  if (person_type === 'student') {
    rows = db.prepare(`
      SELECT a.person_id, s.name, s.gr_number AS roll, s.class, a.status
      FROM attendance a
