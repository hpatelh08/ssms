...
router.post('/', authorize('super_admin', 'admin'), (req, res) => {
  const { name, subject, class: cls, salary, phone, email, status, qualification, join } = req.body;
  ...
  const stmt = db.prepare(`
    INSERT INTO teachers (name, emp, subject, class, salary, phone, email, status, qualification, join_date, teacher_id, teacher_password)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(name, empId, subject || '', cls || '', salary || 0, phone || '',
...
router.put('/:id', authorize('super_admin', 'admin'), (req, res) => {
  ...
  const { name, emp, subject, class: cls, salary, phone, email, status, qualification, join, teacher_id, teacher_password } = req.body;
  const stmt = db.prepare(`
    UPDATE teachers SET name=?, emp=?, subject=?, class=?, salary=?, phone=?,
      email=?, status=?, qualification=?, join_date=?, teacher_id=?, teacher_password=?
    WHERE id = ?
  `);
  stmt.run(name, emp, subject, cls, salary, phone, email, status, qualification, join, teacher_id || null, teacher_password || null, req.params.id);
