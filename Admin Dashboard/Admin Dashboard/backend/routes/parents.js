// =============================================
//  PARENTS ROUTES — Full CRUD (Class 8–10 only)
// =============================================
...
// ---- POST /api/parents/login — Public: Parent login (class 8-10) ----
...
/** Link child to parent (class 8–10 guard) */
function linkChild(parentDbId, studentId) {
  const stu = db.prepare('SELECT id, class FROM students WHERE id = ?').get(studentId);
  if (!stu) return false;
  const cls = parseInt(String(stu.class || '').match(/\d+/)?.[0] || '0', 10);
  if (cls < 8 || cls > 10) return false;
...
router.post('/', authorize('super_admin', 'admin'), (req, res) => {
  ...
  const result = db.transaction(() => {
    ...
    // First clear any stale parent_id pointing to this parentId
    db.prepare('UPDATE students SET parent_id = NULL WHERE parent_id = ? AND CAST(class AS INTEGER) < 8')
      .run(parentId);
    children.forEach(sid => {
      db.prepare('UPDATE students SET parent_id = ?, parent = ? WHERE id = ? AND CAST(class AS INTEGER) >= 8')
  ...
  if (Array.isArray(children)) {
    ...
    if (father_name) {
      db.prepare(`
        ...
        WHERE parent_id = ? AND CAST(class AS INTEGER) >= 8
      `).run(father_name, existing.parent_id);
    }
  }
