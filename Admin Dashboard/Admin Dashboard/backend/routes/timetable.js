...
function parseStdSection(stdRaw, sectionRaw) {
  ...
  if (!Number.isInteger(std) || std < 1 || std > 8) return { error: 'Class/standard must be between 1 and 8.' };
...
router.get('/', (req, res) => {
  const parsed = parseStdSection(req.query.std || 1, req.query.section || 'A');
...
router.get('/subjects', (req, res) => {
  ...
  if (!Number.isInteger(standard) || standard < 1 || standard > 8) {
