...
const { MIN_STANDARD, MAX_STANDARD, PRIMARY_STANDARD_MAX, isSupportedStandard } = require('../config/standards');
...
function subjectListForClass(cls) {
  return parseInt(cls, 10) <= PRIMARY_STANDARD_MAX ? PRIMARY_SUBJ : UPPER_SUBJ;
}
...
router.get('/', (req, res) => {
  ...
  if (cls) {
    ...
    if (!isSupportedStandard(classNo)) return res.json({ data: [] });
    where.push('CAST(m.class AS INTEGER) = ?');
    params.push(classNo);
  } else {
    where.push(`CAST(m.class AS INTEGER) BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}`);
  }
