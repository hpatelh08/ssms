...
function isFeeApplicable(cls) {
  ...
  return isSupportedStandard(classNo);
}
...
router.get('/', (req, res) => {
  ...
  let where = [`${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}`];
  let params = [];
...
router.get('/stats', (req, res) => {
  ...
  const totalPaid = db.prepare(`SELECT COALESCE(SUM(${NORMALIZED_PAID_SQL}),0) as val FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} = 'Paid'`).get().val;
  const totalDue = db.prepare(`SELECT COALESCE(SUM(${NORMALIZED_DUE_SQL}),0) as val FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} != 'Paid'`).get().val;
  const partialAmt = db.prepare(`SELECT COALESCE(SUM(${NORMALIZED_PAID_SQL}),0) as val FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} = 'Partial'`).get().val;
  const totalAmount = db.prepare(`SELECT COALESCE(SUM(${NORMALIZED_AMOUNT_SQL}),0) as val FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}`).get().val;
  const rate = totalAmount > 0 ? Math.round((totalPaid / totalAmount) * 100) : 0;

  const paidCount = db.prepare(`SELECT COUNT(*) as c FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} = 'Paid'`).get().c;
  const pendingCount = db.prepare(`SELECT COUNT(*) as c FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} = 'Pending'`).get().c;
  const partialCount = db.prepare(`SELECT COUNT(*) as c FROM fees WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD} AND ${NORMALIZED_STATUS_SQL} = 'Partial'`).get().c;
...
router.post('/', authorize('super_admin', 'admin', 'accountant'), (req, res) => {
  ...
  if (!isFeeApplicable(cls)) return res.status(400).json({ error: 'Fees are only available for classes 1 to 6.' });
...
router.post('/sync-students', authorize('super_admin', 'admin'), (req, res) => {
  const students9plus = db.prepare(
    'SELECT id, name, class, section FROM students WHERE CAST(class AS INTEGER) BETWEEN ? AND ?'
