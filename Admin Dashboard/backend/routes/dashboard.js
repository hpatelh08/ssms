...
function shiftLocalDateYmd(offsetDays, baseDate = new Date()) {
...
function buildAttendanceSnapshot(dateStr, focusType = null) {
  ...
  const hasData = focusType === 'teacher'
    ? tMarked > 0
    : focusType === 'student'
      ? sMarked > 0
      : (sMarked > 0 || tMarked > 0);
  const lastUpdated = focusType === 'teacher'
    ? (teacherRow.last_marked || null)
    : focusType === 'student'
      ? (studentRow.last_marked || null)
      : (studentRow.last_marked || teacherRow.last_marked || null);
...
router.get('/stats', (req, res) => {
  ...
  const yesterday       = shiftLocalDateYmd(-1);
  ...
  // Fees KPI — keep in sync with Fee Management (fees table records)
  const pendingFees = db.prepare(`
    SELECT COALESCE(SUM(${NORMALIZED_FEE_DUE_SQL}), 0) AS val
    FROM fees
    WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
  `).get().val;
  const totalRevenue = db.prepare(`
    SELECT COALESCE(SUM(${NORMALIZED_FEE_PAID_SQL}), 0) AS val
    FROM fees
    WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
  `).get().val;
  const pendingStudentsCount = db.prepare(`
    SELECT COUNT(*) AS c
    FROM fees
    WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
      AND ${NORMALIZED_FEE_STATUS_SQL} = 'Pending'
  `).get().c;
...
// GET /api/dashboard/fee-summary — counts from students + fee structure + payments
router.get('/fee-summary', (req, res) => {
  const row = db.prepare(`
    SELECT
      COUNT(CASE WHEN paid_sum >= expected_total THEN 1 END)                            AS paid,
      COUNT(CASE WHEN paid_sum > 0 AND paid_sum < expected_total THEN 1 END)            AS partial,
      COUNT(CASE WHEN paid_sum = 0 THEN 1 END)                                           AS pending,
      COUNT(*)                                                                            AS total
    FROM (
      SELECT
        s.id,
        fs.total AS expected_total,
        COALESCE(sp.paid_sum, 0) AS paid_sum
      FROM students s
      JOIN fee_structure fs ON CAST(s.class AS INTEGER) = fs.class
      LEFT JOIN (
        SELECT
          LOWER(TRIM(student)) AS sname,
          ${FEE_CLASS_SQL} AS class_no,
          SUM(CASE WHEN COALESCE(paid,0) < 0 THEN 0 ELSE COALESCE(paid,0) END) AS paid_sum
        FROM fees
        WHERE ${FEE_CLASS_SQL} BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
        GROUP BY LOWER(TRIM(student)), ${FEE_CLASS_SQL}
      ) sp ON LOWER(TRIM(s.name)) = sp.sname AND CAST(s.class AS INTEGER) = sp.class_no
      WHERE CAST(s.class AS INTEGER) BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
        AND fs.total > 0
    ) fee_student_view
  `).get();
  res.json(row);
...
router.get('/class-fee-summary', (req, res) => {
  const rows = db.prepare(`
    WITH RECURSIVE class_range(class_no) AS (
      SELECT ${MIN_STANDARD}
      UNION ALL
      SELECT class_no + 1 FROM class_range WHERE class_no < ${MAX_STANDARD}
    ),
    students_by_class AS (
      SELECT
        CAST(s.class AS INTEGER) AS class_no,
        COUNT(DISTINCT s.id) AS total_students,
        SUM(CASE WHEN COALESCE(sp.paid_sum, 0) >= fs.total AND fs.total > 0 THEN 1 ELSE 0 END) AS paid_count,
        COALESCE(SUM(
          CASE
            WHEN COALESCE(sp.paid_sum, 0) > fs.total THEN fs.total
            ELSE COALESCE(sp.paid_sum, 0)
          END
        ), 0) AS collected_amount,
        COALESCE(SUM(fs.total), 0) AS total_amount
      FROM students s
      JOIN fee_structure fs ON CAST(s.class AS INTEGER) = fs.class
      LEFT JOIN (
        SELECT
    ...
    SELECT
      class_range.class_no AS class,
      COALESCE(students_by_class.total_students, 0) AS total_students,
      COALESCE(students_by_class.paid_count, 0) AS paid_count,
      CASE
        WHEN COALESCE(students_by_class.total_students, 0) > 0
          THEN ROUND((COALESCE(students_by_class.paid_count, 0) * 100.0) / students_by_class.total_students)
        ELSE 0
      END AS percentage,
      ROUND(COALESCE(students_by_class.collected_amount, 0)) AS collected_amount,
      ROUND(COALESCE(students_by_class.total_amount, 0)) AS total_amount
    FROM class_range
    LEFT JOIN students_by_class ON students_by_class.class_no = class_range.class_no
    ORDER BY class_range.class_no
  `).all();
  res.json(rows);
});
...
// GET /api/dashboard/class-marks-progress — Std 1–6 class-wise performance
  ...
  if (!classes.length) {
    try {
      classes = db.prepare(`
        ...
        FROM results
        WHERE class IS NOT NULL AND CAST(class AS INTEGER) BETWEEN ${MIN_STANDARD} AND ${MAX_STANDARD}
...
router.get('/today-attendance', (req, res) => {
  const personType = ['student', 'teacher'].includes(req.query.person_type) ? req.query.person_type : null;
  const requestedDate = formatLocalDateYmd();
  let snapshot = buildAttendanceSnapshot(requestedDate, personType);
  let effectiveDate = requestedDate;
  let isFallback = false;

  if (!snapshot.hasData) {
    const latest = personType
      ? db.prepare('SELECT date FROM attendance WHERE person_type = ? ORDER BY date DESC LIMIT 1').get(personType)
      : db.prepare('SELECT date FROM attendance ORDER BY date DESC LIMIT 1').get();
    if (latest && latest.date && latest.date !== requestedDate) {
      snapshot = buildAttendanceSnapshot(latest.date, personType);
