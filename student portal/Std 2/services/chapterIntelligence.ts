...
function buildOfflineGroqFallback(userPrompt: string): string {
  ...
  if (prompt.includes('create an engaging lesson')) {
    return JSON.stringify({ sections: [{ type: 'introduction', title: 'Chapter', content: 'This chapter has an important idea to learn.', emoji: '📘' }], funFacts: [] });
  }
...
export async function getPracticeQuestions(
  ...
  const systemPrompt = `You are a Class 2 teacher creating practice questions.
ONLY base questions on the textbook context provided.
Return a JSON object with:
- "questions": Array of 6-8 objects with:
  - "question": A simple question for a 7-8 year old
...
export async function getQuizQuestions(
  ...
  const systemPrompt = `You are a Class 2 teacher creating a multiple-choice quiz.
ONLY base questions on the textbook context provided.
Return a JSON object with:
