...
export const ParentDashboard: React.FC<ParentDashboardProps> = ({ knowledgeBase, stats, onAddKnowledge }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-24">
      {/* Header */}
      <motion.div
        className="relative overflow-hidden rounded-[24px] bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 p-8 lg:p-10 text-white shadow-2xl"
      ...
      <GlassCard variant="strong" hover={false} className="p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <SkillCard label="Participation" level={stats.skills.participation} icon={getSmileIcon(stats.skills.participation)} gradient="from-purple-100/60 to-pink-50/40" />
        </div>
              ...
              auditLog.map((entry) => (
                <div key={entry.id} className="flex items-start gap-3 p-3 bg-white/30 rounded-xl text-xs">
                  <span className={`px-2 py-0.5 rounded-lg font-bold uppercase ${
                    ...
                    entry.category === 'game' ? 'bg-purple-100/60 text-purple-600' :
