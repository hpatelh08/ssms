...
const WeeklyBarChart: React.FC<{ data: { day: string; minutes: number }[]; color?: string; height?: number }> = ({
  ...
  const getBarPath = (x: number, y: number, width: number, heightPx: number) => {
    if (heightPx <= 0) return '';
    const radius = Math.min(width / 2, Math.max(0, Math.min(14, heightPx)));
    const bottomY = y + heightPx;
    return [
      `M ${x} ${bottomY}`,
      `L ${x} ${y + radius}`,
      `Q ${x} ${y} ${x + radius} ${y}`,
  ...
  const w = 420;
  const pad = { top: 14, right: 14, bottom: 52, left: 42 };
  const chartH = height - pad.top - pad.bottom;
  const chartW = w - pad.left - pad.right;
  const slotW = chartW / data.length;
  const baseY = pad.top + chartH;
  const maxVal = Math.max(...data.map(d => d.minutes), 1);
  const barW = Math.min(36, slotW - 16);
  ...
  return (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <svg
        ...
        style={{ display: 'block', maxWidth: 820 }}
      >
      ...
      {data.map((d, i) => {
        const rawH = (d.minutes / maxVal) * chartH;
        const barH = d.minutes <= 0 ? 0 : Math.max(8, rawH);
        const cx = pad.left + slotW * i + slotW / 2;
        const x = cx - barW / 2;
        const y = baseY - barH;
        const path = getBarPath(x, y, barW, barH);
        return (
          <g key={d.day}>
            <defs>
            ...
            <motion.path
              d={path}
              fill={`url(#wbar-${i})`}
              initial={{ d: getBarPath(x, baseY, barW, 0) }}
              animate={{ d: path }}
            ...
            <motion.text x={cx} y={Math.max(y - 6, pad.top + 10)} textAnchor="middle" fontSize={9} fontWeight={700} fill="#7C6CFF"
      ...
      <line x1={pad.left} y1={baseY} x2={w - pad.right} y2={baseY} stroke={`${CLR.muted}18`} strokeWidth={1} />
      </svg>
    </div>
  );
};
...
export const AttendancePage: React.FC = () => {
  ...
  const handleDownload = useCallback(() => {
    ...
    try {
      ...
      const addLineItem = (label: string, value: string, color: [number, number, number] = [107, 111, 207]) => {
        if (y > ph - 16) {
          doc.addPage();
          y = 18;
        }
      ...
      doc.roundedRect(10, 10, pw - 20, 34, 8, 8, 'F');
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(20);
      doc.setTextColor(59, 63, 175);
      doc.text('Monthly Report Card', margin, 22);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.setTextColor(107, 111, 207);
      doc.text(`Student: Yash`, margin, 29);
      doc.text(`Month: ${monthLabel}`, margin, 34);
      doc.text(`Generated: ${new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}`, pw - margin, 34, { align: 'right' });
      y = 54;

      addSectionTitle('Attendance Overview');
      const metricBoxW = (contentW - 9) / 2;
      const metrics = [
        ['Attendance Rate', `${calendarStats.rate}%`],
        ['Present Days', `${calendarStats.present}`],
        ['Absent Days', `${calendarStats.absent}`],
        ['Holiday Days', `${calendarStats.holidays}`],
        ['Weekly Learning', `${weeklyStats.totalMin} min`],
        ['Average Session', `${analytics.avgSessionMinutes} min`],
      ...
      metrics.forEach(([label, value], index) => {
        ...
        const boxX = margin + col * (metricBoxW + 9);
        const boxY = y + row * 18;
        doc.setFillColor(250, 252, 255);
        doc.setDrawColor(222, 231, 255);
        doc.roundedRect(boxX, boxY, metricBoxW, 14, 4, 4, 'FD');
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8);
        doc.setTextColor(143, 148, 212);
        doc.text(label, boxX + 4, boxY + 5);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        doc.setTextColor(59, 63, 175);
        doc.text(value, boxX + 4, boxY + 10.5);
      });
      y += Math.ceil(metrics.length / 2) * 18 + 4;

      addSectionTitle('Weekly Learning Minutes');
      weeklyData.forEach((item) => addLineItem(item.day, `${item.minutes} min`));

      addSectionTitle('Activity Breakdown');
      activityData.forEach((item) => addLineItem(item.label, `${item.value} min`, [59, 63, 175]));

      addSectionTitle('Monthly Progress Summary');
      addLineItem('Total Learning Time', `${totalLearningHrs} hours`);
      addLineItem('Completed Chapters', `${MONTHLY_SUMMARY.completedChapters}`);
      addLineItem('Activities Completed', `${MONTHLY_SUMMARY.activitiesCompleted}`);
      addLineItem('Best Learning Day', weeklyStats.bestDay);
      addLineItem('Focus Score', `${weeklyStats.focusScore}%`);

      addSectionTitle('Parent Alerts & Recommendations');
      alerts.forEach((alert) => {
        const lines = doc.splitTextToSize(`${alert.title}: ${alert.desc}`, contentW - 4);
        if (y + lines.length * 5 > ph - 18) {
          doc.addPage();
          y = 18;
        }
        doc.setFont('helvetica', 'normal');
      ...
      doc.text('Generated by SSMS Attendance Intelligence', pw / 2, ph - 10, { align: 'center' });

      doc.save(`monthly_report_${MONTH_NAMES[viewMonth].toLowerCase()}_${viewYear}.pdf`);
    ...
    weeklyData,
    weeklyStats.bestDay,
    weeklyStats.focusScore,
    weeklyStats.totalMin,
  ]);
