...
const WEEKLY_ACTIVITY_KEY = 'weeklyActivity';
const WEEKLY_DAY_LABELS = ['M', 'T', 'W', 'T', 'F', 'S', 'S'] as const;

type WeeklyActivityDay = {
  day: (typeof WEEKLY_DAY_LABELS)[number];
  sessions: number;
  timeSpent: number;
};
...
function generateWeeklyData(): WeeklyActivityDay[] {
  return WEEKLY_DAY_LABELS.map((day) => ({
    day,
    sessions: randomInt(1, 4),
    timeSpent: randomInt(12, 40),
  }));
}

function isValidWeeklyData(data: unknown): data is WeeklyActivityDay[] {
  if (!Array.isArray(data) || data.length !== 7) return false;
  return data.every((item) => {
    if (!item || typeof item !== 'object') return false;
    const rec = item as Record<string, unknown>;
    const sessions = rec.sessions;
    const timeSpent = rec.timeSpent;
    return (
...
function saveWeeklyData(data: WeeklyActivityDay[]): void {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(WEEKLY_ACTIVITY_KEY, JSON.stringify(data));
  } catch {
    // Ignore storage errors to keep UI stable.
  }
}

function getWeeklyData(): WeeklyActivityDay[] {
  const fallback = generateWeeklyData();
  if (typeof window === 'undefined') return fallback;

  try {
    const raw = window.localStorage.getItem(WEEKLY_ACTIVITY_KEY);
    if (!raw) {
      saveWeeklyData(fallback);
      return fallback;
    }
    const parsed: unknown = JSON.parse(raw);
    if (!isValidWeeklyData(parsed)) {
      saveWeeklyData(fallback);
      return fallback;
    }
    return parsed;
...
function calculateStats(data: WeeklyActivityDay[]): {
  ...
  const totalTimeLabel = totalTime < 60
    ? `${totalTime} min`
    : `${Math.floor(totalTime / 60)}h ${String(totalTime % 60).padStart(2, '0')}m`;

  const avgSecondsExact = totalSessions > 0 ? (totalTime * 60) / totalSessions : 0;
  const avgSecondsRounded = Math.round(avgSecondsExact);
  const avgMins = Math.floor(avgSecondsRounded / 60);
  const avgSecs = avgSecondsRounded % 60;
  const avgSessionLabel = avgSecs === 0 ? `${avgMins} min` : `${avgMins}m ${avgSecs}s`;

  return { totalSessions, totalTime, totalTimeLabel, avgSessionLabel };
}

function updateCurrentDaySession(data: WeeklyActivityDay[], duration: number): WeeklyActivityDay[] {
  const safeDuration = Math.max(0, Math.round(duration));
  const todayIdx = (new Date().getDay() + 6) % 7;
  const updated = data.map((d, i) => (i === todayIdx
    ? {
      ...d,
      sessions: d.sessions + 1,
      timeSpent: Math.min(180, d.timeSpent + safeDuration),
    }
    : d));

  saveWeeklyData(updated);
  return updated;
}
...
const MiniWeekBars: React.FC<{ data: WeeklyActivityDay[]; color?: string }> = ({ data, color = CLR.indigo }) => {
  const max = Math.max(...data.map((d) => d.timeSpent), 1);
  const todayIdx = (new Date().getDay() + 6) % 7;
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 5, height: 52, marginTop: 8 }}>
      {data.map((d, i) => {
        const h = Math.max(4, (d.timeSpent / max) * 44);
        const isToday = i === todayIdx;
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <motion.div
              title={`${d.sessions} sessions • ${d.timeSpent} min`}
              style={{
                width: '100%', maxWidth: 18, borderRadius: 6,
                background: d.timeSpent > 0 ? `linear-gradient(180deg, ${color}, ${color}88)` : `${color}15`,
                boxShadow: d.timeSpent > 0 ? `0 2px 6px ${color}25` : 'none',
                outline: isToday ? `1px solid ${color}55` : '1px solid transparent',
              }}
              initial={{ height: 0 }}
              animate={{ height: h }}
              transition={{ duration: 0.5, ease: 'easeOut', delay: 0.3 + i * 0.04 }}
            />
            <span style={{ fontSize: 8, fontWeight: 700, color: CLR.soft }}>{d.day}</span>
          </div>
        );
      })}
    </div>
...
export const OverviewPage: React.FC = () => {
  ...
  const weeklyActivity = useMemo(
    () => dayLabels.map((day, index) => {
      const timeSpent = analytics.weeklyMinutes[index] ?? 0;
      const estimatedSessions = timeSpent <= 0
        ? 0
        : Math.max(1, Math.round(timeSpent / Math.max(analytics.avgSessionMinutes, 1)));
      return { day: WEEKLY_DAY_LABELS[index], sessions: estimatedSessions, timeSpent };
    }),
    [analytics.avgSessionMinutes, analytics.weeklyMinutes],
  );
  const weeklyActivityStats = useMemo(() => calculateStats(weeklyActivity), [weeklyActivity]);
  ...
  const avgSession = Math.round(weeklyTotal / 7);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, position: 'relative', zIndex: 1 }}>
          ...
          <GlassCard gradient={CARD_GRADIENTS.growth} delay={0.06}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
              ...
              <div style={{ background: 'rgba(16,185,129,0.08)', borderRadius: 12, padding: '10px 12px' }}>
                <p style={{ fontSize: 18, fontWeight: 800, color: CLR.mint, margin: 0 }}>{weeklyActivityStats.totalTimeLabel}</p>
            ...
            <div style={{ background: 'rgba(16,185,129,0.06)', borderRadius: 12, padding: '8px 12px', marginBottom: 6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 800, color: CLR.mint }}>{weeklyActivityStats.avgSessionLabel}</span>
              </div>
            </div>
            <MiniWeekBars data={weeklyActivity} color={CLR.mint} />
          </GlassCard>
