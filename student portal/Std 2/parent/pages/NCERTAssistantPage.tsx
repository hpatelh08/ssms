...
const QUICK_ACTIONS = [
  ...
  { label: 'Give worksheet', icon: '📝', prompt: 'Generate a 5-question worksheet from this chapter suitable for a Class 2 student.' },
...
const SubjectToggle: React.FC<{
  ...
  >
    ...
    >
      {(['English', 'Maths', 'Hindi', 'Gujarati'] as Subject[]).map(s => {
        const isActive = selected === s;
        const theme = SUBJECT_THEME[s];
        const icon = s === 'English' ? '📖' : s === 'Maths' ? '🔢' : s === 'Hindi' ? '🌿' : '📙';
        return (
          <motion.button
            key={s}
            onClick={() => onChange(s)}
...
const AskAiSection: React.FC<{
  ...
  const accentColor = subject === 'English' ? '#ea580c' : '#7c3aed';
    ...
    >
          ...
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-10">
              ...
              <p className="text-xs text-gray-400 mt-1 max-w-[300px]">
                Use quick buttons below or type your question. AI stays grounded to NCERT Class 2 {subject} content.
              </p>
            </div>
          )}
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        ...
        <span className="text-[10px] text-gray-400 font-medium">
          AI responses are grounded to NCERT Class 2 {subject} textbook. No external data.
        </span>
      </div>
    </motion.div>
  );
...
export const NCERTAssistantPage: React.FC<{
  ...
  const handleWatchSyllabus = useCallback((item: SyllabusItem) => {
    ...
    if (onPlayVideo) {
      const allVideos = [...VIDEO_DATA.English, ...VIDEO_DATA.Maths, ...VIDEO_DATA.Hindi, ...VIDEO_DATA.Gujarati];
  ...
  const handleAskAIFromUnit = useCallback((title: string) => {
    ...
    prefillRef.current = `Explain "${title}" for Class 2 students.`;
          ...
          {subjectRhymes.length > 0 ? (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-bold text-gray-600">{theme.rhymeLabel}</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <div className="text-center py-6">
              <p className="text-sm text-gray-400 font-medium">More rhymes coming soon! 🎵</p>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
