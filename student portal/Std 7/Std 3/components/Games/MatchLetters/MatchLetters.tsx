  ...
  return (
    <>
      ...
      <div className="bg-white/75 backdrop-blur-2xl border border-white/40 p-8 sm:p-10 rounded-[24px] text-center max-w-md mx-auto relative overflow-hidden shadow-xl">
        ...
        {/* Lowercase row */}
        <div>
          <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-2">Lowercase</p>
          <div className="flex justify-center gap-3">
                ...
                <motion.button
                  ...
                  className={`w-14 h-14 rounded-2xl border-2 font-black text-2xl transition-all ${
                    isMatched
                      ...
                      : 'border-gray-200/60 bg-white/60 text-purple-700 hover:border-purple-200/60'
                  }`}
                  animate={isWrong ? { x: [0, -4, 4, -2, 2, 0] } : {}}
                  transition={{ duration: 0.3 }}
        ...
        <AnimatePresence>
          {selectedUpper && !state.feedback && (
            <motion.p
              ...
              exit={{ opacity: 0 }}
            >
              Now tap the lowercase <span className="text-purple-500">"{selectedUpper.toLowerCase()}"</span>
            </motion.p>
          )}
        </AnimatePresence>
