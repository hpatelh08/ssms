 ...
 */
const mcq = (id, subject, chapter, difficulty, question, options, answerIndex, realLifeHint) => ({ id, subject, chapter, difficulty, question, options, answerIndex, realLifeHint });
const mistake = (id, subject, chapter, difficulty, incorrectStatement, mistakeText, correction, explanation) => ({
    id,
    subject,
    chapter,
    difficulty,
...
const pair = (id, subject, chapter, difficulty, left, right, correctMap) => ({ id, subject, chapter, difficulty, left, right, correctMap });
export const STD7_QUIZ_QUESTIONS = [
    // Maths
...
export const STD7_FIND_MISTAKE_QUESTIONS = [
    // Maths
...
export const STD7_MATCH_PAIR_QUESTIONS = [
    // Maths
...
export function pickStd7QuizQuestion(difficulty) {
    return randomItem(byDifficulty(STD7_QUIZ_QUESTIONS, difficulty));
}
export function pickStd7FindMistakeQuestion(difficulty) {
    return randomItem(byDifficulty(STD7_FIND_MISTAKE_QUESTIONS, difficulty));
}
export function pickStd7MatchPairQuestion(difficulty) {
    return randomItem(byDifficulty(STD7_MATCH_PAIR_QUESTIONS, difficulty));
}
export function buildFindMistakeOptions(item) {
    ...
    return shuffle([item.correction, ...distractors]);
}
const MAP_PERMS = [
    [0, 1, 2],
    [0, 2, 1],
...
function mapCode(map) {
    return map
        .map((rightIndex, leftIndex) => `${leftIndex + 1}-${String.fromCharCode(65 + rightIndex)}`)
        .join(', ');
}
export function buildMatchPairOptions(item) {
    const correct = mapCode(item.correctMap);
    const distractors = randomSample(MAP_PERMS.filter(perm => mapCode(perm) !== correct).map(mapCode), 3);
    return { options: shuffle([correct, ...distractors]), correctAnswer: correct };
}
function answerLetter(answerIndex) {
    return ['A', 'B', 'C', 'D'][answerIndex];
}
export function renderStd7QuestionBankMarkdown() {
