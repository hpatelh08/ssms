...
const DIFFICULTY_UNLOCK_KEY = 'ssms_difficulty_unlock_v1';
const LEVEL_COUNTS: Record<Difficulty, number> = { easy: 40, intermediate: 30, difficult: 30 };
...
type DifficultyUnlockStore = Record<string, DifficultyUnlockStatus>;
...
function normalizeUnlockStatus(raw: Partial<DifficultyUnlockStatus> | undefined): DifficultyUnlockStatus {
  return {
    easy: true,
    intermediate: Boolean(raw?.intermediate),
    difficult: Boolean(raw?.difficult),
  };
}

function loadDifficultyUnlockStore(): DifficultyUnlockStore {
  try {
    const raw = localStorage.getItem(DIFFICULTY_UNLOCK_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw) as Record<string, Partial<DifficultyUnlockStatus>>;
    const normalized: DifficultyUnlockStore = {};
    for (const key of Object.keys(parsed)) {
      normalized[key] = normalizeUnlockStatus(parsed[key]);
    }
    return normalized;
  } catch {
    return {};
...
function saveDifficultyUnlockStore(store: DifficultyUnlockStore): void {
  try {
    localStorage.setItem(DIFFICULTY_UNLOCK_KEY, JSON.stringify(store));
  } catch {
    // ignore storage quota / private mode errors
  }
}

function getDifficultyScorePercent(difficulty: Difficulty, progress: DifficultyProgress): number {
  const earnedScore = Object.values(progress.miniLevels).reduce((sum, level) => sum + level.score, 0);
  const possibleScore = LEVEL_COUNTS[difficulty] * QUESTIONS_PER_LEVEL;
  if (possibleScore <= 0) return 0;
  return earnedScore / possibleScore;
}

function qualifiesForUnlock(difficulty: Difficulty, progress: DifficultyProgress): boolean {
  return progress.completed || getDifficultyScorePercent(difficulty, progress) >= 0.8;
}

function syncDifficultyUnlockStatus(gameKey: string, gp: GameProgress): DifficultyUnlockStatus {
  const unlockStore = loadDifficultyUnlockStore();
  // Always derive from current mastery so stale unlock flags are corrected automatically.
  const intermediateUnlocked = qualifiesForUnlock('easy', gp.easy);
  const difficultUnlocked = intermediateUnlocked && qualifiesForUnlock('intermediate', gp.intermediate);

  const next: DifficultyUnlockStatus = {
    easy: true,
    intermediate: intermediateUnlocked,
    difficult: difficultUnlocked,
  };

  if (
    !unlockStore[gameKey]
    || unlockStore[gameKey].intermediate !== next.intermediate
    || unlockStore[gameKey].difficult !== next.difficult
  ) {
    unlockStore[gameKey] = next;
    saveDifficultyUnlockStore(unlockStore);
  }
...
export function getDifficultyUnlockStatus(
  ...
  const key = getGameKey(subject, chapter, gameType);
  const gp = progress || getGameProgress(subject, chapter, gameType);
  return syncDifficultyUnlockStatus(key, gp);
}
...
export function saveMiniLevelResult(
  ...
  const expected = LEVEL_COUNTS[difficulty] || QUESTIONS_PER_LEVEL;
  ...
  syncDifficultyUnlockStatus(key, gp);
  saveMastery(store);
