...
const SECTIONS: {
  ...
  },
  {
    ...
    icon: '🔬',
    gradient: 'linear-gradient(135deg, #B8F5CD 0%, #86EFAC 40%, #A0E8BA 100%)',
    glowColor: 'rgba(134,239,172,0.40)',
    decorations: ['🌿', '🔬', '🧪', '🌍'],
    mascot: '🦊',
  },
  {
          ...
          {revealed && (
            <>
              ...
              <motion.div
                ...
                style={S.cardsRow}
              >
                {SECTIONS.filter((sec) => sec.id !== 'science').map((sec, i) => (
                  <SectionCard
                    key={sec.id}
                    title={sec.title}
...
const S: Record<string, React.CSSProperties> = {
  ...
  roundedContainer: {
    ...
    boxShadow: '0 20px 60px rgba(91,58,41,0.15), 0 8px 24px rgba(91,58,41,0.08)',
  },
  wrapper: {
    display: 'flex',
    flexDirection: 'column',
