...
import {
  pickStd7QuizQuestion,
  pickStd7FindMistakeQuestion,
  pickStd7MatchPairQuestion,
  buildFindMistakeOptions,
  buildMatchPairOptions,
} from './std7QuestionBank';
...
function optCount(d: Difficulty): number { return d === 'easy' ? 4 : d === 'intermediate' ? 5 : 6; }

/**
 * Std 7 uplift:
 * - Easy behaves like Intermediate
 * - Intermediate behaves like Difficult
 * - Difficult remains Difficult (advanced IQ level)
 */
function upliftStd7Difficulty(d: Difficulty): Difficulty {
  if (d === 'easy') return 'intermediate';
  if (d === 'intermediate') return 'difficult';
  return 'difficult';
}
...
function genNumberTap(d: Difficulty): Question {
  const max = d === 'easy' ? 120 : d === 'intermediate' ? 350 : 900;
  const min = d === 'easy' ? 10 : d === 'intermediate' ? 50 : 100;
  const n = optCount(d);
...
function genMathPuzzle(d: Difficulty): Question {
  const maxSum = d === 'easy' ? 120 : d === 'intermediate' ? 500 : 1200;
  const n = optCount(d);
  const a = randInt(10, maxSum);
...
function genCountObjects(d: Difficulty): Question {
  const n = optCount(d);
  const minC = d === 'easy' ? 4 : d === 'intermediate' ? 8 : 12;
  const maxC = d === 'easy' ? 14 : d === 'intermediate' ? 22 : 30;
...
function numRange(d: Difficulty): [number, number] {
  if (d === 'easy') return [10, 120];
  if (d === 'intermediate') return [100, 600];
  return [500, 2000];
}
...
function optC(d: Difficulty): number { return d === 'easy' ? 4 : d === 'intermediate' ? 5 : 6; }
...
// ── Std 7 Question Bank games ──
function genStd7QuizChallenge(d: Difficulty): Question {
  const item = pickStd7QuizQuestion(d);
  ...
  return {
    ...
    correctAnswer: correct,
    hint: item.realLifeHint,
  };
}

function genStd7FindMistake(d: Difficulty): Question {
  const item = pickStd7FindMistakeQuestion(d);
  return {
    id: uid(),
    text: `${item.subject} - ${item.chapter}\nFind the mistake:\n${item.incorrectStatement}`,
    options: buildFindMistakeOptions(item),
    correctAnswer: item.correction,
    hint: `Mistake: ${item.mistake} ${item.explanation}`,
  };
}

function genStd7MatchPair(d: Difficulty): Question {
  const item = pickStd7MatchPairQuestion(d);
  const { options, correctAnswer } = buildMatchPairOptions(item);
  const left = item.left.map((v, i) => `${i + 1}. ${v}`).join('\n');
  const right = item.right.map((v, i) => `${String.fromCharCode(65 + i)}. ${v}`).join('\n');
  return {
    id: uid(),
    text: `${item.subject} - ${item.chapter}\nLeft:\n${left}\nRight:\n${right}\nChoose the correct matching code.`,
    options,
    correctAnswer,
    hint: 'Code example: 1-A, 2-B, 3-C',
  };
}
...
const GENERATORS: Record<string, GenFn> = {
  ...
  // ── Std 7 question-bank games ──
  std7_quiz_challenge: genStd7QuizChallenge,
  std7_find_mistake: genStd7FindMistake,
  std7_match_pair: genStd7MatchPair,
...
export function generateBatch(
  ...
  const tunedDifficulty = upliftStd7Difficulty(difficulty);
  let attempts = 0;

  while (questions.length < count && attempts < count * 20) {
    attempts++;
    try {
      const q = gen(tunedDifficulty);
