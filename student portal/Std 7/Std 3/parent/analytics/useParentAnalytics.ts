...
function computeActivityDistribution(auditLog: AuditEntry[]): Record<string, number> {
...
export function useParentAnalytics(): StudentAnalytics {
  ...
  return useMemo<StudentAnalytics>(() => {
    ...
    const attendRate = Math.max(FLOOR.attendanceRate, rawAttend);
    const streakDays = Math.max(FLOOR.streakDays, stats.streak);
    ...
    // Add EVS as a tracked subject
    const scienceBaseline = { subject: 'EVS', progress: 45, done: 3, total: 6, colorKey: 'emerald' as const };

    const allSubjectDefs = [
      ...subjectDefs,
      { ...scienceBaseline, real: scienceReal },
    ];
    ...
    const skills: SkillMetric[] = hasGameData ? [
      ...
      { skill: 'Comprehension', value: Math.min(100, Math.max(BASELINE_SKILLS[4].value, scienceReal.accuracy)), maxValue: 100 },
      { skill: 'Creativity',    value: Math.min(100, Math.max(BASELINE_SKILLS[5].value, Math.round((englishReal.accuracy + scienceReal.accuracy) / 2))), maxValue: 100 },
    ...
    const realActivity = computeActivityDistribution(auditLog);
    ...
    if (scienceReal.gamesPlayed > 0) {
      alerts.push({
        id: 'a-sci-active', severity: 'success', title: 'EVS Lab Active',
        description: `${scienceReal.gamesPlayed} EVS games played with ${scienceReal.accuracy}% accuracy.`,
        timestamp: nowIso, category: 'achievement',
      });
    }
    ...
    const totalSessions = Math.max(FLOOR.totalSessions, stats.attendance.length);
    ...
    return {
      ...
      totalSessions,
      avgSessionMinutes: activeDays > 0 ? Math.max(FLOOR.avgSessionMin, Math.round(totalMinutes / activeDays)) : FLOOR.avgSessionMin,
