...
const SettingsCard: React.FC<{
  ...
  <motion.div
    style={{
      background: gradient || 'rgba(255,255,255,0.74)',
    ...
    whileHover={{ y: -1, boxShadow: '0 6px 24px rgba(92,106,196,0.09)' }}
  >
    {children}
  </motion.div>
);
          ...
          {isPasswordToggle && (
            ...
            >
              <i
                ...
                style={{ ...ICON_STYLE, fontSize: 16, color: '#4B5563' }}
              />
            </button>
          )}
        </div>
...
export const SettingsPage: React.FC = () => {
  ...
  const handleSaveParentSettings = () => {
    ...
    const result = updateStudentProfile({
      ...
      parentAccessKey: formState.parentAccessKey.trim(),
    });
    ...
    setSaveFeedback({
      tone: 'success',
      message: 'Parent settings saved. Updated access key will be used the next time Parent mode is unlocked.',
    });
  ...
  return (
    <div style={{ maxWidth: 1040, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 32 }}>
      ...
      <SettingsCard
        delay={0.04}
        gradient="linear-gradient(135deg, rgba(255,255,255,0.82), rgba(238,242,255,0.72), rgba(252,231,243,0.6))"
      >
        <SectionHeading
          icon={<i aria-hidden className="fas fa-user-shield" style={ICON_STYLE} />}
          title="Guardian Details and Access"
          subtitle="Edit the parent-facing information and the access key used to open Parent mode."
        />
        ...
        >
          <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: CLR.muted }}>
            Changes stay on this device and the new parent access key is enforced on the next Student to Parent toggle.
          </p>
          <motion.button
            type="button"
      ...
      <SettingsCard delay={0.24} gradient="linear-gradient(135deg, rgba(248,250,252,0.8), rgba(219,234,254,0.3))">
        <SectionHeading
        ...
        <div style={{ fontSize: 12, color: CLR.secondary, lineHeight: '20px' }}>
          <p style={{ margin: '0 0 6px 0' }}>
            <strong>Student Smart Management System</strong> - Std 7 Edition
          </p>
          <p style={{ margin: '0 0 6px 0' }}>
