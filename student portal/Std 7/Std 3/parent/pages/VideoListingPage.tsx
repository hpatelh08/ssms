 ...
 * Dedicated video listing page (English + Maths only).
 */
...
const SUBJECT_THEME: Record<VideoTab, {
  gradient: string;
  toggleBg: string;
...
const SubjectToggle: React.FC<{
  ...
  >
    ...
    >
      {(['English', 'Maths'] as VideoTab[]).map((s) => {
        const isActive = selected === s;
        const bg = SUBJECT_THEME[s].toggleBg;
        const icon = s === 'English' ? '📖' : '🔢';
        const buttonClass = 'relative px-5 py-2.5 rounded-full text-[13px] font-bold transition-all cursor-pointer ' +
...
export const VideoListingPage: React.FC<Props> = ({ onBack, onPlayVideo }) => {
  const [selectedSubject, setSelectedSubject] = useState<VideoTab>('English');
  ...
  return (
    <div className="w-full px-2 lg:px-4 py-8 space-y-8 relative">
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1 font-medium">English and Maths videos only</p>
      </motion.div>

      <SubjectToggle selected={selectedSubject} onChange={setSelectedSubject} />

      <RecentlyWatchedRow subjectItems={subjectUnits} subjectIdSet={subjectIdSet} onWatch={handleWatch} />
