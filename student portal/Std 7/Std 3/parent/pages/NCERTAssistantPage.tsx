...
import { type Subject, type ChapterInfo } from '../../data/ncertChapters';
...
const QUICK_ACTIONS = [
  ...
  { label: 'Give worksheet', icon: '📝', prompt: 'Generate a 5-question worksheet from this chapter suitable for a Class 7 student.' },
    ...
    >
          ...
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-10">
              ...
              <p className="text-xs text-gray-400 mt-1 max-w-[300px]">
                Use quick buttons below or type your question. AI stays grounded to NCERT Class 7 {subjectLabel} content.
              </p>
            </div>
          )}
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        ...
        <span className="text-[10px] text-gray-400 font-medium">
          AI responses are grounded to NCERT Class 7 {subjectLabel} textbook. No external data.
        </span>
      </div>
    </motion.div>
  );
...
export const NCERTAssistantPage: React.FC<{
  ...
  const [selectedChapter, setSelectedChapter] = useState<ChapterInfo>({
    id: 'en-auto-1',
    subject: 'English',
    chapter: 1,
    name: englishUnits[0]?.title || 'English Chapter 1',
    context: englishUnits[0]?.title || 'English Chapter 1',
  });
  ...
  const chapterOptions: ChapterInfo[] = useMemo(
    () =>
      subjectUnits.map((item, i) => ({
        id: `${selectedSubject.toLowerCase()}-auto-${i + 1}-${item.id}`,
        subject: selectedSubject,
        chapter: i + 1,
        name: item.title,
        context: item.title,
      })),
    [selectedSubject, subjectUnits],
  );
  ...
  useEffect(() => {
    ...
    console.log('[Subject Switch]', selectedSubject, '→ chapter reset to', chapterOptions[0]?.name || 'N/A');
  }, [selectedSubject, chapterOptions]);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (ctx.subject) {
          const s: Subject = ctx.subject === 'Maths' ? 'Maths' : 'English';
  ...
  const handleAskAIFromUnit = useCallback((title: string) => {
    // Find best matching chapter in current subject's visible chapter list.
    const match = chapterOptions.find(c =>
    ...
    if (match) {
      ...
      setSelectedChapter({
        id: `${selectedSubject.toLowerCase()}-manual-${Date.now()}`,
        subject: selectedSubject,
        chapter: chapterOptions.length + 1,
        name: title,
        context: title,
      });
    }
    prefillRef.current = `Explain "${title}" for Class 7 students.`;
