...
const ParentShell: React.FC = () => {
  ...
  const handleOpenBook = useCallback((book: BookEntry) => {
    console.info('[ParentLayout] Opening book', { id: book.id, title: book.title, pdfUrl: book.pdfUrl });
    setReaderBook(book);
  }, []);
  ...
  return (
    <div className="min-h-screen pb-20 lg:pb-6 lg:pl-[240px] relative">
      <FloatingWorld />
      <ParentTopBar onOpenSettings={() => setActiveScreen('settings')} />
      <ParentNav active={activeScreen} onNavigate={handleNavigate} />

      <main className="main-dashboard relative z-10">
