...
export const QuizTab: React.FC<Props> = ({ book, chapter }) => {
  ...
  const handleMCQAnswer = useCallback((optionIdx: number) => {
    ...
    const selectedAnswer = q.options?.[optionIdx] || '';
    const correctAnswer = q.options?.[q.correctIndex!] || '';
    const nextAnswers = new Map(answers);
    nextAnswers.set(currentQ, { answer: selectedAnswer, isCorrect });

    console.debug('[GameValidation] reader-quiz-mcq', {
  ...
  }, [questions, currentQ, showFeedback, answers, finishQuiz]);
  ...
  const handleShortAnswer = useCallback(() => {
    ...
    const selectedAnswer = shortAnswer.trim();
    const isCorrect = selectedAnswer.toLowerCase() === correctAnswer.toLowerCase();
    const nextAnswers = new Map(answers);
    nextAnswers.set(currentQ, { answer: shortAnswer, isCorrect });

    console.debug('[GameValidation] reader-quiz-short', {
      questionIndex: currentQ,
      selectedAnswer,
      correctAnswer,
      isCorrect,
  ...
  }, [questions, currentQ, shortAnswer, answers, finishQuiz]);

  function finishQuiz(finalAnswers: Map<number, { answer: string; isCorrect: boolean }>) {
    const finalScore = Array.from(finalAnswers.values()).filter(a => a.isCorrect).length;
    setShowResult(true);
