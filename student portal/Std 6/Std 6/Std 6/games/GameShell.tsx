...
export const GameShell: React.FC<Props> = ({
  ...
  const resetForDifficulty = useCallback((targetDifficulty: Difficulty) => {
  ...
  const launchLevelSession = useCallback((targetDifficulty: Difficulty, level: number) => {
    ...
    const maxLevel = LEVEL_CONFIG[targetDifficulty];
    const boundedLevel = Math.max(1, Math.min(level, maxLevel));
  ...
  useEffect(() => {
    ...
    resetForDifficulty(autoLaunch.difficulty);
