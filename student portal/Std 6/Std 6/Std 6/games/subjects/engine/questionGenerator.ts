...
const RECENT_KEY = 'subjectGames_recentQ_v2';
...
export function generateQuestions(gameType: string, difficulty: Difficulty): Question[] {
  ...
  const totalQuestions = totalLevels * QUESTIONS_PER_LEVEL;
  const questions: Question[] = [];
  const seen = new Set<string>();
  const recent = getRecentSet();
  const levelSequence = getLevelGameSequence(gameType, totalLevels);
  ...
  for (let level = 0; level < totalLevels; level++) {
    ...
    while (levelQuestions.length < QUESTIONS_PER_LEVEL && attempts < 220) {
      attempts++;
      try {
    ...
    // Fallback for this level: allow recent duplicates but still avoid same-session duplicates.
    let fallbackAttempts = 0;
    while (levelQuestions.length < QUESTIONS_PER_LEVEL && fallbackAttempts < 260) {
      fallbackAttempts++;
      try {
        const q = levelGen(difficulty);
        const key = `${q.text}::${q.correctAnswer}`;
  ...
  // Final fallback (rare): fill remaining slots using base generator.
  if (questions.length < totalQuestions) {
    let fallbackAttempts = 0;
    while (questions.length < totalQuestions && fallbackAttempts < 400) {
      fallbackAttempts++;
      try {
        const q = baseGen(difficulty);
        const key = `${q.text}::${q.correctAnswer}`;
        if (!seen.has(key)) {
          seen.add(key);
          questions.push(q);
