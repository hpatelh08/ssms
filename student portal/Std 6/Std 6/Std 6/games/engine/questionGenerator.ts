...
function generateChapterAlignedEnglishQuestion(
  ...
  if (gameTypeId === 'find_noun' || gameTypeId === 'noun_hunt') {
    ...
    const dist = pickN(mixedVerbs.filter(v => v !== correct), n - 1);
    return {
      id: uid(),
      text: `From "${theme.title}", identify the noun most relevant to the chapter context.`,
      options: shuffle([correct, ...dist]),
      correctAnswer: correct,
    };
  }
  ...
  if (gameTypeId === 'plural_maker') {
    ...
    return {
      id: uid(),
      text: `Choose the correct plural form in context ("${theme.title}"): ${singular}`,
      options: shuffle([correct, ...dist]),
      correctAnswer: correct,
    };
  }
  ...
  if (gameTypeId === 'find_verb' || gameTypeId === 'action_match') {
    ...
    const dist = pickN(mixedNouns.filter(w => w !== correct), n - 1);
    return {
      id: uid(),
      text: `Identify the action verb connected to "${theme.title}".`,
      options: shuffle([correct, ...dist]),
      correctAnswer: correct,
    };
  }
  ...
  if (gameTypeId === 'word_order') {
    const correctSentence = pick(theme.sentences).replace(/[.?!]$/, '');
    const words = correctSentence.split(' ');
    const wrong1 = shuffle(words).join(' ');
    const wrong2 = shuffle(words).reverse().join(' ');
    return {
      id: uid(),
      text: `Reorder these words to form a meaningful sentence from "${theme.title}":\n${words.join(' / ')}`,
      options: shuffle([correctSentence, wrong1, wrong2].slice(0, n)),
      correctAnswer: correctSentence,
    };
  }

  if (gameTypeId === 'missing_word') {
    const base = pick(theme.sentences).replace(/[.?!]$/, '');
    ...
    const dist = pickN([...mixedNouns, ...mixedVerbs].filter(w => w !== ans), n - 1);
    return {
      id: uid(),
      text: `Context cloze (${theme.title}). Fill the missing word:\n${blank}`,
  ...
  if (gameTypeId === 'sentence_fix') {
    const correct = pick(theme.sentences).replace(/[.?!]$/, '');
    const words = correct.split(' ');
    const wrong1 = words.length > 3 ? [words[1], words[0], ...words.slice(2)].join(' ') : shuffle(words).join(' ');
    const wrong2 = `${correct} ${pick(themeNouns)}`;
    const wrong3 = correct.toLowerCase();
    return {
      id: uid(),
      text: `Grammar check (${theme.title}): select the best-formed sentence.`,
      options: shuffle([correct, wrong1, wrong2, wrong3].slice(0, n)),
      correctAnswer: correct,
    };
  }
  ...
  if (gameTypeId === 'match_opposite' || gameTypeId === 'find_opposite' || gameTypeId === 'complete_opposite') {
    ...
    return {
      id: uid(),
      text: gameTypeId === 'complete_opposite'
...
export function generateBatch(
  ...
  const historyKey = `ssms_recent_questions_${historyScope}_${difficulty}`;
  ...
  const nextQuestion = (): Question => (
    context?.subject === 'english' && context.chapterId
      ? (generateChapterAlignedEnglishQuestion(gameTypeId, difficulty, context.chapterId) || baseGen(difficulty))
      : context?.subject === 'maths' && context.chapterId
        ? (generateChapterAlignedMathsQuestion(gameTypeId, difficulty, context.chapterId) || baseGen(difficulty))
        : baseGen(difficulty)
  );
  ...
  while (questions.length < count && attempts < count * 60) {
    ...
    try {
      ...
      if (!seen.has(key) && !usedIds.has(key) && !recent.has(key)) {
        seen.add(key);
        usedIds.add(key);
        recent.add(key);
        questions.push(q);
  ...
  // Fallback: if pool is exhausted, allow old-history reuse but still avoid same-session duplicates.
  while (questions.length < count && attempts < count * 240) {
    attempts++;
    try {
      const q = nextQuestion();
      const key = `${q.text}::${q.correctAnswer}`;
  ...
  // Final fallback: keep generating but still enforce strict uniqueness.
  while (questions.length < count && attempts < count * 600) {
    attempts++;
    try {
      const q = nextQuestion();
      const key = `${q.text}::${q.correctAnswer}`;
