      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: CLR.secondary, fontWeight: 500 }}>Student's progress in MCQ fill-in-the-blanks game</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ minWidth: 120 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: CLR.primary }}>Levels Completed</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: CLR.indigo }}>{completed} / {totalLevels}</div>
          </div>
          <div style={{ minWidth: 120 }}>
        ...
        {completed > 0 && (
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: CLR.secondary, marginBottom: 6 }}>Recent Levels</div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {fillBlanks.slice(-5).reverse().map((p, i) => (
                <div key={i} style={{ background: '#F3F4F6', borderRadius: 10, padding: '8px 14px', fontSize: 14, color: CLR.primary }}>
                  Level {p.level}: <b>{p.correct}/{p.total}</b> correct
