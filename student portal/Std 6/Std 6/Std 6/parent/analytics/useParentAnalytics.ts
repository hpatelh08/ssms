...
export function useParentAnalytics(): StudentAnalytics {
  ...
  return useMemo<StudentAnalytics>(() => {
    ...
    const subjects: SubjectProgress[] = BASELINE_SUBJECTS.map((b, i) => {
      const progress = Math.min(100, b.progress + Math.round(growthBoost * (1 - i * 0.1)));
      const chaptersCompleted = Math.min(b.total, b.done + Math.floor(growthBoost / 20));
      return {
        subject: b.subject,
        progress,
        chaptersCompleted,
    ...
    const milestones: Milestone[] = BASELINE_MILESTONES.map(m => ({ ...m }));
