...
const LogicGame: React.FC = () => {
  ...
  const next = () => {
    if (idx + 1 >= LOGIC_QUESTIONS.length) {
      setFinished(true);
  ...
  const restart = () => {
    if (finished) logPuzzleActivity('game_selected', 'Logic Puzzle');
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <span className="text-xs font-bold text-gray-400">Question {idx + 1} / {LOGIC_QUESTIONS.length}</span>
        <span className="text-xs font-bold" style={{ color: '#8b5cf6' }}>Score: {score}</span>
      </div>
      <AnimatePresence mode="wait">
        <motion.div key={idx} initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -30, opacity: 0 }} transition={spring}>
...
const PatternGame: React.FC = () => {
  ...
  const next = () => {
    if (idx + 1 >= PATTERN_QUESTIONS.length) {
      setFinished(true);
  ...
  const restart = () => {
    if (finished) logPuzzleActivity('game_selected', 'Pattern Puzzle');
    setIdx(0); setSelected(null); setScore(0); setFinished(false); setShowHint(false);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <span className="text-xs font-bold text-gray-400">Puzzle {idx + 1} / {PATTERN_QUESTIONS.length}</span>
