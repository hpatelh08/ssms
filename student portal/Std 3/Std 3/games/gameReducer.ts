...
export function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'START_GAME':
      ...
      if (state.status === 'playing') return state;
      {
        ...
        return {
          ...
          totalRounds,
          roundResults: Array(totalRounds).fill(null),
    ...
    case 'SELECT_ANSWER': {
      ...
      const nextRoundResults = [...state.roundResults];
      nextRoundResults[state.round - 1] = isCorrect;

      console.debug('[GameValidation] arcade-game', {
        gameId: state.gameId,
        round: state.round,
