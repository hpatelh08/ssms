/**
 * ➡️ Number After Module
 * ========================
 * "What number comes AFTER ___?" for Std 3 (age 8-9).
 * Difficulty adjusts range:
 *   Easy: 1–20, Intermediate: 1–50, Difficult: 1–99
 */
...
function getMax(diff: 'easy' | 'intermediate' | 'difficult') {
  return diff === 'easy' ? 20 : diff === 'intermediate' ? 50 : 99;
}
...
function generate(used: Set<string>, wrongQ: Question[], diff: 'easy' | 'intermediate' | 'difficult' = 'easy'): Question {
  ...
  for (let i = 0; i < 30; i++) {
    const given = Math.floor(Math.random() * max) + 1;
    const key = `na-${given}`;
    if (!used.has(key)) {
  ...
  const given = Math.floor(Math.random() * max) + 1;
  const answer = given + 1;
  return { given, answer, options: shuffleArray([answer, answer - 1, answer + 1, given]), key: `na-fb-${given}` };
}

export const NumberAfterModule: React.FC<GameModuleProps> = React.memo(({ state, onSelectAnswer, onSetCorrectAnswer, difficulty }) => {
  ...
  return (
    <>
      <motion.div className="mb-6" key={`${state.round}-${q.given}`} initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 18 }}>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">What comes AFTER?</p>
        <div className="inline-flex items-center gap-3">
          <div className="w-24 h-24 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-3xl border-2 border-dashed border-blue-200/50 shadow-lg flex items-center justify-center">
            <motion.span className="text-4xl font-black text-blue-600" animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2, repeat: Infinity }}>
              {q.given}
            </motion.span>
          </div>
          <span className="text-3xl text-gray-300 font-bold">→</span>
          <div className="w-24 h-24 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-3xl border-2 border-dashed border-indigo-200/50 shadow-lg flex items-center justify-center">
            <span className="text-4xl font-black text-indigo-300">?</span>
          </div>
        </div>
      </motion.div>
...
NumberAfterModule.displayName = 'NumberAfterModule';
