/**
 * 🔤⬅️ Letter Before Module
 * ===========================
 * "What letter comes BEFORE ___?" for Std 3 (age 8-9).
 * Difficulty adjusts:
 *   Easy: uppercase B–Z, Intermediate: lowercase b–z, Difficult: mixed case
 */
...
function generate(used: Set<string>, wrongQ: Question[], diff: 'easy' | 'intermediate' | 'difficult' = 'easy'): Question {
  ...
  for (let i = 0; i < 30; i++) {
    const idx = Math.floor(Math.random() * 25) + 1; // 1-25 (B-Z or b-z)
  ...
  const given = alpha[idx];
  const answer = alpha[idx - 1];
  const next = idx < 25 ? alpha[idx + 1] : alpha[0];
  const far = alpha[(idx + 10) % 26];
  return { given, answer, options: shuffleArray([answer, next, alpha[idx], far]), key: `lb-fb-${given}` };
}

export const LetterBeforeModule: React.FC<GameModuleProps> = React.memo(({ state, onSelectAnswer, onSetCorrectAnswer, difficulty }) => {
  ...
  useEffect(() => {
    if (state.status === 'roundEnd') {
      const wasWrong = state.isCorrect === false;
      if (wasWrong) {
  ...
  return (
    <>
      <motion.div className="mb-6" key={`${state.round}-${q.given}`} initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 18 }}>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">What letter comes BEFORE?</p>
        <div className="inline-flex items-center gap-3">
          <div className="w-24 h-24 bg-gradient-to-br from-amber-50 to-yellow-50 rounded-3xl border-2 border-dashed border-amber-200/50 shadow-lg flex items-center justify-center">
            <span className="text-5xl font-black text-amber-300">?</span>
          </div>
          <span className="text-3xl text-gray-300 font-bold">←</span>
          <div className="w-24 h-24 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-3xl border-2 border-dashed border-yellow-200/50 shadow-lg flex items-center justify-center">
            <motion.span className="text-5xl font-black text-amber-600" animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2, repeat: Infinity }}>
              {q.given}
            </motion.span>
          </div>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-3 max-w-sm mx-auto">
        {q.options.map((letter, i) => {
          ...
          const isAnswer = state.status === 'roundEnd' && state.isCorrect === false && isCorrect;
...
LetterBeforeModule.displayName = 'LetterBeforeModule';
