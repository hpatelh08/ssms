...
export const GameSessionScreen: React.FC<Props> = ({
  ...
  const [questionResults, setQuestionResults] = useState<Array<boolean | null>>(() => Array(5).fill(null));
  ...
  const miniLevelScoreRef = useRef(miniLevelScore);
  miniLevelScoreRef.current = miniLevelScore;
  const miniLevelXPRef = useRef(miniLevelXP);
  miniLevelXPRef.current = miniLevelXP;
  const xpEarnedRef = useRef(xpEarned);
  xpEarnedRef.current = xpEarned;
  const wrongPoolRef = useRef(wrongPool);
  ...
  const handleSelect = useCallback((answer: string) => {
    ...
    const questionPosition = phase === 'retry' ? retryIndex : currentQIndex;
    const correctAnswer = currentQuestion.correctAnswer;
    const correct = validateAnswer(answer, correctAnswer) === true;

    console.debug('[GameValidation] subject-session', {
    ...
    if (correct) {
      ...
      setScore(s => s + 1);
      miniLevelScoreRef.current += 1;
      miniLevelXPRef.current += xp;
      xpEarnedRef.current += xp;
      setMiniLevelScore(miniLevelScoreRef.current);
      setXpEarned(xpEarnedRef.current);
      setMiniLevelXP(miniLevelXPRef.current);
      setLastXPAmount(xp);
      ...
      if (phase === 'playing') {
        ...
        setWrongPool(pool => {
          ...
          const next = [...pool, currentQuestion];
          wrongPoolRef.current = next;
          return next;
        });
      }
    }
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (phase === 'retry') {
        // Retry round
        if (retryIndex + 1 >= wrongPoolRef.current.length) {
          setPhase('retryDone');
        } else {
      ...
      if (currentQIndex + 1 >= 5) {
        // Mini-level complete
        handleMiniLevelComplete(miniLevelScoreRef.current, miniLevelXPRef.current);
      } else {
        setCurrentQIndex(qi => qi + 1);
      }
    }, correct ? 1200 : 1500);
  }, [currentQuestion, inputDisabled, difficulty, phase, currentQIndex, retryIndex, onClickSound, onCorrectAnswer, onWrongAnswer, gameType.id]);

  // ── Mini-level complete ──
  const handleMiniLevelComplete = useCallback((levelScore: number, levelXP: number) => {
    xpEarnedRef.current += XP_MINI_BONUS;
    setMiniLevelScore(levelScore);
    setMiniLevelXP(levelXP);
    setXpEarned(xpEarnedRef.current);
  ...
  }, [subject, chapter.id, gameType.id, difficulty, currentMiniLevel]);
  ...
  const handleContinue = useCallback(() => {
    ...
    if (phase === 'retryDone' || phase === 'complete') {
      // Sync all XP
      if (xpEarnedRef.current > 0) onGameWin(xpEarnedRef.current);
    ...
    if (currentMiniLevel >= 5) {
      ...
      if (wrongPoolRef.current.length > 0) {
        saveRetryPool(sessionKey, wrongPoolRef.current.map(q => q.id));
        setRetryIndex(0);
        setRetryScore(0);
        setQuestionResults(Array(wrongPoolRef.current.length).fill(null));
        setPhase('retry');
      } else {
        setPhase('complete');
      }
      ...
      setMiniLevelScore(0);
      setMiniLevelXP(0);
      miniLevelScoreRef.current = 0;
      miniLevelXPRef.current = 0;
      setQuestionResults(Array(5).fill(null));
      setPhase('playing');
    }
  }, [phase, currentMiniLevel, onGameWin, onExit, sessionKey, subject, chapter.id, gameType.id, difficulty]);
