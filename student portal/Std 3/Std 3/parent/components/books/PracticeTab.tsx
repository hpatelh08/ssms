...
export const PracticeTab: React.FC<Props> = ({ book, chapter }) => {
  ...
  }, [book, chapter, resetQuestionState]);
  ...
  const handleCheck = useCallback(() => {
    ...
    const nextScore = score + (answerIsCorrect ? 1 : 0);

    console.debug('[GameValidation] reader-practice-text', {
    ...
    if (answerIsCorrect) setScore(s => s + 1);
    ...
    setTimeout(() => {
      ...
      if (currentIdx < exercises.length - 1) {
        ...
        const pct = Math.round((nextScore / exercises.length) * 100);
        completePractice(book.id, chapter.id, difficulty, pct);
        setStageComplete(true);
      }
    }, 1500);
  }, [exercises, currentIdx, showFeedback, userAnswer, score, book.id, chapter.id, difficulty, resetQuestionState]);
  ...
  const handleOptionSelect = useCallback((option: string) => {
    ...
    console.debug('[GameValidation] reader-practice-option', {
      questionIndex: currentIdx,
      selectedAnswer: answer,
      correctAnswer,
      isCorrect: answerIsCorrect,
  ...
  }, [exercises, currentIdx, showFeedback, score, book.id, chapter.id, difficulty, resetQuestionState]);
    ...
    >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{
            background: showFeedback ? (isCorrect ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)') : 'rgba(255,255,255,0.85)',
            border: showFeedback ? (isCorrect ? '2px solid rgba(16,185,129,0.3)' : '2px solid rgba(239,68,68,0.3)') : '1px solid rgba(255,255,255,0.5)',
          ...
          {exercise.options && exercise.options.length > 0 ? (
            <div className="grid grid-cols-2 gap-2.5">
              {exercise.options.map((opt, oi) => {
                const showSelectedCorrect = showFeedback && selectedAnswer === opt && isCorrect === true;
                const showSelectedWrong = showFeedback && selectedAnswer === opt && isCorrect === false;
                const showRevealCorrect = showFeedback && isCorrect === false && selectedAnswer !== opt && opt === exercise.answer;

                return (
                  <motion.button
                    key={oi}
                    onClick={() => !showFeedback && handleOptionSelect(opt)}
                    disabled={showFeedback}
                    className={`p-3 rounded-2xl text-[13px] font-bold cursor-pointer transition-all text-left ${
                      showSelectedCorrect ? 'bg-emerald-100 text-emerald-700 border-2 border-emerald-300' :
                      showSelectedWrong ? 'bg-red-100 text-red-700 border-2 border-red-300' :
                      showRevealCorrect ? 'bg-emerald-50 text-emerald-700 border-2 border-emerald-200' :
          ...
          <AnimatePresence>
            {showFeedback && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
              >
                {isCorrect ? '🎉 Correct! Great job!' : `❌ The answer is: ${exercise.answer}`}
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
