...
import {
  DifficultySelector, DIFF_META, XP_PER_DIFFICULTY, XP_MINI_BONUS, XP_DIFF_BONUS, XP_ALL_BONUS,
  QUESTIONS_PER_MINI, LEVEL_CONFIG, type DifficultyUnlockState,
} from './DifficultySelector';
import { MiniLevelTracker } from './MiniLevelTracker';
import { QuestionRenderer } from './QuestionRenderer';
import { SpaceLevelMap } from './SpaceLevelMap';
import { TimeUpModal } from '../components/TimeUpModal';
import { usePlaytimeControl } from '../hooks/usePlaytimeControl';
// Progress store (reuse existing localStorage-backed store)
import {
  getGameProgress, saveMiniLevelResult, saveDifficultyTime, allDifficultiesComplete, getDifficultyUnlockStatus,
...
const BADGE_DEFS: Record<string, { id: string; title: string; icon: string }> = {
  easy_star: { id: 'easy_star', title: 'Brown Medal', icon: '🥉' },
  silver:    { id: 'silver',    title: 'Silver Medal', icon: '🥈' },
  golden:    { id: 'golden',    title: 'Golden Medal', icon: '🥇' },
};
...
function getLockedDifficultyMessage(difficulty: Difficulty): string | null {
  if (difficulty === 'intermediate') return 'Complete Easy first to unlock Intermediate';
  if (difficulty === 'difficult') return 'Complete Intermediate first to unlock Hard';
  return null;
}
...
export const GameShell: React.FC<Props> = ({
  ...
  const [questionResults, setQuestionResults] = useState<Array<boolean | null>>(() => Array(QUESTIONS_PER_MINI).fill(null));
  ...
  const difficultyUnlockState = useMemo<DifficultyUnlockState>(() => {
    const gp = getGameProgress(subject, chapter, gameTypeId);
    return getDifficultyUnlockStatus(subject, chapter, gameTypeId, gp);
  }, [subject, chapter, gameTypeId, phase]);

  const handleLockedDifficultySelect = useCallback((d: Difficulty) => {
    const message = getLockedDifficultyMessage(d);
    if (!message) return;
  ...
  const handleSelectDifficulty = useCallback((d: Difficulty) => {
    const unlockState = getDifficultyUnlockStatus(subject, chapter, gameTypeId);
    if (!unlockState[d]) {
      handleLockedDifficultySelect(d);
      return;
    }
  ...
  }, [subject, chapter, gameTypeId, handleLockedDifficultySelect]);
  ...
  const handleSelectLevel = useCallback((level: number) => {
    ...
    setQuestionResults(Array(batch.length).fill(null));
    setDisabled(false);
    setPhase('playing');
  ...
  const handleMiniLevelComplete = useCallback(() => {
    ...
    if (!isRetryRound) {
      ...
      const gp = getGameProgress(subject, chapter, gameTypeId);
      const completedCount = (Object.values(gp[difficulty].miniLevels) as { completed: boolean }[]).filter(m => m.completed).length;
  ...
  const handleSelectAnswer = useCallback((answer: string) => {
    ...
    console.debug('[GameValidation] game-shell', {
      gameTypeId,
      difficulty,
      questionId: q.id,
  ...
  }, [disabled, selectedAnswer, questions, qIndex, difficulty, handleMiniLevelComplete, onCorrectAnswer, onWrongAnswer, gameTypeId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setLockMessage(null), 2800);
  ...
  const isPlaying = phase === 'playing' || phase === 'retry';

  usePlaytimeControl({
    isActive: isPlaying && !showCeleb && !showTimeUpModal,
    onTimeExpired: handleTimeExpired,
  });

  return (
    <div style={{ width: '100%', maxWidth: containerMax, margin: '0 auto', padding: isLevelPhase ? '0' : '0 clamp(8px, 2vw, 20px)', boxSizing: 'border-box', transition: 'max-width 0.3s ease', borderRadius: 24 }}>
        ...
        <div style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '0 16px', boxSizing: 'border-box' }}>
          ...
          >
            <motion.button
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 14, color: '#CBD5E1', background: 'none', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', minWidth: 0 }}
              whileHover={{ x: -3, color: '#F8FAFC' }}
              whileTap={{ scale: 0.95 }}
            >
              ← Back
            </motion.button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
              ...
              <h2 style={{ fontWeight: 800, color: '#E2E8F0', fontSize: 14, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{title}</h2>
            </div>
          ...
          >
            {lockMessage && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="max-w-[760px] mx-auto mb-4 px-4 py-3 rounded-2xl text-sm font-bold text-center"
                style={{
                  border: '1px solid rgba(245,158,11,0.38)',
                  background: 'linear-gradient(135deg, rgba(245,158,11,0.22), rgba(251,191,36,0.08), rgba(15,23,42,0.82))',
                  color: '#FCD34D',
                }}
              >
                {lockMessage}
            ...
            <DifficultySelector
              ...
              onLockedSelect={handleLockedDifficultySelect}
