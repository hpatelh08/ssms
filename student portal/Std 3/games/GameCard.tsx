 ...
 * Glow colors memoized. Dark glass card style for space theme.
 */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          boxShadow: `0 10px 28px ${game.glowColor}, 0 4px 14px rgba(2,6,23,0.22)`,
        }}
      />

      {/* ── Layer 2: Main body — no motion.div, no 3D tilt ── */}
      <div
        className="relative border rounded-3xl p-4 flex flex-col items-center text-center overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(255,255,255,0.14) 0%, rgba(255,255,255,0.03) 42%, rgba(255,255,255,0.08) 100%), linear-gradient(180deg, rgba(7,12,28,0.9) 0%, rgba(15,23,42,0.84) 100%)',
          borderColor: 'rgba(148,163,184,0.18)',
          boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.08), inset 0 -1px 0 rgba(2,6,23,0.35)',
        }}
      >
        {/* Inner gloss arc */}
        <div
        ...
        {/* Seq badge */}
        <div
          className="absolute top-2.5 left-2.5 text-[8px] font-black rounded-lg flex items-center justify-center border"
          style={{
            width: 20,
            height: 20,
        ...
        {/* Title */}
        <h3 className="text-[13px] font-extrabold text-slate-100 mb-0.5 relative z-10 leading-tight">{game.title}</h3>
        <p className="text-[10px] text-slate-300 mb-2 relative z-10 leading-snug">{game.desc}</p>

        {/* XP badge */}
        <div
          className="flex items-center gap-1 px-2.5 py-1 rounded-xl relative z-10 border shadow-sm"
          style={{
            background: 'linear-gradient(135deg, rgba(245,158,11,0.22), rgba(251,191,36,0.1))',
        ...
        >
          ...
          <span className="font-bold text-amber-200 text-[10px]">+{XP_REWARDS.GAME_WIN} XP</span>
        </div>
        ...
        <div
          className="mt-2 text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 opacity-45 text-slate-300 group-hover:opacity-100 group-hover:text-cyan-300 transition-all duration-200"
        >
          <span>▶</span>
          Tap to Play
        </div>
