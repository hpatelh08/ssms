...
const BigProgressBar: React.FC<{
  current: number; total: number; showFeedback: boolean; isCorrect: boolean | null;
  results: Array<boolean | null>;
  meta: typeof DIFF_META['easy'];
}> = React.memo(({ current, total, showFeedback, isCorrect: currentResultIsCorrect, results, meta }) => (
  <div style={{
    ...
    {Array.from({ length: total }, (_, i) => {
      ...
      const roundResult = results[i] ?? null;
      const isPastCorrect = roundResult === true;
      const isPastWrong = roundResult === false;
      const isPast = isPastCorrect;
      const isCorrect = isCurrent ? currentResultIsCorrect : isPastCorrect;
      const ans = isPastWrong || (showFeedback && isCurrent);
      return (
        <div key={i} style={{ position: 'relative' }}>
          <div style={{
            ...
            fontWeight: 900,
            fontSize: isCurrent ? 'clamp(16px, 2.5vw, 22px)' : 'clamp(13px, 2vw, 18px)',
            color: isPastCorrect || isPastWrong || ans || isCurrent ? '#fff' : '#B0ADA8',
            background: isPastCorrect ? 'linear-gradient(135deg, #68D391, #48BB78)'
              : isPastWrong ? 'linear-gradient(135deg, #FCA5A5, #F87171)'
              : ans ? isCorrect ? 'linear-gradient(135deg, #68D391, #38A169)' : 'linear-gradient(135deg, #FCA5A5, #F87171)'
              : isCurrent ? meta.warmGrad : 'rgba(230,228,224,0.5)',
            boxShadow: isPastCorrect
              ? '0 3px 10px rgba(72,187,120,0.25)'
              : isPastWrong
                ? '0 3px 10px rgba(248,113,113,0.22)'
                : isCurrent
                  ? `0 3px 14px ${meta.glowColor}`
                  : 'none',
            transition: 'all 0.3s ease',
            animation: isCurrent && !ans ? 'qr-dot-pulse 1.5s ease-in-out infinite' : 'none',
          }}>
...
interface AnswerOptionProps {
  ...
  questionId: string;
  isSelected: boolean;
  showSelectedCorrect: boolean;
  showSelectedWrong: boolean;
  showRevealCorrect: boolean;
  disabled: boolean;
  meta: typeof DIFF_META['easy'];
...
const AnswerOption: React.FC<AnswerOptionProps> = React.memo(({
  opt, index, questionId, isSelected, showSelectedCorrect, showSelectedWrong,
  showRevealCorrect, disabled, meta, onSelect,
}) => {
  const handleClick = useCallback(() => onSelect(opt), [onSelect, opt]);
  const showCorrect = showSelectedCorrect;
  const showWrong = showSelectedWrong;

  const tileBg = showSelectedCorrect
    ...
    : showSelectedWrong
      ...
      : showRevealCorrect
        ? 'linear-gradient(135deg, rgba(220,252,231,0.7), rgba(187,247,208,0.35))'
      : isSelected
        ? 'linear-gradient(135deg, rgba(255,248,220,0.7), rgba(253,230,138,0.4))'
        : 'rgba(255,255,255,0.65)';
  const tileBorder = showSelectedCorrect
    ? '2.5px solid #48BB78'
    : showSelectedWrong
      ? '2.5px solid #FC8181'
      : showRevealCorrect
        ? '2.5px solid rgba(72,187,120,0.45)'
      : isSelected
        ? '2.5px solid #F6C343'
        : '2px solid rgba(226,232,240,0.4)';
  const tileShadow = showSelectedCorrect
    ...
    : showSelectedWrong
      ...
      : showRevealCorrect
        ? '0 4px 18px rgba(72,187,120,0.14)'
      : '0 3px 14px rgba(0,0,0,0.04)';

  const dimmed = disabled && !isSelected && !showSelectedCorrect && !showRevealCorrect;
  ...
  return (
    <button
      ...
      style={{
        ...
        border: tileBorder,
        background: tileBg,
        color: showSelectedCorrect || showRevealCorrect ? '#276749' : showSelectedWrong ? '#9B2C2C' : '#4A4540',
        ...
        transform: showSelectedCorrect ? 'scale(1)' : showSelectedWrong ? 'none' : 'scale(1)',
        animation: showSelectedCorrect ? 'qr-answer-correct 0.35s ease-out' : showSelectedWrong ? 'qr-feedback-shake 0.5s ease-out' : 'none',
      }}
    >
      {/* Correct answer glow effect — CSS animation */}
      {(showSelectedCorrect || showRevealCorrect) && (
        <div
          style={{
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 'clamp(10px, 2vw, 18px)' }}>
        ...
        <span style={{
          ...
          fontWeight: 900,
          background: showSelectedCorrect ? '#48BB78' : showSelectedWrong ? '#FC8181' : showRevealCorrect ? 'rgba(72,187,120,0.75)' : meta.warmGrad,
          color: '#fff',
          boxShadow: showSelectedCorrect
            ? '0 3px 12px rgba(72,187,120,0.35)'
            : showSelectedWrong
              ? '0 3px 12px rgba(252,129,129,0.35)'
              : showRevealCorrect
                ? '0 3px 12px rgba(72,187,120,0.2)'
...
export const QuestionRenderer: React.FC<Props> = React.memo(({
  ...
  selectedAnswer, isCorrect, showFeedback, results, onSelect, disabled, xpEarned,
}) => {
  const meta = DIFF_META[difficulty];
  const answered = showFeedback && selectedAnswer !== null;
  ...
  return (
    ...
    >
      ...
      <BigProgressBar current={questionIndex} total={totalInLevel} showFeedback={showFeedback} isCorrect={isCorrect} results={results} meta={meta} />
      ...
      <div style={{ position: 'relative', animation: 'qr-bob 5s ease-in-out infinite' }}>
        ...
        >
          ...
          {/* Question text — HUGE for 1st std */}
          <div
            style={{
          ...
          <div style={{
            ...
            {question.options.map((opt, i) => {
              ...
              const showSelectedCorrect = answered && isSelected && isCorrect === true;
              const showSelectedWrong = answered && isSelected && isCorrect === false;
              const showRevealCorrect = answered && isCorrect === false && !isSelected && opt === question.correctAnswer;
              ...
              return (
                <AnswerOption
                  ...
                  showSelectedCorrect={showSelectedCorrect}
