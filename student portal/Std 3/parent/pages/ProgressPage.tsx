...
const CARD_GRADIENTS = {
  xp:         'linear-gradient(135deg, rgba(99,102,241,0.14) 0%, rgba(168,85,247,0.1) 55%, rgba(236,72,153,0.08) 100%)',
  growth:     'linear-gradient(135deg, rgba(16,185,129,0.14) 0%, rgba(34,197,94,0.08) 100%)',
  attendance: 'linear-gradient(135deg, rgba(56,189,248,0.14) 0%, rgba(99,102,241,0.08) 100%)',
  streak:     'linear-gradient(135deg, rgba(251,146,60,0.14) 0%, rgba(244,114,182,0.08) 100%)',
};

const ADMIN_TOKEN_STORAGE_KEY = 'ssms_std3_admin_token';
...
const SUBJECT_STATUS: Record<string, { label: string; color: string }> = {
  ...
  'EVS':      { label: 'Strong Progress', color: '#10B981' },
...
export const ProgressPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const grade = Number(studentProfile?.grade || user?.grade || 3) || 3;
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', paddingBottom: 40, position: 'relative' }}>
      ...
      {/* Floating particles */}
      <FloatingParticle delay={0} size={80} color="#818CF8" left="5%" top="2%" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 28 }}>
        ...
        <HeroMetric
          ...
          sub="XP remaining"
          accent={CLR.amber} gradient="linear-gradient(135deg, rgba(251,146,60,0.14) 0%, rgba(245,158,11,0.08) 100%)" icon="🎯" delay={0.15}
        />

        {/* Animated Circular Progress */}
        <motion.div
          style={{
            background: panelBackground(CARD_GRADIENTS.growth),
            borderRadius: 18,
            padding: '16px 20px',
            border: `1px solid ${PANEL_BORDER}`,
            boxShadow: PANEL_SHADOW,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
          {strongAreas.map((area, i) => (
            <motion.div
              key={area.title}
              style={{
                background: subPanelBackground(`linear-gradient(135deg, ${area.accent}08, ${area.accent}04)`),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {aiRecommendations.map((rec, i) => (
            <motion.div
              key={i}
              style={{
                background: panelBackground('linear-gradient(135deg, rgba(129,140,248,0.08), rgba(167,139,250,0.04))'),
