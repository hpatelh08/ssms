...
function getLogoutTheme(grade?: number): LogoutTheme {
  return LOGOUT_THEMES[String(grade || 3)] || LOGOUT_THEMES[3];
}
...
export const ParentTopBar: React.FC<ParentTopBarProps> = React.memo(({ onOpenSettings }) => {
  ...
  const logoutTheme = useMemo(() => getLogoutTheme(studentProfile?.grade || user?.grade || 3), [studentProfile?.grade, user?.grade]);
