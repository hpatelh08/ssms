...
function buildOfflineGroqFallback(userPrompt: string): string {
  ...
  if (prompt.includes('question:')) return JSON.stringify({ answer: 'I can help with a simple answer.', sources: [] });
...
export async function getChapterSummary(
  ...
  const systemPrompt = `You are a Class 3 teacher creating a chapter summary for parents.
...
export async function getChapterKeywords(
  ...
  const systemPrompt = `You are a Class 3 teacher. Extract important keywords from this chapter.
...
export async function getPracticeQuestions(
  ...
  const systemPrompt = `You are a Class 3 teacher creating practice questions.
...
export async function getQuizQuestions(
  ...
  const systemPrompt = `You are a Class 3 teacher creating a multiple-choice quiz.
ONLY base questions on the textbook context provided.
Return a JSON object with:
...
NEVER use difficult words. Keep answers to 2-3 short paragraphs max.`
    : `You are a friendly Class 3 AI teacher. A parent is asking about chapter "${chapter.name}" from "${book.title}".
...
export async function getLearnContent(
  ...
  const systemPrompt = `You are a Class 3 teacher preparing a fun, engaging lesson for a 6-year-old.
...
export async function getPracticeExercises(
  ...
  const systemPrompt = `You are a Class 3 teacher creating practice exercises.
...
export async function getAdaptiveQuiz(
  ...
  const systemPrompt = `You are a Class 3 teacher creating an adaptive quiz for a 6-year-old.
ONLY base questions on the textbook context.
...
export async function getFlashcards(
  ...
  const systemPrompt = `You are a Class 3 teacher creating flashcards for young children.
ONLY use the textbook context provided.
Return a JSON object with:
