...
const SPACE_GAME_IDS = new Set([
  'SpaceWar', 'space_war', 'spaceWar',
]);
...
function isSpaceWarAction(e: AuditLogEntry): boolean {
  if (e.category === 'game') {
    const game = String(e.details?.game ?? '');
    if (SPACE_GAME_IDS.has(game) || game.toLowerCase().includes('space')) return true;
  }
  if (e.action === 'navigation' && String(e.details?.screen ?? '') === 'space-war') return true;
  return false;
}

function isGameAction(e: AuditLogEntry): boolean {
  return e.category === 'game' && !isSpaceWarAction(e);
}
...
function computeState(
  ...
  const activity = buildActivitySnapshot(log);
  const attendance = activity.activeDates.length > 0
    ? activity.activeDates
    : normalizeDateList(stats.attendance);
  const streak = activity.activeDates.length > 0
  ...
  const days: DayAttendance[] = weekDates.map((dateKey, index) => {
    ...
    for (const e of entries) {
      ...
      if (isSpaceWarAction(e)) spaceWar++;
      if (isNCERTAction(e)) ncertUsage++;
    }

    // Rough estimate: each non-navigation action ≈ 2 min
    const meaningful = entries.filter(e => e.category !== 'navigation' && e.category !== 'parent').length;
    const timeSpent = activity.currentWeekMinutes[index] || (meaningful * 3);

    return { dateKey, games, spaceWar, ncertUsage, timeSpent, present };
  });
  ...
  const fallbackRate = schoolDays > 0
    ? Math.round((presentThisMonth / schoolDays) * 100)
    : 0;
  const rate = activity.activeDates.length > 0 ? activity.attendanceRate : fallbackRate;

  // Monthly attended days (all, including weekends)
  const monthlyDays = activity.activeDates.length > 0
    ? activity.monthlyActiveDays
    : attendance.filter(d => d.startsWith(prefix)).length;

  // Weekly active (last 7 calendar days)
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - 7);
  const cutoffIso = toLocalDateKey(cutoff);
  const weeklyActive = activity.activeDates.length > 0
    ? activity.weeklyActiveDays
    : attendance.filter(d => d >= cutoffIso).length;
...
export const AttendanceProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [state, setState] = useState<AttendanceState>(() =>
    computeState(readStatsRaw(), readAuditLog()),
  );

  useEffect(() => {
    const tick = () =>
      setState(computeState(readStatsRaw(), readAuditLog()));
