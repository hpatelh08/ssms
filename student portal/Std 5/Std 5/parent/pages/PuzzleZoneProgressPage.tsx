  ...
  >
    ...
    <div style={{ display: 'flex', gap: 8 }}>
      ...
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: 20, fontWeight: 800, color: CLR.green }}>{sessions > 0 ? `${accuracy}%` : '—'}</p>
      </div>
    </div>
...
export const PuzzleZoneProgressPage: React.FC = () => {
  ...
  const played       = puzzleGames.filter(g => g.totalSessions > 0);
  const avgAccuracy  = played.length > 0
    ? Math.round(played.reduce((s, g) => s + g.overallAccuracy, 0) / played.length)
    : 0;
