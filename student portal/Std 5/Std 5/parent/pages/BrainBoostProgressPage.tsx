...
const GameCard: React.FC<{
  ...
  sessions: number; accuracy: number; stars: number; lastPlayed: string;
  delay: number;
}> = ({ icon, label, color, sessions, accuracy, stars, lastPlayed, delay }) => (
  <motion.div
  ...
  >
    ...
    <div style={{ display: 'flex', gap: 8 }}>
      ...
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: 20, fontWeight: 800, color: CLR.mint }}>{sessions > 0 ? `${accuracy}%` : '—'}</p>
      </div>
    </div>

    {/* Accuracy bar */}
    {sessions > 0 && (
      <div style={{ marginTop: 10, height: 5, borderRadius: 4, background: 'rgba(129,140,248,0.12)', overflow: 'hidden' }}>
        <motion.div
          initial={{ width: 0 }}
...
export const BrainBoostProgressPage: React.FC = () => {
  ...
  const avgAccuracy   = brainGames.filter(g => g.totalSessions > 0).length > 0
    ? Math.round(brainGames.filter(g => g.totalSessions > 0).reduce((s, g) => s + g.overallAccuracy, 0) / brainGames.filter(g => g.totalSessions > 0).length)
    : 0;
        ...
        {BRAIN_GAMES.map((id, i) => {
          ...
          return (
            <GameCard
              ...
              accuracy={entry ? Math.round(entry.overallAccuracy) : 0}
              stars={entry?.stars ?? 0}
