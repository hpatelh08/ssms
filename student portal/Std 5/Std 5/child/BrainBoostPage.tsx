...
const BRAIN_GAMES: GameDef[] = [
  ...
  },
  {
    ...
    icon: "🃏",
    bg: "linear-gradient(160deg, #ec4899 0%, #db2777 100%)",
        ...
        {activeGame ? (
          ...
          <motion.div key="hub" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div style={{
              minHeight: "100vh",
              background: `
                radial-gradient(ellipse 80% 40% at 50% 0%, #ffe4f1 0%, transparent 62%),
                radial-gradient(ellipse 60% 60% at 10% 50%, #ffd9ec 0%, transparent 55%),
                radial-gradient(ellipse 50% 50% at 90% 60%, #fbcfe8 0%, transparent 55%),
                linear-gradient(180deg,#fff1f7 0%,#ffe7f3 18%,#fdf2f8 42%,#fce7f3 68%,#fbcfe8 100%)
              `,
              position: "relative",
              overflow: "hidden",
            }}>
              {/* Classroom shelf strip (top right) */}
              <div style={{ position: "absolute", top: 0, right: 0, width: "30%", height: "100%", background: "linear-gradient(90deg, transparent 0%, rgba(236,72,153,0.10) 40%, rgba(236,72,153,0.16) 100%)", pointerEvents: "none", zIndex: 0 }} />
              {/* Floor shadow */}
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "20%", background: "linear-gradient(0deg, rgba(236,72,153,0.16) 0%, transparent 100%)", pointerEvents: "none", zIndex: 0 }} />
              {/* Desk silhouettes */}
              <div style={{ position: "absolute", bottom: 0, left: "5%", width: 120, height: 60, background: "rgba(244,114,182,0.22)", borderRadius: "8px 8px 0 0", pointerEvents: "none", zIndex: 0 }} />
              <div style={{ position: "absolute", bottom: 0, right: "8%", width: 100, height: 50, background: "rgba(236,72,153,0.20)", borderRadius: "8px 8px 0 0", pointerEvents: "none", zIndex: 0 }} />

              {/* Content */}
              <div style={{ position: "relative", zIndex: 1 }}>

            {/* Hero Header */}
            <div style={{ background: "linear-gradient(135deg, #ffe7f3 0%, #ffd3ea 55%, #f9a8d4 100%)", padding: "18px 28px 28px", borderRadius: "20px 20px 36px 36px", margin: "16px 16px 0", position: "relative", overflow: "hidden", boxShadow: "0 8px 32px rgba(236,72,153,0.30), 0 2px 8px rgba(0,0,0,0.10)", border: "2px solid rgba(255,255,255,0.35)" }}>
              ...
              <motion.button whileTap={{ scale: 0.95 }} onClick={onBack} style={{ background: "rgba(255,255,255,0.48)", border: "1.5px solid rgba(255,255,255,0.65)", borderRadius: 14, padding: "8px 16px", color: "#9d174d", fontSize: 13, fontWeight: 800, cursor: "pointer", marginBottom: 16, display: "inline-flex", alignItems: "center", gap: 6, backdropFilter: "blur(6px)" }}>
                ← Back
              </motion.button>
              <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
                <motion.div animate={{ rotate: [0, 8, -8, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} style={{ width: 72, height: 72, borderRadius: 22, background: "rgba(255,255,255,0.48)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, flexShrink: 0, boxShadow: "0 6px 20px rgba(236,72,153,0.24)" }}>🧠</motion.div>
                <div>
                  <h1 style={{ color: "#9d174d", fontSize: 34, fontWeight: 900, margin: 0, lineHeight: 1.1 }}>Brain Boost</h1>
                  <p style={{ color: "rgba(131,24,67,0.85)", fontSize: 14, margin: "6px 0 0", fontWeight: 600 }}>Challenge your mind with 3 thinking games!</p>
                  <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {[{ e: "🔢", l: "Patterns" }, { e: "🃏", l: "Memory" }, { e: "🔎", l: "Logic" }].map(({ e, l }) => (
                      <span key={l} style={{ fontSize: 11, fontWeight: 700, color: "#9d174d", background: "rgba(255,255,255,0.50)", borderRadius: 10, padding: "4px 12px", display: "inline-flex", alignItems: "center", gap: 5, border: "1px solid rgba(255,255,255,0.62)" }}>
                        {e} {l}
                      </span>
                    ))}
                  </div>
