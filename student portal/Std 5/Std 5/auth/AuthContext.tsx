...
function loadPersistedRole(): AuthUser {
  try {
    ...
    if (raw) {
      ...
      if (parsed.role && parsed.grade) {
        return { ...parsed, name: parsed.name || 'Yash' } as AuthUser;
      }
    }
  } catch {
    /* corrupted - fall through to default */
  }
  return { role: 'student', grade: 3, name: 'Yash' };
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setGrade = useCallback((grade: number) => {
    setUser(prev => {
      ...
      const next: AuthUser = { role: prev.role, grade, name: prev.name };
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
      } catch {
        /* ignore */
  ...
  const value = useMemo<AuthContextType>(
    () => ({ user, switchRole, setRole, setGrade, logout }),
    [user, switchRole, setRole, setGrade, logout],
  );
