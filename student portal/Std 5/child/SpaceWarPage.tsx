 ...
 * ─────────────────────────────────────────────────────
 * Space War Education Game — Std 5
 *
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ background: 'linear-gradient(180deg, #0a0a2e 0%, #1a0a3e 30%, #0d1b2a 100%)' }}>
      ...
      <div className="absolute" style={{
        ...
        background: 'radial-gradient(ellipse, rgba(63,143,58,0.08) 0%, transparent 70%)',
        filter: 'blur(40px)',
      }} />
    </div>
  );
  ...
  const meteors = useMemo(
    () =>
      Array.from({ length: 30 }, (_, i) => ({
        ...
        color: ['#fbbf24', '#f59e0b', '#f97316', '#60a5fa', '#3b82f6', '#a7c97f'][Math.floor(Math.random() * 6)],
      })),
    []
  );
        ...
        {(phase === 'cockpit' || phase === 'shower') && (
          ...
          >
            ...
            <div style={{
              ...
              borderTop: '2px solid rgba(63,143,58,0.3)',
            }}>
              {/* Control panels */}
              {[0, 1, 2, 3].map(i => (
                <motion.div
                  ...
                  style={{
                    ...
                    height: 40,
                    borderRadius: 8,
                    background: 'rgba(63,143,58,0.2)',
                    border: '1px solid rgba(63,143,58,0.4)',
                  }}
...
const AsteroidBtn: React.FC<{
  ...
  if (isHidden) return <div style={{ width: 'clamp(72px, 18vw, 120px)', flexShrink: 0 }} />;
  ...
  >
    ...
    <motion.h1
      ...
      style={{
        ...
        fontWeight: 900,
        background: 'linear-gradient(135deg, #60a5fa, #a7c97f, #f472b6)',
        WebkitBackgroundClip: 'text',
        WebkitTextFillColor: 'transparent',
        fontFamily: 'Nunito, sans-serif',
        textAlign: 'center',
    ...
    <motion.button
      ...
      whileHover={{ scale: 1.05, boxShadow: '0 0 40px rgba(63,143,58,0.5)' }}
      whileTap={{ scale: 0.95 }}
      style={{
        marginTop: 32,
        padding: '14px 48px',
...
const subjectBadge = (s: 'maths' | 'english' | 'science') => {
  const map = {
    maths: { icon: '🔢', color: '#3f8f3a', label: 'Maths' },
    english: { icon: '📖', color: '#10b981', label: 'English' },
    science: { icon: '🔬', color: '#f59e0b', label: 'Science' },
  };
  return map[s];
};
...
const SpaceWarPage: React.FC<Props> = ({ onBack }) => {
    ...
    setGameState(prev => {
      ...
      if (isCorrectAnswer) {
        ...
        if (shouldShowMeteorShower) {
          ...
          // After 2 seconds, generate new question and continue game
          feedbackTimerRef.current = setTimeout(() => {
            setCorrectAsteroidId(null);
            setBlastPos(null);
            setGameState(s => {
  ...
  const isPlaying = gameState.phase === 'ready' || gameState.phase === 'playing' || gameState.phase === 'boss';

  return (
    <div className="fixed inset-0 overflow-hidden" style={{ fontFamily: 'Nunito, sans-serif' }}>
      {/* Circular hover glow */}
      <style>{`.asteroid-orb:hover { box-shadow: 0 0 30px rgba(251,191,36,0.6), 0 0 60px rgba(251,191,36,0.3) !important; }`}</style>
      <Starfield />
      ...
      {isPlaying && (
        ...
        >
          <AnimatePresence mode="wait">
            <motion.div
              key={gameState.question.id}
              className="w-full h-full flex items-center justify-evenly px-2"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            >
