...
const HEADER_STYLE: React.CSSProperties = {
  background: 'rgba(248,255,244,0.82)',
  border: '1.5px solid rgba(153,187,136,0.35)',
  boxShadow: '0 2px 16px rgba(88,129,74,0.12)',
};
...
export const GameShell: React.FC<Props> = ({
  ...
  // ── Global play timer ──
  const { enterGame, exitGame, timerState } = useGlobalPlayTimer();
  ...
  const handleSelectLevel = useCallback((level: number) => {
    // Block entry if the global play timer has expired
    if (isTimerExpired) return;
    // Start / resume the global play timer
    enterGame(gameTypeId, level);
    ...
    setQuestions(batch);
    setQIndex(0);
    setQuestionResults(Array.from({ length: batch.length }, () => null));
    setSelectedAnswer(null);
    setIsCorrect(null);
    setDisabled(false);
    setPhase('playing');
  }, [gameTypeId, difficulty, enterGame, isTimerExpired]);
  ...
  const handleMiniLevelComplete = useCallback(() => {
    ...
    // ── SCORE 0: Level failed → retry same level, don't unlock next ──
    if (score === 0 && !isRetryRound) {
      exitGame();
      saveMiniLevelResult(subject, chapter, gameTypeId, difficulty, miniLevel, 0, QUESTIONS_PER_MINI);
      setShowCeleb(true);
      setIsFailed(true);
    ...
    if (!isRetryRound) {
      ...
      if (allLevelsDone) {
        // Difficulty complete!
        exitGame(); // pause timer — session is between levels
        ...
        logAction('game_complete', 'game', {
          game: gameTypeId, subject, chapter, difficulty,
          score, total: QUESTIONS_PER_MINI,
          xp: totalXP + xpBonus + diffBonus + allBonus, timeTaken,
        });
      } else {
        // Level complete — show celebration, then back to level grid
        exitGame(); // pause timer — child is between levels
        soundManager.playCelebrate();
        setCelebXP(xpBonus);
        setShowCeleb(true);
        setIsDiffComplete(false);
      ...
      exitGame(); // pause timer — retry session ended
      soundManager.playCelebrate();
      setCelebXP(xpBonus);
      setShowCeleb(true);
      setIsDiffComplete(false);
  ...
  }, [isRetryRound, subject, chapter, gameTypeId, difficulty, miniLevel, totalXP, onGameWin, exitGame]);
  ...
  const handleSelectAnswer = useCallback((answer: string) => {
    ...
    const correct = validateAnswer(answer, q.correctAnswer) === true;
  ...
  const handleContinue = useCallback(() => {
    ...
    // Failed (0 correct) → restart same level with fresh questions
    if (isFailed) {
      setIsFailed(false);
      setMiniScore(0);
      ...
      setIsCorrect(null);
      setDisabled(false);
      if (!isTimerExpired) enterGame(gameTypeId, miniLevel);
      setPhase('playing');
      return;
    }
    ...
    setPhase('selectLevel');
  }, [phase, isFailed, gameTypeId, difficulty, miniLevel, enterGame, isTimerExpired]);
  ...
  // Ensure timer never keeps running after this screen unmounts
  useEffect(() => {
    return () => {
      exitGame();
    };
  ...
  // ── Auto-exit when global play timer expires mid-game ──
  useEffect(() => {
    if (isTimerExpired && (phase === 'playing' || phase === 'retry')) {
      onExit();
    }
  }, [isTimerExpired, phase, onExit]);
  ...
  const celebData = useMemo(() => {
    ...
    const great = score > 0 && score >= Math.ceil(total * 0.6);
    // Stars: 0/5→0, 1-2/5→1, 3-4/5→2, 5/5→3
    const stars = score === 0 ? 0 : perfect ? 3 : score >= 3 ? 2 : 1;
    const accuracy = total > 0 ? Math.round((score / total) * 100) : 0;

    let msg: { title: string; sub: string; emoji: string };
    if (score === 0) msg = { title: "Don't Give Up! 💪", sub: 'Try again — you can do it!', emoji: '😅' };
    else if (isDiffComplete && perfect) msg = { title: 'PERFECT CLEAR! 🏆', sub: 'You mastered every single question!', emoji: '🏆' };
  ...
  const isForestPhase = phase === 'selectDifficulty' || phase === 'selectLevel';
  ...
  return (
    <div style={{
      ...
      margin: '0 auto',
      padding: isLevelPhase ? '0' : isForestPhase ? '12px clamp(8px, 2vw, 20px) 18px' : '0 clamp(8px, 2vw, 20px)',
      boxSizing: 'border-box',
      transition: 'max-width 0.3s ease',
      background: isForestPhase ? 'linear-gradient(180deg, rgba(237,249,229,0.86), rgba(211,232,193,0.7))' : 'transparent',
      borderRadius: isForestPhase ? 24 : 0,
      border: isForestPhase ? '1px solid rgba(153,187,136,0.3)' : 'none',
      boxShadow: isForestPhase ? '0 8px 30px rgba(88,129,74,0.14)' : 'none',
    }}>
      ...
      {!isLevelPhase && (
        <div style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '0 16px', boxSizing: 'border-box' }}>
          ...
          >
            <motion.button
              onClick={() => { onClickSound?.(); exitGame(); onExit(); }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 14, color: '#3f6f45', background: 'none', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', minWidth: 0 }}
              whileHover={{ x: -3, color: '#2f5f3d' }}
              whileTap={{ scale: 0.95 }}
            >
              ← Back
            </motion.button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
              ...
              <h2 style={{ fontWeight: 800, color: '#2f5f3d', fontSize: 14, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{title}</h2>
            </div>
      ...
      <AnimatePresence>
        {showCeleb && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <ConfettiEffect trigger={!isFailed} />
            ...
            >
              ...
              {/* Next level hint */}
              {celebPhase >= 3 && !isDiffComplete && !isRetryRound && !isFailed && miniLevel < LEVEL_CONFIG[difficulty] && (
              ...
              {/* Retry tip */}
              {celebPhase >= 3 && !celebPerfect && !isDiffComplete && !isRetryRound && !isFailed && (
              ...
              {celebPhase >= 3 && (
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                >
                  {isFailed ? '🔄 Try Again!' : isDiffComplete ? '🏠 Back to Menu' : isRetryRound ? '🎉 Done!' : '📋 Level Select ▶'}
                </motion.button>
              )}
            </motion.div>
          </motion.div>
