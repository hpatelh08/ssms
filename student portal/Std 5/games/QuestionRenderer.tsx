...
interface Props {
  ...
  questionResults: Array<boolean | null>;
...
const BigProgressBar: React.FC<{
  current: number; total: number; results: Array<boolean | null>;
  meta: typeof DIFF_META['easy'];
}> = React.memo(({ current, total, results, meta }) => (
  <div style={{
    ...
    {Array.from({ length: total }, (_, i) => {
      ...
      const isCorrectResult = result === true;
      const isWrongResult = result === false;
      ...
      return (
        <div key={i} style={{ position: 'relative' }}>
          <div style={{
            ...
            color: isCorrectResult || isWrongResult || isCurrent ? '#fff' : '#B0ADA8',
            ...
            {isCorrectResult ? '✓' : isWrongResult ? '✕' : num}
          </div>
...
export const QuestionRenderer: React.FC<Props> = React.memo(({
  question, questionIndex, totalInLevel, questionResults, difficulty,
  selectedAnswer, isCorrect, onSelect, disabled, xpEarned,
}) => {
  const meta = DIFF_META[difficulty];
  const answered = selectedAnswer !== null;
  const [hideMemorySequence, setHideMemorySequence] = useState(false);

  const memoryPrompt = useMemo(() => {
    const marker = 'Remember:';
    if (!question.text.startsWith('🃏 Memory Cards!') || !question.text.includes(marker)) {
      return null;
    }

    const lines = question.text.split('\n');
    const rememberLineIndex = lines.findIndex((line) => line.trim().startsWith(marker));
    if (rememberLineIndex === -1) return null;

    return { lines, rememberLineIndex };
  }, [question.text]);

  useEffect(() => {
    setHideMemorySequence(false);
    if (!memoryPrompt) return;

    const timer = setTimeout(() => {
      setHideMemorySequence(true);
    }, 2500);

    return () => clearTimeout(timer);
  }, [question.id, memoryPrompt]);

  const displayQuestionText = useMemo(() => {
    if (!memoryPrompt || !hideMemorySequence) return question.text;

    const maskedLines = [...memoryPrompt.lines];
    const line = maskedLines[memoryPrompt.rememberLineIndex] ?? '';
    const [label, value = ''] = line.split(':');
    const maskedValue = value.replace(/[^\s]/g, '•');
    maskedLines[memoryPrompt.rememberLineIndex] = `${label}: ${maskedValue.trimStart()}`;
    return maskedLines.join('\n');
  }, [question.text, memoryPrompt, hideMemorySequence]);
  ...
  return (
    ...
    >
      ...
      <BigProgressBar current={questionIndex} total={totalInLevel} results={questionResults} meta={meta} />
