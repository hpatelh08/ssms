...
const READING_SESSIONS_KEY = 'ssms_reading_sessions';

interface ReadingSessionLite {
  bookId: string;
  startedAt: string;
  endedAt?: string;
  durationMs: number;
...
function loadReadingSessions(): ReadingSessionLite[] {
  try {
...
export function stopActivityTimer(): number {
  ...
  const elapsed = Date.now() - _activeTimer.start;

  // Save time to progress
  const all = loadProgress();
  const key = getKey(_activeTimer.bookId, _activeTimer.chapterId);
  if (all[key]) {
    all[key].timeSpent[_activeTimer.activity] += elapsed;
...
export function getBookProgress(bookId: string, bookTitle: string, subject: string, totalChapters: number): BookProgress {
  const all = loadProgress();
  const readingSessions = loadReadingSessions().filter(s => s.bookId === bookId);
  ...
  // Reading-session based progress (real page coverage)
  const viewedPages = new Set<number>();
  let estimatedTotalPages = 0;
  let readingTimeMs = 0;
  let readingLastActivityAt: string | null = null;

  for (const s of readingSessions) {
    readingTimeMs += s.durationMs || 0;
    for (const p of s.pagesViewed || []) {
      if (typeof p === 'number' && p > 0) viewedPages.add(Math.floor(p));
    }
    const maxViewed = (s.pagesViewed && s.pagesViewed.length > 0) ? Math.max(...s.pagesViewed) : 0;
    const inferredTotalPages = Math.max(s.totalPages || 0, maxViewed);
    estimatedTotalPages = Math.max(estimatedTotalPages, inferredTotalPages);

    const sessionLast = s.endedAt || s.startedAt;
    if (sessionLast && (!readingLastActivityAt || sessionLast > readingLastActivityAt)) {
      readingLastActivityAt = sessionLast;
    }
  }

  const chapterPercent = totalChapters > 0
    ? Math.round((completedChapters / totalChapters) * 100)
    : 0;
  const readingPercent = estimatedTotalPages > 0
    ? Math.round((viewedPages.size / estimatedTotalPages) * 100)
    : 0;
  const mergedLastActivityAt =
    (!lastActivityAt || (readingLastActivityAt && readingLastActivityAt > lastActivityAt))
      ? readingLastActivityAt
      : lastActivityAt;
  ...
  return {
    ...
    totalChapters,
    completedChapters,
    completionPercent: Math.max(chapterPercent, readingPercent),
    totalStars,
    maxStars: totalChapters * 15,  // 5 activities × 3 stars each
    totalTimeMs: totalTimeMs + readingTimeMs,
