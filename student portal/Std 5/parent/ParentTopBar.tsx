...
function getLogoutTheme(grade?: number): LogoutTheme {
  return LOGOUT_THEMES[String(grade || 5)] || LOGOUT_THEMES[5];
}
...
export const ParentTopBar: React.FC<ParentTopBarProps> = React.memo(({ onOpenSettings }) => {
  ...
  const logoutTheme = useMemo(() => getLogoutTheme(studentProfile?.grade || user?.grade || 5), [studentProfile?.grade, user?.grade]);
            ...
            <div className="dashboard-header-right gap-3">
              <div className="relative">
                <motion.button
                  ...
                  whileHover={{ scale: 1.08, boxShadow: '0 14px 28px rgba(34,197,94,0.24)' }}
                  whileTap={{ scale: 0.92 }}
