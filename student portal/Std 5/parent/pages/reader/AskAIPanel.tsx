...
Your job is to answer student and parent questions using the page context first, and general knowledge when the page does not contain the exact answer.
...
export const AskAIPanel: React.FC<AskAIPanelProps> = ({
  ...
  const quickActions = [
    ...
    { label: '❓ Practice question', question: 'Generate a simple practice question based on this page for a Class 5 student.' },
  ];
              ...
              {quickActions.map((a) => (
                <motion.button
                  ...
                  style={{
                    ...
                    background: 'rgba(63,143,58,0.04)',
                    color: '#2F6F3A',
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: 'pointer',
                  ...
                  whileHover={{ scale: 1.02, background: 'rgba(63,143,58,0.08)' }}
                  whileTap={{ scale: 0.98 }}
                >
                  {a.label}
                </motion.button>
          ...
          >
            ...
            <div
              style={{
                ...
                background:
                  msg.role === 'user'
                    ? 'linear-gradient(135deg, #3F8F3A, #78350F)'
                    : '#FFFFFF',
                color: msg.role === 'user' ? '#FFFFFF' : '#13351d',
                WebkitTextFillColor: msg.role === 'user' ? '#FFFFFF' : '#13351d',
                opacity: 1,
                mixBlendMode: 'normal',
                fontSize: 14,
                lineHeight: 1.6,
                fontWeight: 500,
                border: msg.role === 'ai' ? '1px solid rgba(132,204,22,0.28)' : 'none',
                boxShadow: msg.role === 'ai' ? '0 8px 24px rgba(20,83,45,0.06)' : 'none',
                whiteSpace: 'pre-wrap',
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 4, padding: '8px 14px', background: '#F3F4F6', borderRadius: 12 }}>
              {[0, 1, 2].map((i) => (
                <motion.div
                  key={i}
                  style={{ width: 7, height: 7, borderRadius: '50%', background: '#3F8F3A' }}
            ...
            {quickActions.slice(0, 3).map((a) => (
              <motion.button
                ...
                style={{
                  ...
                  border: '1px solid rgba(63,143,58,0.15)',
                  background: 'rgba(63,143,58,0.04)',
                  color: '#3F8F3A', fontSize: 9, fontWeight: 600,
                  cursor: 'pointer',
                }}
