...
const glass: React.CSSProperties = {
  ...
  boxShadow: '0 6px 28px rgba(63,143,58,0.06)',
};
...
const SUBJECTS: {
  ...
  {
    key: 'english', label: 'English', icon: '📖', color: '#3F8F3A',
...
async function generateWorksheetAI(
  ...
  const systemPrompt = `You are an expert Class 5 (age 8-9) worksheet creator for Indian CBSE/NCERT curriculum.
Subject: ${subjectLabel}
Topic: ${topic}
Difficulty: ${difficulty}

Rules:
- Use simple language appropriate for 6-7 year old children
...
const MCQRenderer: React.FC<{
  ...
  >
    <p className="text-[13px] font-bold text-gray-700 mb-3">
      <span className="text-green-500 mr-2">Q{idx + 1}.</span>
      {q.question}
    </p>
    <div className="grid grid-cols-2 gap-2">
      {q.options.map((opt, oi) => {
        ...
        if (showResult && isSelected && isCorrect) {
          ...
          bg = 'rgba(63,143,58,0.10)';
          border = '1px solid rgba(63,143,58,0.25)';
        }
        return (
          <motion.button
            key={oi}
          ...
          {q.leftColumn.map((left, li) => {
            ...
            return (
              <motion.button
                ...
                style={{
                  background: correct ? 'rgba(16,185,129,0.12)' : wrong ? 'rgba(239,68,68,0.10)' : isSelected ? 'rgba(63,143,58,0.12)' : paired ? 'rgba(122,163,68,0.08)' : 'rgba(255,255,255,0.5)',
                  border: isSelected ? '2px solid rgba(63,143,58,0.4)' : '1px solid rgba(0,0,0,0.06)',
                  color: '#374151',
                }}
                whileHover={!showResult ? { scale: 1.02 } : {}}
              >
                {li + 1}. {left}
                {paired && !showResult && <span className="ml-1 text-green-400">→ {q.rightColumn[userPairs[li]]}</span>}
        ...
        <div className="space-y-2">
          {q.rightColumn.map((right, ri) => (
            <motion.button
              ...
              className="w-full rounded-xl px-4 py-2.5 text-left text-[12px] font-medium cursor-pointer"
              style={{
                background: selectedLeft !== null ? 'rgba(63,143,58,0.06)' : 'rgba(255,255,255,0.5)',
                border: '1px solid rgba(0,0,0,0.06)',
                color: '#374151',
              }}
              whileHover={selectedLeft !== null && !showResult ? { scale: 1.02, background: 'rgba(63,143,58,0.12)' } : {}}
            >
    ...
    <div className="w-full px-2 lg:px-4 py-8 space-y-6 relative" ref={contentRef}>
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#3F8F3A" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
        ...
        {worksheet && step !== 'config' && (
          <motion.button
            ...
            style={{
              ...
              boxShadow: '0 4px 16px rgba(63,143,58,0.25)',
              border: 'none',
            }}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {WORKSHEET_TYPES.map(wt => (
                  <motion.button
                    ...
                    style={{
                      background: selectedType === wt.key ? 'rgba(63,143,58,0.08)' : 'rgba(255,255,255,0.4)',
                      border: selectedType === wt.key ? '2px solid rgba(63,143,58,0.2)' : '1px solid rgba(0,0,0,0.04)',
                    }}
                    whileHover={{ scale: 1.03, y: -2 }}
                    whileTap={{ scale: 0.97 }}
                  >
                    <span className="text-xl block mb-2">{wt.icon}</span>
                    <span className="text-[12px] font-bold block" style={{ color: selectedType === wt.key ? '#3F8F3A' : '#6B7280' }}>
                      {wt.label}
                    </span>
              ...
              >
                ...
                <div className="flex justify-center gap-3 mt-4">
                  <motion.button
                    ...
                    style={{ background: 'rgba(63,143,58,0.1)', color: '#3F8F3A', border: 'none' }}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                  >
                    📝 New Worksheet
                  ...
                  <motion.button
                    ...
                    style={{ background: 'linear-gradient(135deg, #3F8F3A, #8FCF94)', border: 'none', boxShadow: '0 4px 16px rgba(63,143,58,0.25)' }}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                  >
                    📥 Save as PDF
