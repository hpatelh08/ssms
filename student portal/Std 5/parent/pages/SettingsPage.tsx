  ...
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{
        ...
        background: 'linear-gradient(135deg, rgba(63,143,58,0.08), rgba(127,174,101,0.05))',
...
export const SettingsPage: React.FC = () => {
  ...
  const { user } = useAuth();
  const { updateConfig } = useGlobalPlayTimer();
  ...
  const [playtimeSaved, setPlaytimeSaved] = useState(false);
  const showPlaytimeSaved = useCallback(() => {
    setPlaytimeSaved(true);
    setTimeout(() => setPlaytimeSaved(false), 2200);
  }, []);
  ...
  const handlePlaytimeToggle = useCallback(() => {
    setPlaytimeEnabled(prev => {
      ...
      updateConfig(playtimeLimit, next);
      showPlaytimeSaved();
      return next;
    });
  }, [playtimeLimit, updateConfig, showPlaytimeSaved]);
  ...
  const handlePlaytimeChange = useCallback((mins: number) => {
    ...
    updateConfig(mins, playtimeEnabled);
    showPlaytimeSaved();
  }, [playtimeEnabled, updateConfig, showPlaytimeSaved]);
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 32 }}>
      ...
      <SettingsCard delay={0.03}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
          <input value={parentName} onChange={e => setParentName(e.target.value)} placeholder="Parent name" style={{ borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 12px' }} />
          <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone number" style={{ borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 12px' }} />
          <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" style={{ borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 12px' }} />
          <input value={emergencyContact} onChange={e => setEmergencyContact(e.target.value)} placeholder="Emergency contact" style={{ borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 12px' }} />
          <div style={{ position: 'relative' }}>
            <input value={oldParentAccessKey} onChange={e => setOldParentAccessKey(e.target.value)} placeholder="Current access key" type={showCurrentAccessKey ? 'text' : 'password'} style={{ width: '100%', borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 42px 10px 12px', boxSizing: 'border-box' }} />
          ...
          <div style={{ position: 'relative' }}>
            <input value={parentAccessKey} onChange={e => setParentAccessKey(e.target.value)} placeholder="New access key" type={showNewAccessKey ? 'text' : 'password'} style={{ width: '100%', borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 42px 10px 12px', boxSizing: 'border-box' }} />
        ...
        <textarea value={address} onChange={e => setAddress(e.target.value)} placeholder="Address" rows={3} style={{ width: '100%', marginTop: 10, borderRadius: 10, border: '1px solid rgba(63,143,58,0.15)', padding: '10px 12px' }} />
        ...
        <div style={{ marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
          <motion.button onClick={handleSaveParentSettings} type="button" style={{ padding: '8px 16px', fontSize: 12, fontWeight: 700, borderRadius: 10, color: '#fff', background: 'linear-gradient(135deg, #3F8F3A, #7AA344)', border: 'none', cursor: 'pointer' }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>
      ...
      <SettingsCard delay={0.1}>
        ...
        <AnimatePresence>
          {playtimeEnabled && (
            ...
            >
              <div style={{ paddingTop: 14, borderTop: '1px solid rgba(63,143,58,0.08)', marginTop: 8 }}>
      ...
      <SettingsCard delay={0.15}>
        ...
        <div style={{ borderTop: '1px solid rgba(63,143,58,0.08)', paddingTop: 12, marginTop: 8 }}>
          <SettingRow
            icon="🗑️"
      ...
      <SettingsCard delay={0.2} gradient="linear-gradient(135deg, rgba(248,250,252,0.8), rgba(219,234,254,0.3))">
        ...
        <div style={{ fontSize: 12, color: CLR.secondary, lineHeight: '20px' }}>
          <p style={{ margin: '0 0 6px 0' }}><strong>Student Smart Management System</strong> - Std 5 Edition</p>
