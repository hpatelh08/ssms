  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        {!hasAnyData && (
          <span style={{ fontSize: 10, color: CLR.muted, fontStyle: 'italic' }}>Play games to see activity!</span>
        )}
      </div>
      {hasAnyData ? (
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 130 }}>
          {data.map((d, i) => {
            const barH = Math.max((d.sessions / max) * 90, d.sessions > 0 ? 12 : 0);
            const dayLabel = new Date(d.date + 'T00:00:00').toLocaleDateString('en', { weekday: 'short' });
            const isToday = d.date === todayKey;
            return (
              <div key={d.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: CLR.indigo, minHeight: 14 }}>
                  {d.sessions > 0 ? d.sessions : ''}
                </span>
                <div style={{ width: '100%', maxWidth: 32, borderRadius: 8, background: 'rgba(127,174,101,0.12)', height: 90, position: 'relative', display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
                  <motion.div
                    initial={{ height: 0 }}
                    animate={{ height: barH }}
                    transition={{ delay: 0.2 + i * 0.06, type: 'spring', stiffness: 180, damping: 18 }}
                    style={{
                      width: '100%', borderRadius: 8,
                      background: d.sessions > 0
                        ? `linear-gradient(180deg, ${CLR.indigo}, ${CLR.sky})`
                        : 'transparent',
                      boxShadow: d.sessions > 0 ? `0 2px 8px rgba(63,143,58,0.35)` : 'none',
                    }}
                  />
                </div>
                <span style={{
                  fontSize: 9, fontWeight: isToday ? 700 : 600,
                  color: isToday ? CLR.indigo : CLR.muted,
                }}>{dayLabel}</span>
              </div>
            );
          })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, minHeight: 130 }}>
          {data.map(d => {
            const dayLabel = new Date(d.date + 'T00:00:00').toLocaleDateString('en', { weekday: 'short' });
            const isToday = d.date === todayKey;
            return (
              <div key={d.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: CLR.muted }}>0</span>
                <div style={{ width: 8, height: 8, borderRadius: 999, background: 'rgba(127,174,101,0.22)' }} />
                <span style={{
                  fontSize: 9, fontWeight: isToday ? 700 : 600,
                  color: isToday ? CLR.indigo : CLR.muted,
                }}>{dayLabel}</span>
...
const DiffBadge: React.FC<{
  ...
  <div style={{
    ...
    background: unlocked ? `${color}10` : 'rgba(200,200,220,0.06)',
    border: `1px solid ${unlocked ? `${color}22` : 'rgba(200,200,220,0.1)'}`,
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          background: `linear-gradient(135deg, ${config.gradient.replace('from-', '').split(' ')[0]}, rgba(127,174,101,0.1))`,
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div style={{ padding: '0 16px 16px', borderTop: '1px solid rgba(127,174,101,0.08)' }}>
              ...
              {hasPlayed && (
                <div style={{
                  ...
                  background: 'rgba(127,174,101,0.04)',
                }}>
...
const SECTION_ORDER: { key: string; icon: string; label: string; gradient: string; border: string }[] = [
  { key: 'english', icon: '📚', label: 'English', gradient: 'linear-gradient(135deg, #E7F4DF 0%, #EEF8E6 100%)', border: '1px solid rgba(127,174,101,0.15)' },
  { key: 'maths',   icon: '🔢', label: 'Maths',   gradient: 'linear-gradient(135deg, #D7EDD0 0%, #E0F2FE 100%)', border: '1px solid rgba(56,189,248,0.15)' },
  { key: 'science', icon: '🔬', label: 'EVS', gradient: 'linear-gradient(135deg, #D1FAE5 0%, #ECFDF5 100%)', border: '1px solid rgba(16,185,129,0.15)' },
  { key: 'arcade',  icon: '🕹️', label: 'Activity Games', gradient: 'linear-gradient(135deg, #FEF3C7 0%, #FFEDD5 100%)', border: '1px solid rgba(245,158,11,0.15)' },
];
    ...
    <div style={{ maxWidth: 1450, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
        <HeroCard
          ...
          color={CLR.indigo}
          gradient="linear-gradient(135deg, #E7F4DF 0%, #EEF8E6 50%, #F3F8E3 100%)"
        />
        <HeroCard
          icon="🎯"
          label="Overall Accuracy"
          value={data.totalAttempts > 0 ? `${Math.round(data.overallAccuracy * 100)}%` : '—'}
          color={data.overallAccuracy >= 0.7 ? CLR.mint : CLR.amber}
        ...
        <HeroCard
          ...
          color={CLR.sky}
          gradient="linear-gradient(135deg, #D7EDD0 0%, #E0F2FE 50%, #EFF6FF 100%)"
        />
        <HeroCard
          icon="✅"
          ...
          color={CLR.amber}
          gradient="linear-gradient(135deg, #FEF3C7 0%, #FFEDD5 50%, #FFF7ED 100%)"
        />
      </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          {weakGame && weakGame.totalSessions > 0 && weakGame.config.id !== bestGame?.config.id && (
            <motion.div
              ...
              style={{
                flex: '1 1 200px', padding: '14px 16px', borderRadius: CARD_RADIUS,
                background: 'linear-gradient(135deg, rgba(251,146,60,0.08), rgba(244,114,182,0.04))',
