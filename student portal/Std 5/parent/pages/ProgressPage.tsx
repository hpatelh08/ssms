...
const SUBJECT_STATUS: Record<string, { label: string; color: string }> = {
  'English':  { label: 'Improving',       color: '#3F8F3A' },
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <defs>
        <linearGradient id="radarFillProg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#8FCF94" stopOpacity={0.28} />
          <stop offset="100%" stopColor="#D8B674" stopOpacity={0.12} />
        </linearGradient>
      </defs>
      ...
      <motion.polygon points={dataPts} fill="url(#radarFillProg)" stroke="#8FCF94" strokeWidth={2.5}
      ...
      {skills.map((s, i) => {
        ...
        return <motion.circle key={`d-${i}`} cx={cx + r * Math.cos(a)} cy={cy + r * Math.sin(a)} r={4}
          fill="#8FCF94" stroke="white" strokeWidth={2}
...
export const ProgressPage: React.FC = () => {
  ...
  const selectedMarksRecord = useMemo(() => {
    if (marksRecords.length === 0) return null;
    const byExam = (examType: string) => marksRecords.find((row) => row.exam_type === examType) || null;
    return byExam('annual') || byExam('midterm') || marksRecords[0];
  }, [marksRecords]);

  const subjectMarks = useMemo(() => {
    if (!selectedMarksRecord?.marks) return [] as Array<[string, number]>;
    return Object.entries(selectedMarksRecord.marks)
      .map(([subject, value]) => [subject, Number(value)] as [string, number])
      .sort(([a], [b]) => a.localeCompare(b));
  }, [selectedMarksRecord]);

  useEffect(() => {
    const studentName = studentProfile?.studentName || '';
    const grade = Number(studentProfile?.grade || user?.grade || 5) || 5;
    ...
    const loadMarks = async () => {
      try {
        ...
        if (!active) return;
        setMarksError(error instanceof Error ? error.message : 'Unable to load marks right now.');
        setMarksRecords([]);
        setMarksLoading(false);
      }
    };
  ...
  return (
    <div style={{ maxWidth: 1450, margin: '0 auto', paddingBottom: 40, position: 'relative' }}>
      ...
      {/* Floating particles */}
      <FloatingParticle delay={0} size={80} color="#8FCF94" left="5%" top="2%" />
      <FloatingParticle delay={1.5} size={60} color="#F472B6" left="85%" top="8%" />
      <FloatingParticle delay={2.3} size={50} color="#10B981" left="92%" top="45%" />
      <FloatingParticle delay={3} size={70} color="#F59E0B" left="3%" top="55%" />
      ...
      {/* ═══ SECTION 2B — TEACHER MARKS ═══ */}
      <GlassCard delay={0.2} style={{ marginBottom: 24 }}>
        <SectionTitle title="Teacher Uploaded Marks" subtitle="Latest exam scores shared by class teacher" icon="📝" />
        {marksLoading ? (
          <p style={{ fontSize: 12, fontWeight: 600, color: CLR.muted }}>Loading marks...</p>
        ) : marksError ? (
          <p style={{ fontSize: 12, fontWeight: 600, color: CLR.rose }}>{marksError}</p>
        ) : selectedMarksRecord ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
              <span style={{
                ...
                {selectedMarksRecord.exam_type || 'Exam'}
              </span>
              <span style={{ fontSize: 12, fontWeight: 700, color: CLR.primary }}>
                Total: {selectedMarksRecord.total} / {selectedMarksRecord.maxTotal}
              </span>
              <span style={{ fontSize: 12, fontWeight: 700, color: CLR.mint }}>
                {selectedMarksRecord.percent}%
              </span>
              <span style={{
                fontSize: 11,
                fontWeight: 700,
                ...
                Grade {selectedMarksRecord.grade}
              </span>
            </div>

            {subjectMarks.length > 0 ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>
                {subjectMarks.map(([subject, value]) => (
                  <div
                    key={subject}
                    style={{
                  ...
                  >
                    <div style={{ fontSize: 11, fontWeight: 700, color: CLR.secondary }}>{subject}</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: CLR.primary, marginTop: 4 }}>
                      {Number.isFinite(value) ? value : '--'}
                      <span style={{ fontSize: 11, fontWeight: 600, color: CLR.muted }}> / 100</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p style={{ fontSize: 12, fontWeight: 600, color: CLR.muted }}>No subject marks found for this exam.</p>
            )}
          </div>
        ) : (
          <p style={{ fontSize: 12, fontWeight: 600, color: CLR.muted }}>No marks uploaded yet.</p>
        )}
      </GlassCard>
      ...
      <GlassCard delay={0.5} gradient="linear-gradient(135deg, rgba(63,143,58,0.06) 0%, rgba(167,201,127,0.04) 100%)" style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {aiRecommendations.map((rec, i) => (
            <motion.div
              key={i}
              style={{
                background: 'linear-gradient(135deg, rgba(127,174,101,0.08), rgba(167,201,127,0.04))',
                border: '1px solid rgba(127,174,101,0.14)',
              ...
              whileHover={{ x: 3, boxShadow: '0 4px 20px rgba(63,143,58,0.08)' }}
            >
              <div style={{
                width: 36, height: 36, borderRadius: 12, flexShrink: 0,
                background: 'linear-gradient(135deg, #8FCF94, #A7C97F)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: '0 2px 10px rgba(127,174,101,0.25)',
              }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                    <path d="M8 1L10 5.5L15 6.2L11.5 9.5L12.4 14.5L8 12.2L3.6 14.5L4.5 9.5L1 6.2L6 5.5L8 1Z" fill="#8FCF94" />
                  </svg>
      ...
      {/* Decorative shapes */}
      <div style={{ position: 'fixed', bottom: 60, right: 20, width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(127,174,101,0.06), transparent)', pointerEvents: 'none', zIndex: 0 }} />
