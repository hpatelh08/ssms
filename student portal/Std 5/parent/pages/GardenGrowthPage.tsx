...
/* ── Tree stage calculation from growth % ── */
...
function getStageFromGrowth(growth: number) {
  let current = GARDEN_TREE_STAGES[0];
  let next = GARDEN_TREE_STAGES[1];
  for (let i = GARDEN_TREE_STAGES.length - 1; i >= 0; i--) {
    if (growth >= GARDEN_TREE_STAGES[i].threshold) {
      current = GARDEN_TREE_STAGES[i];
      next = GARDEN_TREE_STAGES[Math.min(i + 1, GARDEN_TREE_STAGES.length - 1)];
      break;
    }
  }
  return { current, next };
...
const PASTEL: Record<string, string> = {
  ...
  indigo:  'linear-gradient(135deg, #E7F4DF 0%, #EEF8E6 50%, #F3F8E3 100%)',
  purple:  'linear-gradient(135deg, #EEF8E6 0%, #E7F4DF 50%, #DDF1D4 100%)',
};
...
export const GardenGrowthPage: React.FC = () => {
  ...
  // Derive garden data from real analytics
  const growthPct = analytics.gardenGrowth;
  const { current: stageInfo, next: nextStageInfo } = getStageFromGrowth(growthPct);

  const ts = {
    stage: stageInfo.stage,
    stageEmoji: stageInfo.emoji,
    overallGrowth: growthPct,
    xpContribution: analytics.xp,
    nextStage: nextStageInfo.stage,
    nextStageThreshold: nextStageInfo.threshold,
  };

  const GARDEN_METRICS = [
    { icon: '🌸', label: 'Flowers',              value: analytics.gardenFlowers,   sub: 'Bloomed this month',    colorKey: 'rose'    as const },
    { icon: '🍎', label: 'Fruits',               value: analytics.gardenFruits,    sub: 'From perfect sessions', colorKey: 'amber'   as const },
    { icon: '🌰', label: 'Seeds Collected',      value: analytics.totalSessions,   sub: 'From learning sessions', colorKey: 'emerald' as const },
    { icon: '🪴', label: 'Growth Sessions',      value: analytics.totalSessions,   sub: 'Learning interactions', colorKey: 'cyan'    as const },
    { icon: '🍃', label: 'Leaf Density',         value: Math.round(growthPct * 0.9), sub: 'Tree canopy health', colorKey: 'green'   as const, isPercent: true },
    { icon: '📅', label: 'Attendance Influence', value: analytics.attendanceRate,  sub: 'Days active this month', colorKey: 'indigo'  as const, isPercent: true },
  ];

  const resp = {
    overall: Math.round((analytics.gardenWater + analytics.gardenSunlight + analytics.attendanceRate) / 3),
    breakdown: [
      { label: 'Homework Completion', value: analytics.gardenWater, color: '#3F8F3A' },
      { label: 'Game Participation',  value: analytics.gardenSunlight, color: '#5F8B3D' },
      { label: 'Daily Practice',      value: analytics.attendanceRate, color: '#A7C97F' },
    ],
  };

  const GARDEN_CARE_FACTORS = [
    { icon: '💧', label: 'Water Level', sub: 'Homework',   value: analytics.gardenWater,     color: '#3B82F6', tooltip: 'Water Level increases when homework activities are completed.' },
    { icon: '☀️', label: 'Sunlight',    sub: 'Games',      value: analytics.gardenSunlight,  color: '#F59E0B', tooltip: 'Sunlight grows with game participation and practice sessions.' },
    { icon: '😊', label: 'Happiness',   sub: 'Attendance', value: analytics.attendanceRate,   color: '#10B981', tooltip: 'Happiness rises with regular attendance and daily logins.' },
  ];

  // Dynamic timeline from recent milestones
  ...
  const GARDEN_TIMELINE = analytics.milestones.slice(0, 4).map((m, i) => {
    const d = TIMELINE_DECOR[i % TIMELINE_DECOR.length];
    return { day: m.date?.slice(5, 10) || 'Recent', icon: d.icon, text: m.description, accent: d.accent };
  });
  ...
  return (
    <div style={{ maxWidth: 1450, margin: '0 auto', paddingBottom: 40, position: 'relative' }}>
      ...
      <FloatingParticle delay={1.5} size={60} color="#A7C97F" left="88%" top="6%" />
      ...
      >
        ...
        <p style={{ fontSize: 13, fontWeight: 500, color: CLR.muted, marginTop: 4 }}>
          Track responsibility, engagement, and garden growth progress.
        </p>
      </motion.div>
      ...
      <GlassCard gradient="linear-gradient(135deg, rgba(63,143,58,0.08), rgba(167,201,127,0.04))" delay={0.2} style={{ marginBottom: 24 }}>
      ...
      <GlassCard delay={0.3} style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative', paddingLeft: 28 }}>
          ...
          <div style={{
            ...
            background: 'linear-gradient(to bottom, rgba(63,143,58,0.15), rgba(167,201,127,0.05))',
            borderRadius: 1,
          }} />
      ...
      <div style={{ position: 'fixed', top: '35%', left: 10, width: 90, height: 90, borderRadius: '50%', background: 'radial-gradient(circle, rgba(95,139,61,0.05), transparent)', pointerEvents: 'none', zIndex: 0 }} />
    </div>
  );
};
