    ...
    >
      ...
      <motion.div
        className={[
          ...
          bg,
          isToday ? 'ring-2 ring-emerald-400 ring-offset-2 ring-offset-transparent' : '',
        ].join(' ')}
        style={{
          boxShadow: shadow,
      ...
      >
        ...
        {isToday && (
          <motion.div
            className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-green-400 flex items-center justify-center"
            style={{ boxShadow: '0 2px 8px rgba(63,143,58,0.4)' }}
            animate={{ scale: [1, 1.15, 1] }}
      ...
      <div className="flex items-start justify-between gap-2 overflow-x-auto pb-2">
        {weekBubbles.map((b, i) => {
          ...
          const totalActivities = dayData
            ? dayData.games + dayData.spaceWar + dayData.ncertUsage + dayData.gardenActions
            : 0;
