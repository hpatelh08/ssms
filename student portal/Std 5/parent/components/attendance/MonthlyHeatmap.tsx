  ...
  for (let row = 0; row < 6; row++) {
    ...
    for (let col = 0; col < 7; col++) {
      ...
      if (cellIndex < startDow || dayCounter > daysInMonth) {
        ...
        const spaceWar = d?.spaceWar ?? 0;
        const ncertUsage = d?.ncertUsage ?? 0;
        const gardenActions = d?.gardenActions ?? 0;
        const total = games + spaceWar + ncertUsage + gardenActions;
    ...
    >
      ...
      {cell.isFuture ? (
        ...
        <div className="space-y-0.5">
          ...
          {cell.spaceWar > 0 && <div>🚀 {cell.spaceWar} space war{cell.spaceWar > 1 ? 's' : ''}</div>}
