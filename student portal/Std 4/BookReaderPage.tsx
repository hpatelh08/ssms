...
import { saveFlipbookProgress } from './services/flipbookProgress';

// Reader sub-modules
import { detectPageAnimations, generateAnimationCSS, type PageAnimation } from './parent/pages/reader/storyAnimations';
import { useNarration } from './parent/pages/reader/useNarration';
import { WordPopover } from './parent/pages/reader/WordPopover';
import { AskAIPanel } from './parent/pages/reader/AskAIPanel';
import { useReadingTracker } from './parent/pages/reader/useReadingTracker';
...
const BookReaderPage: React.FC<BookReaderPageProps> = ({ book, onBack }) => {
  ...
  const onFlip = useCallback((e: any) => {
    ...
    // Save flipbook reading progress
    if (numPages > 0) {
      // e.data is the 0-based page index. So display page = e.data + 1
      saveFlipbookProgress(book.id, e.data + 1, numPages);
    }
  }, [narration, numPages, book.id]);
