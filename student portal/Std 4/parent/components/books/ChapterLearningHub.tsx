...
const TABS: TabDef[] = [
  { key: 'learn',    label: 'Learn',    icon: '📖', color: '-yellow- -emerald-',     lockable: false },
  ...
  { key: 'quiz',     label: 'Quiz',     icon: '🧠', color: '-orange- to-pink-400',   lockable: true },
  { key: 'revision', label: 'Revision', icon: '🔁', color: 'from-teal-400 to-emerald-400',  lockable: false },
  { key: 'ai',       label: 'Ask AI',   icon: '🤖', color: '-amber- -orange-', lockable: true },
];
...
export const ChapterLearningHub: React.FC<ChapterLearningHubProps> = ({
  ...
  return (
    <motion.div
      ...
      style={{ background: 'linear-gradient(180deg, #f8fafc 0%, #fef3c7 40%, #faf5ff 100%)' }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${book.gradient || '-yellow- -amber-'} flex items-center justify-center shrink-0`}>
