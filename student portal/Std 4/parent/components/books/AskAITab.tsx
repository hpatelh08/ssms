    ...
    >
      ...
      <div className="text-center py-3">
        ...
        <p className="text-[10px] font-bold text-gray-400 mt-1">
          Ask me anything about <span className="-amber-">{chapter.name}</span>!
        </p>
      </div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              {SUGGESTED_QUESTIONS.map((sq, i) => (
                <motion.button
                  ...
                  className="p-3 rounded-2xl bg-white border border-gray-100 text-left cursor-pointer hover:-amber- hover:-amber-/30 transition-all"
            ...
            >
              <div
                className={`max-w-[85%] rounded-2xl p-3.5 ${
                  msg.role === 'user'
                    ? 'bg-gradient-to-r -amber- -orange- text-white'
              ...
              >
                ...
                {msg.role === 'ai' && (
                  <motion.button
                    ...
                    className="mt-2 text-[9px] font-bold -amber- cursor-pointer hover:-amber-"
                    whileTap={{ scale: 0.95 }}
                  >
        ...
        {loading && (
          ...
          >
            <div className="bg-white border border-gray-100 rounded-2xl p-3 flex items-center gap-1.5" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
              ...
              <motion.span className="w-2 h-2 rounded-full -amber-" animate={{ scale: [1, 1.4, 1] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0 }} />
              <motion.span className="w-2 h-2 rounded-full -amber-" animate={{ scale: [1, 1.4, 1] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0.2 }} />
              <motion.span className="w-2 h-2 rounded-full -amber-" animate={{ scale: [1, 1.4, 1] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0.4 }} />
            </div>
          </motion.div>
        )}
      </div>
      ...
      <div className="pt-3 pb-2 px-1">
        <div className="flex items-center gap-2 bg-white rounded-2xl border border-gray-200 p-1.5" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            ...
            <motion.button
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm cursor-pointer ${
                isListening
                  ...
                  : 'bg-gray-100 text-gray-500 hover:-amber- hover:-amber-'
              }`}
              whileTap={{ scale: 0.9 }}
          ...
          <motion.button
            ...
            className="w-9 h-9 rounded-xl bg-gradient-to-r -amber- -orange- flex items-center justify-center text-white text-sm cursor-pointer disabled:opacity-30"
