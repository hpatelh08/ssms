      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          <motion.button
            ...
            className="px-4 py-2 rounded-2xl text-[12px] font-bold -amber- -amber- border -amber- cursor-pointer disabled:opacity-30"
...
const ErrorCard: React.FC<{ message: string; onRetry: () => void }> = ({ message, onRetry }) => (
  <motion.div className="flex flex-col items-center justify-center py-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <motion.button
      onClick={onRetry}
      className="px-4 py-2 rounded-2xl -amber- -amber- text-xs font-bold cursor-pointer"
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
    >
      Try Again
