            ...
            >
              ...
              <motion.div
                className={[
                  ...
                  isToday ? 'ring-2 -amber- ring-offset-2' : '',
                ].join(' ')}
                style={
                  b.present && !isFuture
...
const cardStyles: Record<string, { bg: string; text: string }> = {
  ...
  monthly: { bg: '-yellow-/60',    text: '-yellow-' },
};
            ...
            <div className="px-4 pb-3">
              {day.present ? (
                <div className="space-y-0.5">
                  <StatRow icon="🎮" label="Games Played" value={day.games} color="-amber-" />
                  <StatRow icon="🎨" label="Color Magic Completed" value={day.colorMagic} color="text-pink-500" />
                  <StatRow icon="🤖" label="NCERT AI Used" value={day.ncertUsage} color="-yellow-" />
...
const AttendanceSectionInner: React.FC = () => (
  <motion.div
    className="rounded-3xl bg-gradient-to-br -yellow- -orange- p-6 shadow-md space-y-6 relative overflow-hidden"
    initial={{ opacity: 0, y: 12 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ ...spring, delay: 0.16 }}
  >
    {/* Decorative blob */}
    <div className="absolute -top-14 -left-14 w-44 h-44 bg-gradient-to-br -yellow- -emerald- rounded-full opacity-[0.07] blur-3xl" />
    <div className="absolute -bottom-10 -right-10 w-36 h-36 bg-gradient-to-br -orange- -amber- rounded-full opacity-[0.06] blur-3xl" />
