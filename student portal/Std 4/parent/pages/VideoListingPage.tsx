        ...
        return (
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-1.5">
              {s === 'English' ? '📖' : s === 'Maths' ? '🔢' : s === 'Hindi' ? '📕' : s === 'Gujarati' ? '📙' : '🔬'} {s}
            </span>
          </motion.button>
        );
      })}
    ...
    <div className="relative aspect-video overflow-hidden cursor-pointer" onClick={onWatch}>
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity">
        <div className="w-14 h-14 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="#f59e0b"><path d="M8 5v14l11-7z" /></svg>
        </div>
      </div>
      {isWatched && (
...
export const VideoListingPage: React.FC<Props> = ({ onBack, onPlayVideo }) => {
  ...
  const subjectAllItems: SyllabusItem[] = subjectUnits;
  ...
  const handleWatch = useCallback((item: SyllabusItem) => {
    ...
    if (match) {
      ...
      // Fallback: construct a VideoEntry from SyllabusItem
      onPlayVideo(
