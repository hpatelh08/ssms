...
const AiHero: React.FC = () => (
  ...
  >
    <div className="absolute -top-12 -left-12 w-40 h-40 -orange- rounded-full opacity-[0.06] blur-3xl" />
      ...
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <div className="flex gap-2">
          {showAskAI && (
            <motion.button
              onClick={onAskAI}
              className="text-[11px] font-bold -amber- px-4 py-2 rounded-full cursor-pointer"
...
const SyllabusCard: React.FC<{
  ...
  return (
    <motion.div
      className={`rounded-3xl relative overflow-hidden group ${isPlaying ? 'ring-2 -amber-/40' : ''}`}
      style={glass}
      ...
      >
        ...
        {isStreaming && (
          <motion.span
            ...
            style={{ background: isUser ? '#fff' : '#f59e0b' }}
            animate={{ opacity: [0, 1] }}
