...
import { getActivitySummary, resetToToday } from '../../services/activityLogger';
...
const SkillRadar: React.FC<{ skills: { skill: string; value: number }[]; size?: number }> = ({ skills, size = 180 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <motion.polygon points={dataPts} fill="url(#radarFill)" stroke="#fbbf24" strokeWidth={2}
...
export const OverviewPage: React.FC = () => {
  ...
  const [activitySummary, setActivitySummary] = useState(getActivitySummary);
  useEffect(() => {
    // Ensure we start with today's data only
    resetToToday();
    setActivitySummary(getActivitySummary());
    
    const refresh = () => setActivitySummary(getActivitySummary());
  ...
  const getSubjectTag = (subject: string, progress: number) => {
    ...
    if (progress >= 45) return { tag: 'On Track', color: '#f59e0b' };
  ...
  return (
    <div style={{ maxWidth: 1120, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24, position: 'relative' }}>
      ...
      <div style={{ position: 'relative' }}>
        {/* Floating particles */}
        <FloatingParticle x={8} y={20} size={40} color="#fbbf24" delay={0} />
        <FloatingParticle x={72} y={10} size={32} color="#fb923c" delay={1.2} />
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <span style={{
              fontSize: 10, fontWeight: 700, padding: '5px 14px', borderRadius: 20,
              background: 'rgba(129,140,248,0.10)', color: '#f59e0b',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, position: 'relative', zIndex: 1 }}>
          ...
          <GlassCard gradient={CARD_GRADIENTS.xp} delay={0.02}>
            ...
            <div style={{ height: 8, borderRadius: 4, background: 'rgba(124,58,237,0.10)', overflow: 'hidden', marginBottom: 10 }}>
              <motion.div
                style={{ height: '100%', borderRadius: 4, background: 'linear-gradient(90deg, #7C3AED, #fcd34d)' }}
                initial={{ width: 0 }}
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{ flex: 1, height: 8, borderRadius: 4, background: `${CLR.indigo}12`, overflow: 'hidden' }}>
                <motion.div
                  style={{ height: '100%', borderRadius: 4, background: `linear-gradient(90deg, #fbbf24, #f59e0b)` }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <GlassCard gradient={GRADIENTS.indigo} delay={0.10}>
          ...
          <div style={{ display: 'flex', gap: 20, marginTop: 14, justifyContent: 'center' }}>
            ...
            <div style={{ width: 1, background: `${CLR.muted}20` }} />
            <div style={{ textAlign: 'center' }}>
              <motion.p style={{ fontSize: 22, fontWeight: 800, color: '#d97706', margin: 0 }}
        ...
        <GlassCard gradient={GRADIENTS.rose} delay={0.14}>
          <SectionTitle icon="🎯" title="Learning Focus Distribution" subtitle={`Today's activity progress (${new Date().toLocaleDateString()})`} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {(() => {
              const rawItems = [
                { icon: '📘', label: 'Watch Video', minutes: activitySummary.watchVideo, color: '#f59e0b' },
                { icon: '🎮', label: 'Games', minutes: activitySummary.games, color: '#eab308' },
                { icon: '📖', label: 'Reading', minutes: activitySummary.reading, color: '#10B981' },
                { icon: '✏️', label: 'Odd One Out', minutes: activitySummary.oddOneOut, color: '#F59E0B' },
                { icon: '🎨', label: 'World Builder', minutes: activitySummary.worldBuilder, color: '#F472B6' },
              ];
              const totalMins = rawItems.reduce((a, b) => a + b.minutes, 0);
              const maxMins = Math.max(...rawItems.map(r => r.minutes), 1);
              const focusItems = rawItems.map(r => ({
                ...r,
                pct: totalMins > 0 ? Math.round((r.minutes / totalMins) * 100) : 0,
                barPct: Math.round((r.minutes / maxMins) * 100),
              }));
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 8 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 600, color: '#10B981' }}>
                Live • Today Only • Updates every 3s
              </span>
            </div>
          </div>
        </GlassCard>
