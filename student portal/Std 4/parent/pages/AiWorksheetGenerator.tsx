...
const SUBJECTS: {
  ...
  {
    key: 'english', label: 'English', icon: '📖', color: '#f59e0b',
...
async function generateWorksheetAI(
  ...
  const systemPrompt = `You are an expert Class 4 (age 6-7) worksheet creator for Indian CBSE/NCERT curriculum.
...
const MCQRenderer: React.FC<{
  ...
  >
    <p className="text-[13px] font-bold text-gray-700 mb-3">
      <span className="-amber- mr-2">Q{idx + 1}.</span>
      {q.question}
    </p>
        ...
        <div className="space-y-2">
          {q.leftColumn.map((left, li) => {
            ...
            return (
              ...
              >
                {li + 1}. {left}
                {paired && !showResult && <span className="ml-1 -orange-">→ {q.rightColumn[userPairs[li]]}</span>}
  ...
  return (
    ...
    >
      <p className="text-[13px] font-bold text-gray-700 mb-2">
        <span className="-orange- mr-2">🧩 Q{idx + 1}.</span>
        {q.clue}
      </p>
      {showHint && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {WORKSHEET_TYPES.map(wt => (
                  ...
                  >
                    ...
                    <span className="text-[12px] font-bold block" style={{ color: selectedType === wt.key ? '#f59e0b' : '#6B7280' }}>
                      {wt.label}
                    </span>
              ...
              >
                ...
                <div className="flex justify-center gap-3 mt-4">
                  <motion.button
                    ...
                    style={{ background: 'rgba(99,102,241,0.1)', color: '#f59e0b', border: 'none' }}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                  >
                    📝 New Worksheet
                  ...
                  <motion.button
                    ...
                    style={{ background: 'linear-gradient(135deg, #f59e0b, #fbbf24)', border: 'none', boxShadow: '0 4px 16px rgba(99,102,241,0.25)' }}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                  >
                    📥 Save as PDF
