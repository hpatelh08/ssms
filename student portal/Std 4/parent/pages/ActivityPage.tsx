...
function actionLabel(action: string): { label: string; icon: string; color: string } {
  const map: Record<string, { label: string; icon: string; color: string }> = {
    game_selected: { label: 'Started a game', icon: '🎮', color: '#d97706' },
    game_complete: { label: 'Completed a game', icon: '🏆', color: '#10b981' },
    xp_earned: { label: 'Earned XP', icon: '⚡', color: '#eab308' },
...
const ScreenTimeCard: React.FC<{ today: number; weekAvg: number }> = ({ today, weekAvg }) => (
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="-yellow-/60 rounded-xl p-3.5 text-center">
        <p className="text-2xl font-extrabold -yellow-">{today}m</p>
        <p className="text-[10px] font-semibold -yellow- mt-0.5">Today</p>
      </div>
      <div className="-amber-/60 rounded-xl p-3.5 text-center">
        <p className="text-2xl font-extrabold -amber-">{weekAvg}m</p>
        <p className="text-[10px] font-semibold -amber- mt-0.5">Daily Avg (7d)</p>
      </div>
    </div>
