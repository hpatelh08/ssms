 ...
 * AI Buddy Learning Zone â€” Strict Subject Isolation
 *
 * Structure per subject:
 *  1. AiHero â€” pastel gradient banner
 *  2. SubjectToggle â€” [ English ] [ Maths ] pill switch
 *  3. Recently Watched row (filtered by subject)
 *  4. Subject Section â€” English Units + English Rhymes  OR  Maths Chapters + Maths Rhymes
 *  5. Ask AI Buddy Chat â€” LOCKED to selectedSubject + selectedChapter
 *
 * âœ… English tab â†’ English units + English rhymes + English AI only
 * âœ… Maths tab  â†’ Maths chapters + Maths rhymes + Maths AI only
 * âœ… No cross-subject mixing. Ever.
 */
...
type AISubject = Extract<Subject, 'English' | 'Maths' | 'Science'>;
const AI_SUBJECTS: AISubject[] = ['English', 'Maths', 'Science'];
const isAISubject = (subject: Subject): subject is AISubject =>
  AI_SUBJECTS.includes(subject as AISubject);
...
const SUBJECT_THEME = {
  ...
  Hindi: {
    ...
    sectionGradient: 'from-rose-100 to-red-100',
    pageTint: 'rgba(254,226,226,0.12)',
    icon: 'HI',
    label: 'Hindi Learning Chapters',
    rhymeLabel: 'Hindi Kavita',
    rhymeIcon: 'R',
  },
  Gujarati: {
    toggleBg: 'bg-amber-500',
    gradient: 'from-yellow-500 to-amber-500',
    sectionGradient: 'from-yellow-100 to-amber-100',
    pageTint: 'rgba(254,243,199,0.12)',
    icon: 'GU',
    label: 'Gujarati Learning Chapters',
    rhymeLabel: 'Gujarati Kavita',
    rhymeIcon: 'R',
  },
  Science: {
    toggleBg: 'bg-emerald-500',
    gradient: 'from-teal-500 to-emerald-500',
    sectionGradient: 'from-teal-100 to-emerald-100',
    pageTint: 'rgba(209,250,229,0.12)',
    icon: 'SC',
    label: 'Science Learning Chapters',
    rhymeLabel: 'Science Facts',
    rhymeIcon: 'R',
  },
} as const;
...
const QUICK_ACTIONS = [
  { label: 'Explain simply', icon: '1', prompt: 'Explain this chapter in simple words.' },
  { label: 'Key points', icon: '2', prompt: 'Give the most important key points from this chapter.' },
  { label: 'Give worksheet', icon: '3', prompt: 'Create 5 simple practice questions from this chapter.' },
  { label: 'Parent tip', icon: '4', prompt: 'Give one practical teaching tip for this chapter.' },
  { label: 'Word meanings', icon: '5', prompt: 'List important words from this chapter with simple meanings.' },
  { label: 'Oral questions', icon: '6', prompt: 'Create 5 oral/viva questions from this chapter with answers.' },
  { label: 'Revision plan', icon: '7', prompt: 'Make a 3-day revision plan for this chapter for a Class 4 student.' },
];
...
const ENGLISH_IDS = new Set(englishUnits.map(u => u.id));
const MATHS_IDS = new Set(mathsChapters.map(c => c.id));
const HINDI_IDS = new Set(hindiChapters.map(c => c.id));
const GUJARATI_IDS = new Set(gujaratiChapters.map(c => c.id));
...
const SubjectToggle: React.FC<{
  ...
  >
    ...
    >
      {(['English', 'Maths', 'Hindi', 'Gujarati', 'Science'] as Subject[]).map(s => {
        ...
        return (
          <motion.button
            key={s}
            onClick={() => onChange(s)}
            className={`relative px-7 py-2.5 rounded-full text-[13px] font-bold transition-all cursor-pointer
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-1.5">
              {s === 'English' ? 'EN' : s === 'Maths' ? 'MA' : s === 'Hindi' ? 'HI' : s === 'Gujarati' ? 'GU' : 'SC'} {s}
            </span>
          </motion.button>
        );
      })}
...
const SyllabusCard: React.FC<{
  ...
  showAskAI?: boolean;
  recommended?: boolean;
}> = ({ item, isWatched, isPlaying, gradient, onWatch, onAskAI, showAskAI = true, recommended }) => {
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex gap-2">
          <motion.button
            onClick={onWatch}
            className={`${showAskAI ? 'flex-1' : 'w-full'} py-2.5 rounded-2xl text-[12px] font-bold text-white shadow-md cursor-pointer bg-gradient-to-r ${gradient}`}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ASK AI SECTION (Chat) â€” LOCKED to parent subject
...
const AskAiSection: React.FC<{
  ...
  const accentColor = subject === 'English' ? '#ea580c' : subject === 'Maths' ? '#7c3aed' : '#059669';
  ...
  useEffect(() => {
    ...
    setError(null);
    setInput('');
    console.log('[AI Chat] Subject changed â†’', subject);
  }, [subject]);
  ...
  useEffect(() => {
    setMessages([]);
    setError(null);
    console.log('[AI Chat] Chapter changed â†’', selectedChapter.name, '(', selectedChapter.subject, ')');
  }, [selectedChapter]);
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full ml-2" style={{
          background: subject === 'English' ? 'rgba(234,88,12,0.1)' :
            subject === 'Maths' ? 'rgba(124,58,237,0.1)' :
              'rgba(5,150,105,0.1)',
          color: accentColor,
        }}>
          {subject === 'English' ? 'EN' : subject === 'Maths' ? 'MA' : 'SC'} {subject}
        </span>
      </div>

      {/* Chapter selector â€” only shows chapters for current subject */}
        ...
        >
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-10">
              <span className="text-4xl mb-3">AI</span>
              <p className="text-sm font-bold text-gray-600">
                Ask about "{selectedChapter.name}"
              </p>
              <p className="text-xs text-gray-400 mt-1 max-w-[300px]">
                Use quick buttons below or type your question. AI stays grounded to NCERT Class 4 {subject} content.
              </p>
            </div>
          )}
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        ...
        <span className="text-[10px] text-gray-400 font-medium">
          AI responses are grounded to NCERT Class 4 {subject} textbook. No external data.
        </span>
      </div>
    </motion.div>
  );
...
export const NCERTAssistantPage: React.FC<{
  ...
  // Chat state â€” ALWAYS synced with selectedSubject
  ...
  const subjectUnits: SyllabusItem[] = useMemo(() => {
    ...
    if (selectedSubject === 'Hindi') return hindiChapters;
    if (selectedSubject === 'Gujarati') return gujaratiChapters;
  ...
  const subjectIdSet = useMemo(() => {
    ...
    if (selectedSubject === 'Hindi') return HINDI_IDS;
    if (selectedSubject === 'Gujarati') return GUJARATI_IDS;
    return SCIENCE_IDS;
  }, [selectedSubject]);
  ...
  // âœ… STEP 3: Reset ALL state on subject switch
  useEffect(() => {
    setSelectedChapter(CHAPTER_DATA[selectedSubject][0]);
    setActiveItem(null);
    setActiveIsRhyme(false);
    console.log('[Subject Switch]', selectedSubject, 'â†’ chapter reset to', CHAPTER_DATA[selectedSubject][0].name);
  }, [selectedSubject]);
  ...
  const handleWatchSyllabus = useCallback((item: SyllabusItem) => {
    ...
    if (onPlayVideo) {
      const allVideos = [...VIDEO_DATA.English, ...VIDEO_DATA.Maths, ...VIDEO_DATA.Hindi, ...VIDEO_DATA.Gujarati, ...VIDEO_DATA.Science];
      const match = allVideos.find(v => v.embedId === item.embedId || v.id === item.id);
      if (match) {
        onPlayVideo(match, selectedSubject);
        return;
      }
    }
    setActiveItem(item);
  ...
  const handleAskAIFromUnit = useCallback((title: string) => {
    ...
    prefillRef.current = `Explain "${title}" for Class 4 students.`;
  ...
  const chatEnabled = isAISubject(selectedSubject);
  ...
  return (
    <div className="w-full px-2 lg:px-4 py-8 space-y-10 relative">
      ...
      <AnimatePresence>
        {activeItem && (
          <VideoPlayerSection
            ...
            showAskAI={chatEnabled && !activeIsRhyme}
            onAskAI={() => {
      ...
      {/* 3. Subject Content (animated swap) â€” units/chapters ONLY for selected subject */}
      ...
      {/* 4. Ask AI Section â€” only for English/Maths/Science */}
      {chatEnabled && (
