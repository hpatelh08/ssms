 ...
 * ========================================
 * Addition within 10 for Standard 1.
...
export const MathPuzzle: React.FC<MathPuzzleProps> = React.memo(({ onExit, onWin, onNextGame }) => {
  ...
  const checkAnswer = useCallback(() => {
    ...
    logAction(input === correct ? 'math_correct' : 'math_wrong', 'game', { problem: `${problem.a}+${problem.b}`, input });
  ...
  return (
    <>
      ...
      <div className="bg-white/75 backdrop-blur-2xl border border-white/40 p-8 sm:p-10 rounded-[24px] text-center max-w-md mx-auto relative overflow-hidden shadow-xl">
        ...
        <AnimatePresence>
          {state.feedback && (
            <FeedbackOverlay type={state.feedback} correctAnswer={`${problem.a} + ${problem.b} = ${problem.answer}`} />
          )}
        </AnimatePresence>
      </div>
    </>
