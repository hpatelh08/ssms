...
const PARENT_PROMPT = `You are a Standard 1 academic assistant.
...
- Use simple words suitable for a 6–7-year-old child.
...
Return a JSON object with exactly two fields:
  ...
  - "simplified": ONE simple sentence (max 20 words) a 6-year-old can understand.`;

const STUDENT_PROMPT = `You are a friendly Standard 1 teacher talking directly to a 6–7 year old child.
Answer ONLY using the provided textbook context.
...
- Do NOT use advanced language. Use only words a 6-year-old knows.
  ...
  try {
        ...
        messages: [
          { role: 'system', content: mode === 'student' ? STUDENT_PROMPT : PARENT_PROMPT },
          {
            ...
            content: mode === 'student'
              ? `TEXTBOOK CONTEXT:\n${contextString}\n\n---\n\nQuestion:\n${query}\n\nAnswer simply for a 6-year-old. Then give practice questions from this chapter only.`
              : `TEXTBOOK CONTEXT:\n${contextString}\n\n---\n\nQuestion:\n${query}\n\nExplain in simple language suitable for a 6–7 year old child.`,
          },
        ],
        temperature: 0.15,
        max_tokens: 1024,
    ...
    if (!groqResponse.ok) {
      ...
      if (isGroqAuthError(groqResponse.status)) {
        const fallback: ChapterRAGResponse = {
          explanation: 'I could not reach the AI service right now, so I am using a simple textbook-only fallback.',
          simplified_explanation: 'Please read the chapter again and try one more time.',
          chapter: chapterRaw,
          page_numbers: pages,
          similarity_scores: searchResults.map(r => +r.score.toFixed(4)),
          source_chunks,
          ...
          isFallback: true,
          mode,
          ...(mode === 'student' ? { practice_questions: [] as string[] } : {}),
        };
        return fallback;
      }
    ...
    if (error instanceof Error && /Groq API error \(40[13]\)/.test(error.message)) {
      return {
        explanation: 'I could not reach the AI service right now, so I am using a simple textbook-only fallback.',
        simplified_explanation: 'Please read the chapter again and try one more time.',
        chapter: chapterRaw,
