...
const themedSurface = {
  shell: {
    ...
    background: [
      'radial-gradient(circle at 12% 18%, color-mix(in srgb, #ff9f6b 30%, transparent), transparent 34%)',
      'radial-gradient(circle at 88% 20%, color-mix(in srgb, #74d3ff 28%, transparent), transparent 32%)',
      'radial-gradient(circle at 82% 82%, color-mix(in srgb, #ffd86b 26%, transparent), transparent 30%)',
      'linear-gradient(135deg, #fff7ef 0%, #fffef8 42%, #eefbf3 100%)',
    ].join(', '),
  } as React.CSSProperties,
  dots: {
    backgroundImage: [
      'radial-gradient(circle at 8% 20%, rgba(255,255,255,0.96) 0 1px, transparent 1px)',
      'radial-gradient(circle at 24% 76%, rgba(255,214,122,0.8) 0 1px, transparent 1px)',
      'radial-gradient(circle at 72% 28%, rgba(116,211,255,0.8) 0 1px, transparent 1px)',
      'radial-gradient(circle at 85% 64%, rgba(255,159,107,0.9) 0 1px, transparent 1px)',
    ].join(', '),
  } as React.CSSProperties,
  panel: {
  ...
  story: {
    background: 'linear-gradient(180deg, #fff4df 0%, #ffd6e7 52%, #d8f8f2 100%)',
  ...
  primaryButton: {
    background: 'linear-gradient(135deg, #ff7a59 0%, #ffb347 52%, #f59e0b 100%)',
    boxShadow: '0 10px 24px rgba(255, 153, 84, 0.32)',
  } as React.CSSProperties,
  error: {
  ...
  pill: {
    ...
    boxShadow: '0 10px 24px rgba(255, 122, 89, 0.12)',
  } as React.CSSProperties,
  overlay: {
...
const modalCopy = {
  badge: 'Cartoon Family Pass',
  title: 'Enter Parent Access Key',
  description: 'Confirm family access to open the parent dashboard for',
  label: 'Parent Access Key',
  placeholder: 'Enter parent key',
  helper: 'Use this family key to switch from the cartoon child view to the parent dashboard.',
  submit: 'Open Parent View',
};
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden p-4 md:p-8" style={themedSurface.shell}>
      ...
      >
        <div className="relative hidden flex-col justify-between p-8 md:flex lg:p-10" style={themedSurface.story}>
          <div aria-hidden className="pointer-events-none absolute right-6 top-8 h-28 w-28 rounded-full blur-2xl" style={{ background: 'rgba(255,122,89,0.22)' }} />
          <div aria-hidden className="pointer-events-none absolute left-10 top-16 h-16 w-16 rounded-full blur-2xl" style={{ background: 'rgba(116,211,255,0.22)' }} />
          <div aria-hidden className="pointer-events-none absolute bottom-6 right-6 h-24 w-24 rounded-full blur-2xl" style={{ background: 'rgba(255,216,107,0.2)' }} />
          <div>
            ...
            <p className="mt-3 max-w-sm text-[14px] font-semibold" style={{ color: 'var(--text-secondary)' }}>
              Sign in with your Student ID and continue your learning journey in the same cartoon theme as your dashboard.
            </p>
          </div>

          <div className="rounded-[28px] p-6" style={themedSurface.card}>
            <div className="mb-4 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl" style={themedSurface.pill}>
                <span className="text-2xl" aria-hidden>🚌</span>
              </div>
              <div>
                <div className="text-[12px] font-extrabold uppercase tracking-[0.2em]" style={{ color: '#ff7a59' }}>Std 1 Story Gate</div>
                <div className="text-[12px] font-semibold" style={{ color: 'var(--text-secondary)' }}>A playful login for little explorers</div>
              </div>
            </div>
            <div className="text-xl font-black" style={{ color: '#ff7a59' }}>Ready for story time?</div>
            <p className="mt-3 text-[13px] font-bold" style={{ color: 'var(--text-primary)' }}>Colors, games, and happy learning are waiting for you.</p>
            <p className="mt-1 text-[12px]" style={{ color: 'var(--text-secondary)' }}>Your login view now stays visually synced with the cartoon world.</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="rounded-full px-3 py-1 text-[11px] font-extrabold text-[#ff7a59]" style={themedSurface.pill}>🌈 Bright colors</span>
              <span className="rounded-full px-3 py-1 text-[11px] font-extrabold text-[#0891b2]" style={themedSurface.pill}>⭐ Fun learning</span>
              <span className="rounded-full px-3 py-1 text-[11px] font-extrabold text-[#2f9e44]" style={themedSurface.pill}>🧩 Playful steps</span>
            </div>
          </div>
        </div>
