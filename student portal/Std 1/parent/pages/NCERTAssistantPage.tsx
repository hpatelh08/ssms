...
const QUICK_ACTIONS = [
  ...
  { label: 'Give worksheet', icon: '📝', prompt: 'Generate a 5-question worksheet from this chapter suitable for a Class 1 student.' },
    ...
    >
          ...
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-10">
              ...
              <p className="text-xs text-gray-400 mt-1 max-w-[300px]">
                Use quick buttons below or type your question. AI stays grounded to NCERT Class 1 {subject} content.
              </p>
            </div>
          )}
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        ...
        <span className="text-[10px] text-gray-400 font-medium">
          AI responses are grounded to NCERT Class 1 {subject} textbook. No external data.
        </span>
      </div>
    </motion.div>
  );
...
export const NCERTAssistantPage: React.FC<{
  ...
  const subjectUnits: SyllabusItem[] = useMemo(
    () => selectedSubject === 'English' ? englishUnits : mathsChapters,
    [selectedSubject],
  );

  const subjectRhymes: RhymeEntry[] = useMemo(
    () => selectedSubject === 'English' ? englishRhymes : mathsRhymes,
    [selectedSubject],
  );
  ...
  const subjectIdSet = useMemo(
    () => selectedSubject === 'English' ? ENGLISH_IDS : MATHS_IDS,
    [selectedSubject],
  );
  ...
  const handleAskAIFromUnit = useCallback((title: string) => {
    ...
    prefillRef.current = `Explain "${title}" for Class 1 students.`;
          ...
          >
            <div className="text-center">
              ...
              <h2 className="text-xl font-black text-gray-800">
                {selectedSubject === 'English' ? 'English' : 'Maths'} Rhymes Corner
              </h2>
