...
const BigProgressBar: React.FC<{
  current: number; total: number; done: boolean; isCorrect: boolean | null;
  meta: typeof DIFF_META['easy'];
}> = React.memo(({ current, total, done, isCorrect, meta }) => (
  <div style={{
      ...
      return (
        <div key={i} style={{ position: 'relative' }}>
          <div style={{
            ...
            color: isPast || (ans && isCorrect) ? '#fff' : ans && !isCorrect ? '#fff' : isCurrent ? '#fff' : '#B0ADA8',
            background: isPast ? 'linear-gradient(135deg, #68D391, #48BB78)'
            ...
            boxShadow: isPast ? '0 3px 10px rgba(72,187,120,0.25)' : isCurrent ? `0 3px 14px ${meta.glowColor}` : 'none',
...
export const QuestionRenderer: React.FC<Props> = React.memo(({
  ...
  return (
    ...
    >
      ...
      <BigProgressBar current={questionIndex} total={totalInLevel} done={answered} isCorrect={isCorrect} meta={meta} />
