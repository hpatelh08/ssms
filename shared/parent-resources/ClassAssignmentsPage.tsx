  ...
  return (
    <div className="w-full px-3 lg:px-6 py-6 space-y-5 max-w-6xl mx-auto">
      <section
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(99,102,241,0.96), rgba(124,58,237,0.96), rgba(236,72,153,0.9))',
          boxShadow: '0 22px 50px rgba(99,102,241,0.24)',
        }}
      >
        <div className="relative z-10 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-black tracking-[0.35em] uppercase text-white/70">Class Assignments</p>
            <h1 className="text-2xl lg:text-3xl font-black mt-2">Assignments for your child&apos;s class</h1>
            <p className="mt-2 text-sm lg:text-base text-white/80 max-w-2xl">
              Only assignments for {studentProfile?.className || `Std ${studentProfile?.grade || ''}`}
      ...
      {loading ? (
        ...
        <div className="rounded-3xl bg-white/80 backdrop-blur-md border border-white/70 shadow-sm p-10 text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">Assignments uploaded for this class will appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {items.map((item) => {
            ...
            return (
              <article
                key={item._id}
                className="rounded-3xl bg-white/88 backdrop-blur-md border border-white/70 shadow-[0_16px_40px_rgba(99,102,241,0.10)] p-5"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-black tracking-[0.28em] uppercase text-indigo-500">Assignment</p>
                    <h2 className="text-xl font-black text-slate-800 mt-1 truncate">{item.title || 'Untitled assignment'}</h2>
                    <p className="text-sm text-slate-500 mt-1">
                      {subject || 'Subject not set'} {item.assignmentType ? `• ${item.assignmentType}` : ''}
                    </p>
                  </div>
                  <span className="shrink-0 px-3 py-1.5 rounded-full text-xs font-black bg-indigo-50 text-indigo-700">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-2xl bg-slate-50 p-3">
                    ...
                    <p className="font-bold text-slate-700 mt-1">{item.dueDate ? new Date(item.dueDate).toLocaleDateString('en-IN') : 'N/A'}</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 p-3">
                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Total marks</p>
                    <p className="font-bold text-slate-700 mt-1">{item.totalMarks ?? 'N/A'}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  ...
                  {fileLink ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-indigo-600 to-violet-600 px-4 py-2 text-xs font-black text-white shadow-md hover:shadow-lg transition-shadow"
                    >
                      View / Download
                    </a>
                  ) : (
